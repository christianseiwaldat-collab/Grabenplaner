...
test("Windows package assertions reject each missing runtime control module", {
  ...
  const harness = [
    ...
    "  if ([IO.Path]::GetDirectoryName($resolvedBase) -ne $expectedParent -or [IO.Path]::GetFileName($resolvedBase) -notmatch '^GrabenplanerRuntimeModules-[a-f0-9]{32}$') { throw 'Unsafe fixture cleanup path' }",
