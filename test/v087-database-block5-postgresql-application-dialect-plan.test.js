...
test("Block 5/7: der nicht ausführbare PostgreSQL-Plan deckt alle 1348 Statements genau einmal ab", () => {
  ...
  assert.equal(plan.entries.length, 1348);
  assert.equal(new Set(plan.entries.map((entry) => entry.statementId)).size, 1348);
...
test("Block 5/7: nur portable Einträge enthalten kompiliertes PostgreSQL-SQL", () => {
  ...
  assert.equal(summary.statementCount, 1348);
  assert.equal(summary.portableGeneratedCount, 1233);
  assert.equal(summary.requiresOverrideCount, 115);
...
test("Block 5/7: Summary erfasst die bekannten Override-Grenzen stabil", () => {
  ...
  assert.equal(counts["sqlite.json-functions"], 30);
...
test("Block 5/7: die Block-4-Plan-Fixture bleibt unverändert nicht ausführbar", () => {
  ...
  assert.equal(fixture.entries.length, 1348);
