...
test("Block 5/7: der nicht ausführbare PostgreSQL-Plan deckt alle 1015 Statements genau einmal ab", () => {
  ...
  assert.equal(plan.entries.length, 1015);
  assert.equal(new Set(plan.entries.map((entry) => entry.statementId)).size, 1015);
...
test("Block 5/7: nur portable Einträge enthalten kompiliertes PostgreSQL-SQL", () => {
  ...
  assert.equal(summary.statementCount, 1015);
  assert.equal(summary.portableGeneratedCount, 909);
...
test("Block 5/7: die Block-4-Plan-Fixture bleibt unverändert nicht ausführbar", () => {
  ...
  assert.equal(fixture.entries.length, 1015);
