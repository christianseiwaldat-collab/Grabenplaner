...
test("Block 5/7: der nicht ausführbare PostgreSQL-Plan deckt alle 1048 Statements genau einmal ab", () => {
  ...
  assert.equal(plan.entries.length, 1048);
  assert.equal(new Set(plan.entries.map((entry) => entry.statementId)).size, 1048);
...
test("Block 5/7: nur portable Einträge enthalten kompiliertes PostgreSQL-SQL", () => {
  ...
  assert.equal(summary.statementCount, 1048);
  assert.equal(summary.portableGeneratedCount, 942);
...
test("Block 5/7: die Block-4-Plan-Fixture bleibt unverändert nicht ausführbar", () => {
  ...
  assert.equal(fixture.entries.length, 1048);
