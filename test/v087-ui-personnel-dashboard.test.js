...
test("UI Block 1: Personalverwaltung trennt Dashboard-Ziel und Disclosure semantisch", () => {
  ...
  assert.match(app, /\["dashboard", "employees", "costCenters", "ruleDrafts", "collectiveAgreements", "vacations"\]\.includes\(requestedSection\)/);
});
...
test("UI Block 1: Dashboard ist persönlich anpassbar und zeigt nur erlaubte Arbeitsbereiche", () => {
  ...
  for (const id of ["employees", "requests", "timeTracking", "costCenters", "ruleDrafts", "collectiveAgreements", "vacations", "dataRequests"]) {
