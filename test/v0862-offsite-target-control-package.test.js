...
test("offsite module v6 owns the isolated target-control artifacts", () => {
  assert.equal(schema.moduleVersion, 6);
  ...
  assert.match(contract, /CURRENT_MODULE_VERSION = 6/);
  assert.match(compatibility, /SUPPORTED_MODULE_VERSIONS = new Set\(\[1, 2, 3, 4, 5, 6\]\)/);
});
