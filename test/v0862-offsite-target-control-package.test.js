...
test("offsite module v9 owns the isolated target-control artifacts", () => {
  ...
  assert.match(contract, /CURRENT_MODULE_VERSION = 9/);
  assert.match(compatibility, /SUPPORTED_MODULE_VERSIONS = new Set\(\[1, 2, 3, 4, 5, 6, 7, 8, 9\]\)/);
});
...
test("installer and uninstaller manage the target-control socket transactionally", () => {
  ...
  assert.match(installer, /\[1, 2, 3, 4, 5, 6, 7, 8, 9\]\.includes\(value\.moduleVersion\)/);
