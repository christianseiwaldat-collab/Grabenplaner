...
function harness(t,{initialize,health=()=>({status:200,body:{ok:true}})}={}){
 const root=fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(),'gp-smoke-startup-')));
 ...
 const subject={
  async initializeApplicationPersistence(){events.push('init-start');await initialize?.();initialized=true;events.push('init-done');},
 ...
 return {root,events,budgets,routes,reachedDataChecks,
