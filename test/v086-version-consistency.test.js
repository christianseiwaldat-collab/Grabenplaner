...
test("v0.92.66: Serverpaket, UI und aktuelle Dokumentation bleiben konsistent", () => {
  const packageJson = JSON.parse(read("package.json"));
  assert.equal(packageJson.version, "0.92.66-beta");
  ...
  assert.match(indexHtml, /v0\.92\.66 Beta/);
  ...
  assert.match(readme, /v0\.92\.66 Beta/);
  ...
  assert.equal(devcontainer.name, "Grabenplaner v0.92.66 Codespaces-Demo");
  ...
  assert.match(versionLog, /v0\.92\.27 Beta · Geschütztes Importfundament und Verkaufs-\/Kassenhistorie/);
  assert.match(versionLog, /v0\.92\.26 Beta · Direkter TradeFoto-Datenbankimport/);
  assert.match(versionLog, /v0\.92\.25 Beta · Artikelstamm-Verwaltung und kontrollierter TradeFoto-Import/);
  assert.match(versionLog, /v0\.92\.24 Beta · CRM, zentraler Artikelstamm und bearbeitbare Schnuppertermine/);
  assert.match(versionLog, /v0\.90\.7 Beta/);
  assert.match(versionLog, /v0\.92\.21 Beta · Delegierbare AL-Grundrechte und optionale Positionszeile/);
  assert.match(versionLog, /v0\.92\.20 Beta · Positionsverwaltung, Filialaufsicht und Dienstplansperre/);
  assert.match(versionLog, /v0\.92\.8 Beta · Schulungsprozesse und Fähigkeitsprofile/);
  assert.match(versionLog, /v0\.92\.17 Beta · Sichtbare Einsätze in anderen Filialen/);
  assert.match(versionLog, /v0\.92\.16 Beta · Präzisierte Wochenmatrix und standortübergreifende Einplanung/);
  assert.match(versionLog, /v0\.92\.15 Beta · Anpassbare Wochenmatrix und Katalogsuche/);
  assert.match(versionLog, /v0\.92\.14 Beta · Vollständige Bestellkatalog-Migration beim Serverstart/);
  assert.match(versionLog, /v0\.92\.13 Beta · Konfigurierbare Startwidgets und kompakter Bestellkatalog/);
  assert.match(versionLog, /v0\.92\.12 Beta · Dienstplan-PDFs, persönliches Startdashboard und Dienstsuche/);
  ...
  assert.match(versionLog, /v0\.89\.1 Beta/);
  assert.match(versionLog, /v0\.89 Beta/);
  assert.match(versionLog, /v0\.88\.6 Beta/);
  assert.match(versionLog, /v0\.88\.5 Beta/);
