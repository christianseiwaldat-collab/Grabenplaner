...
test("DB Block 5: der generierte Dialektplan ist niemals selbst ein ausfuehrbarer Vollkatalog", () => {
  ...
  assert.throws(
    ...
    /acceptance gate is closed at 0\/1069/,
  );
  assert.throws(
...
test("DB Block 5: auch 1069 Deklarationen bleiben bei 0/1069 Receipts ohne Vollfreigabe", () => {
  ...
  assert.throws(
    ...
    /must cover exactly 1069 statements/,
  );
  assert.throws(
  ...
  assert.equal(deliberatelyPartial.acceptance.requiredReceiptCount, 1069);
  assert.throws(
  ...
  assert.throws(
    ...
    /partial PostgreSQL development catalog cannot contain all 1069 statements/,
  );
  assert.equal(deliberatelyPartial.entries.length, 1068);
  assert.equal(deliberatelyPartial.providerEntries.length, 1068);
