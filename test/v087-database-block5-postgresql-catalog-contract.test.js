...
test("DB Block 5: der generierte Dialektplan ist niemals selbst ein ausfuehrbarer Vollkatalog", () => {
  ...
  assert.throws(
    ...
    /acceptance gate is closed at 0\/1051/,
  );
  assert.throws(
...
test("DB Block 5: auch 1051 Deklarationen bleiben bei 0/1051 Receipts ohne Vollfreigabe", () => {
  ...
  assert.throws(
    ...
    /must cover exactly 1051 statements/,
  );
  assert.throws(
  ...
  assert.equal(deliberatelyPartial.acceptance.requiredReceiptCount, 1051);
  assert.throws(
  ...
  assert.throws(
    ...
    /partial PostgreSQL development catalog cannot contain all 1051 statements/,
  );
  assert.equal(deliberatelyPartial.entries.length, 1050);
  assert.equal(deliberatelyPartial.providerEntries.length, 1050);
