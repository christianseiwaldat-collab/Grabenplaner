...
test("DB Block 5: der generierte Dialektplan ist niemals selbst ein ausfuehrbarer Vollkatalog", () => {
  ...
  assert.throws(
    ...
    /acceptance gate is closed at 0\/899/,
  );
  assert.throws(
...
test("DB Block 5: auch 899 Deklarationen bleiben bei 0/899 Receipts ohne Vollfreigabe", () => {
  ...
  assert.throws(
    ...
    /must cover exactly 899 statements/,
  );
  assert.throws(
  ...
  assert.equal(deliberatelyPartial.acceptance.requiredReceiptCount, 899);
  assert.throws(
  ...
  assert.throws(
    ...
    /partial PostgreSQL development catalog cannot contain all 899 statements/,
  );
  assert.equal(deliberatelyPartial.entries.length, 898);
  assert.equal(deliberatelyPartial.providerEntries.length, 898);
