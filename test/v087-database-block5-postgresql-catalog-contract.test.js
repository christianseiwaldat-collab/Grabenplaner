...
test("DB Block 5: der generierte Dialektplan ist niemals selbst ein ausfuehrbarer Vollkatalog", () => {
  ...
  assert.throws(
    ...
    /acceptance gate is closed at 0\/1099/,
  );
  assert.throws(
...
test("DB Block 5: auch 1099 Deklarationen bleiben bei 0/1099 Receipts ohne Vollfreigabe", () => {
  ...
  assert.throws(
    ...
    /must cover exactly 1099 statements/,
  );
  assert.throws(
  ...
  assert.equal(deliberatelyPartial.acceptance.requiredReceiptCount, 1099);
  assert.throws(
  ...
  assert.throws(
    ...
    /partial PostgreSQL development catalog cannot contain all 1099 statements/,
  );
  assert.equal(deliberatelyPartial.entries.length, 1098);
  assert.equal(deliberatelyPartial.providerEntries.length, 1098);
