...
test("DB Block 5: der generierte Dialektplan ist niemals selbst ein ausfuehrbarer Vollkatalog", () => {
  ...
  assert.throws(
    ...
    /acceptance gate is closed at 0\/1015/,
  );
  assert.throws(
...
test("DB Block 5: auch 1015 Deklarationen bleiben bei 0/1015 Receipts ohne Vollfreigabe", () => {
  ...
  assert.throws(
    ...
    /must cover exactly 1015 statements/,
  );
  assert.throws(
  ...
  assert.equal(deliberatelyPartial.acceptance.requiredReceiptCount, 1015);
  assert.throws(
  ...
  assert.throws(
    ...
    /partial PostgreSQL development catalog cannot contain all 1015 statements/,
  );
  assert.equal(deliberatelyPartial.entries.length, 1014);
  assert.equal(deliberatelyPartial.providerEntries.length, 1014);
