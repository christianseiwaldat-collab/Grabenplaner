...
test("DB Block 5: der generierte Dialektplan ist niemals selbst ein ausfuehrbarer Vollkatalog", () => {
  ...
  assert.throws(
    ...
    /acceptance gate is closed at 0\/928/,
  );
  assert.throws(
...
test("DB Block 5: auch 928 Deklarationen bleiben bei 0/928 Receipts ohne Vollfreigabe", () => {
  ...
  assert.throws(
    ...
    /must cover exactly 928 statements/,
  );
  assert.throws(
  ...
  assert.equal(deliberatelyPartial.acceptance.requiredReceiptCount, 928);
  assert.throws(
  ...
  assert.throws(
    ...
    /partial PostgreSQL development catalog cannot contain all 928 statements/,
  );
  assert.equal(deliberatelyPartial.entries.length, 927);
  assert.equal(deliberatelyPartial.providerEntries.length, 927);
