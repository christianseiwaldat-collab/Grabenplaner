...
test("DB Block 5: der generierte Dialektplan ist niemals selbst ein ausfuehrbarer Vollkatalog", () => {
  ...
  assert.throws(
    ...
    /acceptance gate is closed at 0\/891/,
  );
  assert.throws(
...
test("DB Block 5: auch 891 Deklarationen bleiben bei 0/891 Receipts ohne Vollfreigabe", () => {
  ...
  assert.throws(
    ...
    /must cover exactly 891 statements/,
  );
  assert.throws(
  ...
  assert.equal(deliberatelyPartial.acceptance.requiredReceiptCount, 891);
  assert.throws(
  ...
  assert.throws(
    ...
    /partial PostgreSQL development catalog cannot contain all 891 statements/,
  );
  assert.equal(deliberatelyPartial.entries.length, 890);
  assert.equal(deliberatelyPartial.providerEntries.length, 890);
