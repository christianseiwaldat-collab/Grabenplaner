...
test("DB Block 5: der generierte Dialektplan ist niemals selbst ein ausfuehrbarer Vollkatalog", () => {
  ...
  assert.throws(
    ...
    /acceptance gate is closed at 0\/1048/,
  );
  assert.throws(
...
test("DB Block 5: auch 1048 Deklarationen bleiben bei 0/1048 Receipts ohne Vollfreigabe", () => {
  ...
  assert.throws(
    ...
    /must cover exactly 1048 statements/,
  );
  assert.throws(
  ...
  assert.equal(deliberatelyPartial.acceptance.requiredReceiptCount, 1048);
  assert.throws(
  ...
  assert.throws(
    ...
    /partial PostgreSQL development catalog cannot contain all 1048 statements/,
  );
  assert.equal(deliberatelyPartial.entries.length, 1047);
  assert.equal(deliberatelyPartial.providerEntries.length, 1047);
