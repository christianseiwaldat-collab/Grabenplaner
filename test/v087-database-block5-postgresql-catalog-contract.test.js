...
test("DB Block 5: der generierte Dialektplan ist niemals selbst ein ausfuehrbarer Vollkatalog", () => {
  ...
  assert.throws(
    ...
    /acceptance gate is closed at 0\/1013/,
  );
  assert.throws(
...
test("DB Block 5: auch 1013 Deklarationen bleiben bei 0/1013 Receipts ohne Vollfreigabe", () => {
  ...
  assert.throws(
    ...
    /must cover exactly 1013 statements/,
  );
  assert.throws(
  ...
  assert.equal(deliberatelyPartial.acceptance.requiredReceiptCount, 1013);
  assert.throws(
  ...
  assert.throws(
    ...
    /partial PostgreSQL development catalog cannot contain all 1013 statements/,
  );
  assert.equal(deliberatelyPartial.entries.length, 1012);
  assert.equal(deliberatelyPartial.providerEntries.length, 1012);
