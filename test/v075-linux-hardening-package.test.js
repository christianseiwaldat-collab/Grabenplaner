...
test("v0.75 Linux package builder expands the complete hardening artifact list", {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(temporaryRoot, "Grabenplaner-Server-v0.80.2-beta-linux-x64.zip")));
    assert.ok(fs.existsSync(path.join(temporaryRoot, "Grabenplaner-Server-v0.80.2-beta-linux-x64.zip.sha256")));
  } finally {
    fs.rmSync(temporaryRoot, { recursive: true, force: true });
  }
});
