...
test("v0.75 Linux package builder expands the complete hardening artifact list", {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(temporaryRoot, "Grabenplaner-Server-v0.75.1-beta-linux-x64.zip")));
    assert.ok(fs.existsSync(path.join(temporaryRoot, "Grabenplaner-Server-v0.75.1-beta-linux-x64.zip.sha256")));
  } finally {
    fs.rmSync(temporaryRoot, { recursive: true, force: true });
  }
});
...
test("v0.75 hardening package schema stays exact and explicitly activated", () => {
  ...
  assert.equal(
    read("server-tools/linux/hardening/templates/60-grabenplaner-journald.conf"),
    read("server-tools/linux/hardening/templates/zz-grabenplaner-journald.conf"),
    "The legacy signature and the prioritized replacement must describe the same policy.",
  );
});
