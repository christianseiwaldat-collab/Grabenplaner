...
test("v0.78.1: Paket, Launcher, UI und aktuelle Dokumentation bleiben konsistent", () => {
  const packageJson = JSON.parse(read("package.json"));
  assert.equal(packageJson.version, "0.78.1-beta");

  assert.equal(fs.existsSync(path.join(root, "Grabenplaner v0.78.1 Beta starten.cmd")), true);
  ...
  assert.match(indexHtml, /v0\.78\.1 Beta/);
  assert.match(readme, /v0\.78\.1 Beta/);
  ...
  assert.match(usbNotes, /Version v0\.78\.1 Beta/);
