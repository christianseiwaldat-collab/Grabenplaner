...
test("v0.87 Datenbank Block 5: historische Baselines und aktuelle Phasen bleiben auditierbar", () => {
  ...
  // Personalmodul-, Lernmodul- inklusive Fortschritt, Filialbestellungs- und Verkaufsanalyse-Schemaoperationen sowie die persistenzfreie Funktionssuche sind explizit klassifiziert.
  assert.equal(report.summary.productionDirectFiles, 64);
  ...
  assert.equal(report.phase5Progress.dialectPlanStatementCount, 1099);
  assert.equal(
...
test("v0.87 Datenbank Block 5: Dokumentation und CI bilden den nicht produktiven Zwischenstand ab", () => {
  ...
  assert.match(phase4, /1099 Statementvertr/);
  assert.match(phase4, /SQLite-Baseline \| 29/);
  assert.match(phase4, /SQLite-Dialektvariante \| 1069/);
  assert.match(phase4, /996[\s\S]{0,100}Dollar-Parameter/i);
  ...
  assert.match(phase4, /988 Syntaxkandidaten \(`portable-generated`\)/);
  ...
  assert.match(phase5, /Abdeckungen 4\/4, 2\/2, 2\/2 und 1\/1[\s\S]{0,120}0\/1099/);
  ...
  assert.match(
    strategy,
    /1099 Anwendungsstatements:[\s\S]{0,60}988[\s\S]{0,100}`portable-generated`[\s\S]{0,60}111[\s\S]{0,100}`requires-override`[\s\S]{0,100}0 von 1099[\s\S]{0,100}Vollanwendungskatalog/i,
  );
  assert.match(
    strategy,
