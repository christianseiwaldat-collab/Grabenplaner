...
test("v0.87 Datenbank Block 5: historische Baselines und aktuelle Phasen bleiben auditierbar", () => {
  ...
  // Filialbestellungs- und Verkaufsanalyse-Schemaoperationen sowie die persistenzfreie
  // Funktionssuche sind explizit klassifiziert.
  assert.equal(report.summary.productionDirectFiles, 66);
  ...
  assert.equal(report.phase5Progress.dialectPlanStatementCount, 1115);
  assert.equal(
...
test("v0.87 Datenbank Block 5: Dokumentation und CI bilden den nicht produktiven Zwischenstand ab", () => {
  ...
  assert.match(phase4, /1115 Statementvertr/);
  assert.match(phase4, /SQLite-Baseline \| 33/);
  assert.match(phase4, /SQLite-Dialektvariante \| 1082/);
  assert.match(phase4, /1009[\s\S]{0,100}Dollar-Parameter/i);
  ...
  assert.match(phase4, /1004 Syntaxkandidaten \(`portable-generated`\)/);
  ...
  assert.match(phase5, /Abdeckungen 4\/4, 2\/2, 2\/2 und 1\/1[\s\S]{0,120}0\/1115/);
  ...
  assert.match(
    strategy,
    /1115 Anwendungsstatements:[\s\S]{0,60}1004[\s\S]{0,100}`portable-generated`[\s\S]{0,60}111[\s\S]{0,100}`requires-override`[\s\S]{0,100}0 von 1115[\s\S]{0,100}Vollanwendungskatalog/i,
  );
  assert.match(
    strategy,
