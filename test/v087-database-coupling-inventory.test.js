...
test("v0.87 Datenbank Block 5: historische Baselines und aktuelle Phasen bleiben auditierbar", () => {
  ...
  assert.equal(report.summary.productionDirectFiles, 53);
  ...
  assert.equal(report.phase5Progress.dialectPlanStatementCount, 950);
  assert.equal(
...
test("v0.87 Datenbank Block 5: Dokumentation und CI bilden den nicht produktiven Zwischenstand ab", () => {
  ...
  assert.match(phase4, /950 Statementvertr/);
  assert.match(phase4, /SQLite-Baseline \| 24/);
  assert.match(phase4, /SQLite-Dialektvariante \| 926/);
  assert.match(phase4, /853[\s\S]{0,100}Dollar-Parameter/i);
  ...
  assert.match(phase4, /848 Syntaxkandidaten \(`portable-generated`\)/);
  ...
  assert.match(phase5, /Abdeckungen 4\/4, 2\/2, 2\/2 und 1\/1[\s\S]{0,120}0\/950/);
  ...
  assert.match(
    strategy,
    /950 Anwendungsstatements:[\s\S]{0,60}848[\s\S]{0,100}`portable-generated`[\s\S]{0,60}102[\s\S]{0,100}`requires-override`[\s\S]{0,100}0 von 950[\s\S]{0,100}Vollanwendungskatalog/i,
  );
  assert.match(
    strategy,
