...
test("v0.74 separates redacted status and technical diagnostics permissions", () => {
  ...
  assert.match(server, /app\.get\("\/api\/server-status"[\s\S]*serverStatusForActor\(\(await serverDiagnostics\(\)\), actor\)/);
  assert.match(server, /app\.get\("\/api\/server-diagnostics"[\s\S]*"system:diagnostics:technical"/);
  assert.match(server, /serverStatus: statusAllowed \? \(await serverStatusForActor\(diagnosticSnapshot, request\.portalSession\)\) : null/);
