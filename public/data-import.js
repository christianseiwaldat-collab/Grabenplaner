(function attach(root,factory){'use strict';const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.GrabenplanerDataImport=api;
  ...
  function renderSources(items,selectedId) {
    ...
    return `<div class="data-import-scroll data-import-source-table" tabindex="0" role="region" aria-label="Hochgeladene Datenbanken"><table><caption>Hochgeladene Datenbanken · Datei für Details auswählen</caption><thead><tr><th scope="col">Originaldatei</th><th scope="col">Status</th><th scope="col">Letzte Aktualisierung</th></tr></thead><tbody>${items.map(s=>`<tr${s.id===selectedId?' class="is-selected"':''}><th scope="row"><button type="button" data-i-source="${escape(s.id)}"${s.id===selectedId?' aria-current="true"':''}>${escape(sourceFileName(s))}</button>${!s.fileName?'<small>Originalname bei diesem älteren Import nicht erfasst; angezeigt wird der Dateityp.</small>':''}</th><td>${escape(label(s.background?.status||s.status))}</td><td><time datetime="${escape(s.updatedAt||s.createdAt||'')}">${escape(dateTime(s.updatedAt||s.createdAt))}</time></td></tr>`).join('')}</tbody></table></div><p class="data-import-note">Letzte Aktualisierung = Bearbeitungsstand im GP, nicht das Datum des letzten Umsatzes.</p>`;
  }
  function renderSource(source,projection={}) {
    ...
    return `<header><h3>${escape(sourceFileName(source))}</h3><p>${escape(sourceLabel(source.kind))} · ${escape(label(job?.status||source.status))} · ${number(received)} gelesene / ${number(expected)} deklarierte Zeilen</p></header>
      ...
      <p class="data-import-note">Hochgeladen: ${escape(dateTime(source.createdAt))} · Zuletzt aktualisiert: ${escape(dateTime(source.updatedAt||source.createdAt))}</p><details><summary>Technische Dateiidentität</summary><p class="data-import-note">Dateifingerabdruck <code>${escape(source.fileSha256)}</code></p></details>
      ...
      ${source.storage==='cash-compact-v1'?`<p class="data-import-note">Gesamte Kassenhistorie · alle Zeiträume · alle Filialen in einem Import. ${number(source.verifiedRows)} gespeicherte Zeilen vollständig zurückgelesen und verglichen. Die spätere Filialzuordnung dient nur der Auswertung; sie schränkt den Import nicht ein.</p>`:''}
    ...
    async function load() {
      ...
      try {
        ...
        body.innerHTML=`<p>${escape(context.message)}</p>
          ...
          <div data-i="sources"></div><button type="button" data-i="next" hidden>Ältere Importe</button><section data-i="detail"></section><section data-i="publication"></section><section data-i="log"></section>`;
  ...
  return {mount,renderSource,renderSources};
}));
