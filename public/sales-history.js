(function attach(root, factory) {
  ...
  function renderSummary(result) {
    ...
    return `<section class="sales-history-summary"><h3>${result.totals ? 'Geprüfte importierte Positionen' : 'Datenabdeckung und offene Prüfungen'}</h3>
      <p>${escape(c.label)}</p><p>${c.complete ? `${c.counts.records} passende importierte Datensätze` : `Begrenzte Auswahl: ${c.limit} Datensätze – keine vollständige Zeitraumsauswertung`} · ${c.counts.checked} geprüft · ${c.counts.review} offen</p>
  ...
  function mount(root, { api, customerId = null, calendarFactory = globalThis.GrabenplanerDateRangeCalendar?.createDateRangeCalendar }) {
    ...
    let from = '', to = '', sort = { key: 'date', direction: -1 };
    const el = name => body.querySelector(`[data-h="${name}"]`);
    function cancel() { generation++; controller?.abort(); controller = null; }
    function clearResults() { cancel(); lastResult = null; lastQuery = null; if (el('results')) el('results').innerHTML = ''; if (el('next')) el('next').hidden = true; root.removeAttribute('aria-busy'); }
    ...
    async function search(next = false) {
      ...
      try {
        const url = customerId ? `/api/crm/customers/${encodeURIComponent(customerId)}/purchases/search` : '/api/sales-history/search';
        ...
        if (disposed || ticket !== generation) return;
        lastResult = null; el('message').textContent = error.message || 'Daten konnten nicht gelesen werden.';
    ...
    async function load() {
      ...
      try {
        context = await api('/api/sales-history/context', { signal: controller.signal });
        if (disposed || ticket !== generation) return;
        ...
        body.innerHTML = `<p>${customerId ? 'Nur ausdrücklich zugeordnete Kundenkäufe in deinen freigegebenen Filialen.' : 'Einzelverkäufe sind unabhängig von den bisherigen PDF-Statistikberichten.'} Suche wird erst nach Bestätigung ausgeführt.</p>
          ...
          <div data-h="results"></div><button data-h="next" type="button" class="secondary-button" hidden>Nächste Seite</button>
    ...
    function onToggle() { if (root.open) void load(); }
