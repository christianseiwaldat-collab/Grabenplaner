(function attach(root, factory) {
  ...
  function renderTable(items, kind, sort = { key: 'date', direction: -1 }) {
    ...
    return `<div class="sales-history-table-scroll" tabindex="0" role="region" aria-label="Suchergebnisse, sortierbare aktuelle Seite"><table>
      ...
      <dt>Prüfung</dt><dd>${escape(row.metric ? ({ sale: 'Verkauf', return: 'Rückgabe', adjustment: 'Rabatt / Gegenbuchung', payment: 'Zahlungsmittel', voucher_issue: 'Gutscheinausgabe · kein Warenumsatz', uid_clearing: 'UID-Zwischenbuchung · kein Warenumsatz', excluded: 'Ausgeschlossen' }[row.metric.status] || 'Geprüft') : (row.issues || []).map(issueText).join(' · '))}</dd></dl></details></td></tr>`).join('') || '<tr><td colspan="6">Keine zugeordneten Datensätze im gewählten Bereich. Das beweist keinen Umsatz von null.</td></tr>'}</tbody></table></div>`;
  }
