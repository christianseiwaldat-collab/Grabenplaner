(function attachTradeInsights(host,factory){
 ...
 'use strict';
 const tabs=Object.freeze(['purchasing','transfers','stock-summary','inventory','repairs','customer-history','device-history']);
...
`;
 ...
 function render(){
  const columns=model.columns(kind,context.projection),sorted=model.sortRows(rows,columns,sortKey,sortDirection);
 ...
 async function openJob(id){
  ...
  try{
   ...
   currentJob=id;opening=null;rows=data.result.rows||[];summary=data.result.totals||null;scanned=data.processed;sortKey='';sortDirection='asc';
   ...
   q('snapshot-info').textContent='Abfrage '+timestamp(data.created)+' · gespeichert '+timestamp(data.completedAt)+' · damaliger Ergebnisstand';
 ...
 async function load(){
  ...
  if(kind!=='purchasing')delete query.supplier;
  ...
  const button=form.querySelector('button');button.disabled=true;const ticket=generation;
 ...
 async function choose(button,{replace=false}={}){
  ...
  root.querySelector('[data-customer]').hidden=!['repairs','customer-history','device-history'].includes(kind);root.querySelector('[data-serial]').hidden=!['repairs','device-history'].includes(kind);form.elements.customer.required=kind==='customer-history';form.elements.serial.required=kind==='device-history';
   root.querySelector('[data-supplier]').hidden=kind!=='purchasing';const stock=['inventory','stock-summary'].includes(kind);form.elements.locationId.required=false;
  ...
  q('query-label').textContent=['inventory','stock-summary'].includes(kind)?'Artikelnr. oder Bezeichnung':kind==='purchasing'?'Artikel oder Bestellnummer':'Artikel oder Vorgangsnummer';form.elements.query.required=false;
 ...
 async function activate(tab=requestedTab){
  ...
  if(!context){
   ...
   const pending=api('context').then(data=>{
    ...
    for(const b of root.querySelectorAll('[data-kind]')){
     b.disabled=!data.projection?.[['inventory','stock-summary'].includes(b.dataset.kind)?'inventory':b.dataset.kind==='repairs'?'repairs':['customer-history','device-history'].includes(b.dataset.kind)?'customers':'purchasing'];
    }
  ...
  else {if(kind!==requestedTab)onTabChange(kind,{replace:true});void refreshJobs();}
 }
