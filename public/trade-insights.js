(function attachTradeInsights(host,factory){
 ...
 'use strict';
 const tabs=Object.freeze(['purchasing','transfers','inventory','prices','repairs','customer-history','device-history']);
...
`;
 ...
 async function choose(button,{replace=false}={}){
  ...
  root.querySelector('[data-supplier]').hidden=kind!=='purchasing';const stock=['inventory','prices'].includes(kind);
  root.querySelectorAll('[data-stock]').forEach(n=>n.hidden=!stock);root.querySelectorAll('[data-inventory]').forEach(n=>n.hidden=kind!=='inventory');
  q('classification').hidden=kind!=='inventory'||!context.projection.classify;
  q('query-label').textContent=kind==='prices'?'Exakte Artikelnr.':kind==='inventory'?'Artikelnr. oder Bezeichnung':kind==='purchasing'?'Artikel oder Bestellnummer':'Artikel oder Vorgangsnummer';form.elements.query.required=kind==='prices';
 ...
 function suspend(){
  ...
  if(searching){status.textContent='Suche angehalten. Auswahl prüfen und neu suchen oder fortsetzen.';more.hidden=!next||rows.length>=1000;}
 }
 async function activate(tab=requestedTab){
  ...
  if(!context){
   ...
   const pending=api('context').then(data=>{
    ...
    for(const b of root.querySelectorAll('[data-kind]')){
     b.disabled=!data.projection?.[['inventory','prices'].includes(b.dataset.kind)?'inventory':b.dataset.kind==='repairs'?'repairs':['customer-history','device-history'].includes(b.dataset.kind)?'customers':'purchasing'];
    }
  ...
  if(!selected||button.dataset.kind!==kind||(['inventory','prices'].includes(kind)&&!metadata))await choose(button,{replace:button.dataset.kind!==requestedTab});
