 ...
 async function load(continuing=false){
  ...
  let pages=0;try{
   do{
    ...
    next=data.next;if(data.cumulative){scanned=data.scanned||0;rows=data.rows||[];}else{scanned+=data.scanned||0;rows.push(...(data.rows||[]));}q('note').textContent=data.available===false?'Noch kein vollständig übernommener Quelldatenstand vorhanden.':`${data.note||''} ${kind==='stock-summary'?'Importiert am':'Datenstand'}: ${date(data.sourceDate)}.`;
    ...
    if(kind==='stock-summary'&&summary){
     ...
     const html=`<section class="trade-stock-summary" aria-label="Gesamtbestand"><p>${summary.complete?'Alle Quellenpositionen geprüft':'Zwischenstand – Gesamtbestand noch nicht vollständig'} · ${number(scanned)} geprüft</p><div class="facts">${card('Positionen mit positivem Bestand',number(summary.positivePositions))}${card('Erfasster Bestand (vorläufig)',number(summary.positiveQuantity))}${context.projection.costs?card('Bewerteter Warenwert netto (vorläufig)',money(summary.provisionalNet))+card('Davon bestätigte Lagerware',money(summary.confirmedNet)):''}</div><p>${number(summary.excluded)} ausgeschlossene Positionen · ${number(summary.ambiguous)} mehrdeutig · ${number(summary.missingArticle)} ohne Artikelstamm · ${number(summary.missingQuantity)} ohne Menge · ${number(summary.negative)} negative Bestände · ${number(summary.unclassified)} ungeklärte Artikelarten${context.projection.costs?` · ${number(summary.missingCost)} ohne gültigen EK · ${number(summary.zeroCost)} mit EK 0`:''}. Prüffälle können sich überschneiden. Dienstleistungen zählen nicht zum Lagerbestand. Der vorläufige Bestand enthält Artikel mit noch ungeklärter Einordnung. Bestätigte Lagerware wird getrennt ausgewiesen. Bei offenen Prüffällen ist der Warenwert nur ein Teilwert.</p></section>`;
