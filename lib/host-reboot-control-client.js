...
const BOOT_ID_CREDENTIAL_NAME = "host-boot-id";
const MAX_BOOT_ID_CREDENTIAL_BYTES = 64;
...
function credentialBootIdPath(credentialsDirectory) {
  const rawDirectory = String(credentialsDirectory || "");
  if (!rawDirectory
    || rawDirectory.includes("\0")
    || !path.posix.isAbsolute(rawDirectory)
    || path.posix.normalize(rawDirectory) !== rawDirectory
    || rawDirectory === "/") {
    return null;
  }
  return path.posix.join(rawDirectory, BOOT_ID_CREDENTIAL_NAME);
}

function safeCredentialMetadata(directoryStat, credentialStat) {
  const directoryMode = numericStatValue(directoryStat?.mode);
  const directoryLinks = numericStatValue(directoryStat?.nlink);
  const credentialMode = numericStatValue(credentialStat?.mode);
  const credentialLinks = numericStatValue(credentialStat?.nlink);
  const credentialSize = numericStatValue(credentialStat?.size);
  return Boolean(directoryStat
    && credentialStat
    && typeof directoryStat.isDirectory === "function"
    && directoryStat.isDirectory()
    && (typeof directoryStat.isSymbolicLink !== "function" || !directoryStat.isSymbolicLink())
    && typeof credentialStat.isFile === "function"
    && credentialStat.isFile()
    && (typeof credentialStat.isSymbolicLink !== "function" || !credentialStat.isSymbolicLink())
    && directoryMode !== null
    && directoryLinks !== null
    && credentialMode !== null
    ...
    && credentialLinks === 1n
    && credentialSize > 0n
    && credentialSize <= BigInt(MAX_BOOT_ID_CREDENTIAL_BYTES)
    && (directoryMode & 0o022n) === 0n
    && (credentialMode & 0o022n) === 0n);
}

function readCredentialBootId(credentialsDirectory, readFile, lstat) {
  const credentialPath = credentialBootIdPath(credentialsDirectory);
  if (!credentialPath) return null;
  try {
    const directoryStat = lstat(path.posix.dirname(credentialPath), { bigint: true });
    const credentialStat = lstat(credentialPath, { bigint: true });
    if (!safeCredentialMetadata(directoryStat, credentialStat)) return null;
    const rawBootId = String(readFile(credentialPath, "utf8"));
    const bootId = rawBootId.endsWith("\n") ? rawBootId.slice(0, -1) : rawBootId;
    return rawBootId.length <= MAX_BOOT_ID_CREDENTIAL_BYTES
...
function readHostBootGeneration(options = {}) {
  ...
  const credentialsDirectory = Object.hasOwn(options, "credentialsDirectory")
    ? options.credentialsDirectory
    : process.env.CREDENTIALS_DIRECTORY;
  const credentialBootId = readCredentialBootId(credentialsDirectory, readFile, lstat);
  if (credentialBootId) return hashedBootGeneration("boot-id", credentialBootId);
  try {
