...
function defineTradeFotoSalesPolicy(input) {
  ...
  for (const rule of input.statusRules) {
    ...
    if (!['sale', 'return', 'adjustment', 'payment', 'voucher_issue', 'uid_clearing', 'excluded'].includes(rule.status) || !['positive', 'negative', 'zero', 'any'].includes(rule.quantitySign)) C.fail('IMPORT_SALES_STATUS_RULES');
...
function reconcileTradeFotoReceipt({ head, lines, headRevision, snapshotDate, policy = null, coverage = null, scopeId, sourceInstance }) {
  ...
  for (const line of lines) {
    ...
    const countKey = result.status === 'sale' ? 'sales' : result.status === 'return' ? 'returns' : result.status === 'adjustment' ? 'adjustments' : result.status === 'payment' ? 'payments' : result.status === 'voucher_issue' ? 'voucherIssues' : result.status === 'uid_clearing' ? 'uidClearings' : 'excluded';
