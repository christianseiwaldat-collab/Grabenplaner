...
function defineTradeFotoSalesPolicy(input) {
  C.exact(input, ['id', 'version', 'scopeId', 'sourceInstance', 'schemaSha256', 'evidenceSha256', 'approvedBy', 'approvedAt',
    'currency', 'minorUnits', 'priceBasis', 'priceMeaning', 'headerBasis', 'rounding', 'vatRates', 'statusRules', 'headerToleranceMinor', 'headerZeroMeaning', 'zeroPriceVatCodes']);
...
function classifyLine(row, policy) {
  ...
  if (policy.zeroPriceVatCodes?.includes(code) && price !== 0n) return { issue: 'VAT_CODE_UNKNOWN' };
...
function reconcileTradeFotoReceipt({ head, lines, headRevision, snapshotDate, policy = null, coverage = null, scopeId, sourceInstance }) {
  ...
  let header = null, difference = null, headerStatus = 'checked';
  try { header = minor(head.RechnungsBetrag, policy.minorUnits);
    ...
    else { difference = (policy.headerBasis === 'gross' ? gross : net) - header;
  ...
  return { canAggregate: !issues.size, issues: [...issues].sort(), counts, policy: { id: policy.id, version: policy.version, fingerprint: policy.fingerprint },
    ...
    reconciliation: { headerBasis: policy.headerBasis, headerStatus, difference: difference === null ? null : format(difference, policy.minorUnits), toleranceMinor: policy.headerToleranceMinor } };
}
