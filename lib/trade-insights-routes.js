...
function registerTradeInsightRoutes(app,{runtime,dispatchRead,requireSession,refreshSession,assertCsrf}){
 ...
 for(const kind of ['purchasing','transfers','inventory','prices','stock-metadata','classification','classification-save','repairs','repair-detail','repair-save','customer-history','device-history'])app.post('/api/trade-insights/'+kind,route(kind));
}
