...
function createLocalBackupArchive({ backupDirectory, binary, binarySha256, vault, minimumFreeBytes = RESERVE_BYTES, expectedStream, verifyBinaryOnConstruction = true, clock = () => new Date() } = {}) {
  ...
  function preflightBackup() {
    ...
    return lock((c, cfg) => {
      ...
      free(); run(["check", "--read-data"], c); free();
  ...
  function maintainRetention(latestSnapshot, { verifyPair } = {}) {
    ...
    return lock((c, cfg) => {
      ...
      // A complete cryptographic read checks every archive point, then an
      // independent GP restore proves the retained newest point is usable.
      ...
      // Also reclaims already-forgotten packs after an explicit reconciliation.
      run(["prune", "--max-unused", "0", "--max-repack-size", "0"], c);
      // Never delete raw fallbacks using a proof obtained before mutation.
      run(["check", "--read-data"], c);
      const afterPrune = restore(c, latest, verifyPair); afterPrune.cleanup();
      inventory(c, cfg);
