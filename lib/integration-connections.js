...
const CONNECTION_KINDS = Object.freeze({
  personnel_sql_source: Object.freeze({
    ...
    direction: "import",
    purpose: "personnel",
    contractId: CONTRACT_IDS.personnelSqlView,
  ...
  payroll_https_target: Object.freeze({
    ...
    direction: "export",
    purpose: "payroll",
    contractId: CONTRACT_IDS.payrollHttpsJson,
...
function normalizeSqlConfiguration(provider, value) {
  ...
  if (configuration.contractId !== undefined && configuration.contractId !== CONTRACT_IDS.personnelSqlView) {
    throw connectionError("INTEGRATION_CONNECTION_CONTRACT_INVALID");
  }
  return {
    contractId: CONTRACT_IDS.personnelSqlView,
...
function normalizeApiConfiguration(provider, value) {
  ...
  const configuration = requirePlainObject(value, "INTEGRATION_CONNECTION_CONFIGURATION_INVALID");
  assertNoPublicSecrets(configuration);
  if (configuration.contractId !== undefined && configuration.contractId !== CONTRACT_IDS.payrollHttpsJson) {
    throw connectionError("INTEGRATION_CONNECTION_CONTRACT_INVALID");
  }
...
function safeConfigurationProjection(kind, configuration = {}) {
  ...
  if (kind === "personnel_sql_source") {
    const candidate = Object.fromEntries(["contractId", "host", "port", "database", "instanceName", "schemaName", "objectName", "objectType", "allowedColumns", "tlsMode", "timeoutMs", "rowLimit", "scope"]
  ...
  if (kind === "payroll_https_target") {
    const candidate = Object.fromEntries(["contractId", "endpoint", "method", "authenticationType", "apiKeyHeader", "idempotencyHeader", "timeoutMs", "requestLimitBytes", "responseLimitBytes", "scope"]
      .filter((key) => value[key] !== undefined).map((key) => [key, value[key]]));
