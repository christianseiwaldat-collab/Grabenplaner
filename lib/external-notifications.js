...
const STAFF_ASSIGNMENT_REQUEST_MESSAGES = Object.freeze({
  submitted: Object.freeze({
    subject: "Grabenplaner: Neue Einsatzanfrage",
    text: "Eine neue Anfrage für einen temporären Filialeinsatz wartet im Grabenplaner auf Prüfung. Bitte anmelden.",
  }),
  accepted: Object.freeze({
    subject: "Grabenplaner: Einsatzanfrage genehmigt",
    text: "Ihre Anfrage für einen temporären Filialeinsatz wurde genehmigt. Details finden Sie im Grabenplaner.",
  }),
  rejected: Object.freeze({
    subject: "Grabenplaner: Einsatzanfrage abgelehnt",
    text: "Ihre Anfrage für einen temporären Filialeinsatz wurde abgelehnt. Details finden Sie im Grabenplaner.",
  }),
});
const DESTINATION_VERIFICATION_SUBJECT = "Grabenplaner: Kontakt bestätigen";
const PASSWORD_RESET_SUBJECT = "Grabenplaner: Passwort neu festlegen";
...
function createExternalNotificationAdapter(options = {}) {
  ...
  async function sendStaffAssignmentRequestAlert(input = {}) {
    const recipient = validateRecipient("email", input.recipient);
    const kind = String(input.kind || "").trim().toLowerCase();
    const message = STAFF_ASSIGNMENT_REQUEST_MESSAGES[kind];
    if (!message) {
      throw notificationError(
        "Der Benachrichtigungstyp für die Einsatzanfrage ist ungültig.",
        "EXTERNAL_NOTIFICATION_EVENT_INVALID",
      );
    }
    await sendEmail(recipient, {
      event: "staff_assignment_request",
      subject: message.subject,
      text: message.text,
    });
    return { delivered: true, channel: "email", kind };
  }
  ...
  async function sendPasswordReset(input = {}) {
    ...
    let resetUrl;
    try {
      resetUrl = new URL(String(input.resetUrl || ""));
    } catch {
      throw notificationError(
        "Der Link zum Zurücksetzen des Passworts ist ungültig.",
        "EXTERNAL_NOTIFICATION_PASSWORD_RESET_URL_INVALID",
      );
    }
    if (resetUrl.protocol !== "https:"
      || resetUrl.username
      || resetUrl.password
      || resetUrl.pathname !== "/portal.html"
      || resetUrl.search
      || !/^#password-reset=[A-Za-z0-9_-]{43}$/.test(resetUrl.hash)) {
      throw notificationError(
        "Der Link zum Zurücksetzen des Passworts ist ungültig.",
        "EXTERNAL_NOTIFICATION_PASSWORD_RESET_URL_INVALID",
      );
    ...
    await sendEmail(recipient, {
      ...
      text: [
        "Für Ihren persönlichen Grabenplaner-Zugang wurde das Zurücksetzen des Passworts angefordert.",
        "",
        `Neues Passwort festlegen: ${resetUrl.toString()}`,
        "",
  ...
  async function sendBranchOrder(input = {}) {
    ...
    const deliveryMode = String(input.deliveryMode || "message").trim().toLowerCase();
    const subject = String(input.subject || "").trim();
    const text = String(input.text || "").trim();
    if (!new Set(["message", "message_pdf", "pdf_only"]).has(deliveryMode)) {
      throw notificationError(
        "Die Versandart der Filialbestellung ist ungültig.",
        "EXTERNAL_NOTIFICATION_DELIVERY_MODE_INVALID",
      );
    }
    ...
    if ((deliveryMode !== "pdf_only" && !text) || text.length > 12_000) {
      throw notificationError(
    ...
    if (deliveryMode !== "message") {
      const attachment = Buffer.isBuffer(input.attachment?.buffer)
        ? Buffer.from(input.attachment.buffer)
        : Buffer.alloc(0);
      if (!attachment.length || attachment.length > 8 * 1024 * 1024) {
        throw notificationError(
          "Die Bestell-PDF ist leer oder zu groß für den E-Mail-Versand.",
          "EXTERNAL_NOTIFICATION_ATTACHMENT_INVALID",
        );
      }
      if (!smtp.configured || !smtpTransport) {
        throw notificationError(
          "Für Bestell-PDFs ist ein betriebsbereiter SMTP-Versand erforderlich.",
          "EXTERNAL_NOTIFICATION_SMTP_REQUIRED",
        );
      }
      const filename = String(input.attachment?.filename || "Filialbestellung.pdf")
        .replace(/[\r\n"\\/:*?<>|]/g, "-")
        .slice(0, 160);
      attachments = [{ filename, content: attachment, contentType: "application/pdf" }];
    } else if (input.attachment) {
      throw notificationError(
        "Die Versandart der Filialbestellung erlaubt keinen Anhang.",
        "EXTERNAL_NOTIFICATION_DELIVERY_MODE_INVALID",
      );
    }
    await sendEmail(recipient, {
  ...
  return Object.freeze({
    ...
    sendPasswordReset,
    sendProcessAlert,
    sendStaffAssignmentRequestAlert,
    sendStaffingAlert,
