...
function normalizeSmtpConfiguration(input = {}) {
  ...
  const dispatchEnabled = input.dispatchEnabled !== false;
  const allowedEvents = input.allowedEvents === undefined
    ? [...SUPPORTED_EMAIL_EVENTS]
    : [...new Set(Array.isArray(input.allowedEvents) ? input.allowedEvents : [])]
      .map((entry) => String(entry || "").trim().toLowerCase())
      .filter((entry) => SUPPORTED_EMAIL_EVENTS.includes(entry));
  ...
  if (
    ...
    || !credentialsComplete
    || (input.authenticationRequired === true && (!user || !password))
  ) {
    return {
      enabled: true,
      configured: false,
...
function createExternalNotificationAdapter(options = {}) {
  ...
  function providerStatus() {
    ...
    const emailDispatchEnabled = smtp.configured ? smtp.dispatchEnabled : true;
    const enabledEvents = smtp.configured ? [...smtp.allowedEvents] : [...SUPPORTED_EMAIL_EVENTS];
    const emailAvailable = Boolean(
      (smtpTransportAvailable || emailWebhookAvailable)
    ...
    return {
      email: {
        ...
        provider: smtp.enabled ? smtp.providerId : webhooks.email.configured ? "https-webhook" : null,
        ...
        issueCode: !smtp.valid || !webhooks.email.valid
          ...
          : smtp.configured && smtpTransportUnavailable && !webhooks.email.configured
            ...
            : webhooks.email.configured && typeof fetchImplementation !== "function"
              ? "transport_unavailable"
              : smtp.configured && !smtp.dispatchEnabled
                ? "dispatch_disabled"
                : smtp.configured && !smtp.allowedEvents.length
                  ? "no_events_enabled"
                  : null,
      },
  ...
  function canSendEvent(channel, event) {
    ...
    if (normalizedChannel !== "email" || !smtp.configured) return true;
    return smtp.allowedEvents.includes(normalizedEvent);
  }

  async function sendEmail(recipient, content) {
    if (smtp.configured && smtpTransport) {
      if (!smtp.dispatchEnabled) {
        throw notificationError(
          "Der E-Mail-Versand ist administrativ deaktiviert.",
          "EXTERNAL_NOTIFICATION_DISPATCH_DISABLED",
        );
      }
      if (!smtp.allowedEvents.includes(String(content.event || "").trim().toLowerCase())) {
        throw notificationError(
          "Diese E-Mail-Benachrichtigung ist nicht freigeschaltet.",
