...
// Per-request processing budget, not a limit on the selected year or result set.
const MAX_DAYS = 366, ANALYSIS_LIMIT = 200;
...
function historyBusinessDate(source, table, data, H) {
  ...
  const value = ['Bondatum', 'Datum', 'Aenderung', 'WEDatum', 'Anlegedatum', 'Bestandsänderungsdatum', 'LDatum','InventurDatum','Löschdatum'].map(key => values[key]).find(v => v !== undefined && v !== null);
