...
function createSystemCenterMetricsStore(db, {
  retentionDays = DEFAULT_RETENTION_DAYS,
  sampleIntervalHours = DEFAULT_SAMPLE_INTERVAL_HOURS,
} = {}) {
  if (!db || typeof db.prepare !== "function" || typeof db.exec !== "function") {
    throw new TypeError("Eine SQLite-Datenbank wird benoetigt.");
  }
  ...
  function prune(nowMs) {
    ...
    db.prepare("DELETE FROM system_center_trust_metrics WHERE recorded_at < ?").run(cutoff);
    const overflow = Number(db.prepare("SELECT MAX(0, COUNT(*) - ?) AS count FROM system_center_trust_metrics")
      .get(maximumSamples)?.count || 0);
    if (overflow > 0) {
      const rows = db.prepare("SELECT interval_key FROM system_center_trust_metrics ORDER BY recorded_at, interval_key LIMIT ?")
        .all(overflow);
      const remove = db.prepare("DELETE FROM system_center_trust_metrics WHERE interval_key = ?");
      for (const row of rows) remove.run(row.interval_key);
    }
  }

  function record(input) {
    if (!read().integrityVerified) return false;
    const sample = normalizeSample(input);
    if (db.prepare("SELECT 1 FROM system_center_trust_metrics WHERE interval_key = ?").get(sample.intervalKey)) {
      return false;
    }
    const previous = db.prepare("SELECT sample_hash FROM system_center_trust_metrics ORDER BY recorded_at DESC, interval_key DESC LIMIT 1").get();
    sample.previousHash = previous?.sample_hash || null;
    const hash = sampleHash(sample);
    db.prepare(`
      INSERT OR IGNORE INTO system_center_trust_metrics
        (interval_key, recorded_at, trust_score, coverage, trust_state, database_bytes,
         storage_free_bytes, automation_state, previous_hash, sample_hash)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(sample.intervalKey, sample.recordedAt, sample.trustScore, sample.coverage, sample.trustState,
      sample.databaseBytes, sample.storageFreeBytes, sample.automationState, sample.previousHash, hash);
    prune(normalizedNow(input.now));
    return true;
  }

  function read() {
    const rows = db.prepare(`
      SELECT interval_key, recorded_at, trust_score, coverage, trust_state, database_bytes,
             storage_free_bytes, automation_state, previous_hash, sample_hash
      FROM system_center_trust_metrics ORDER BY recorded_at, interval_key
      LIMIT ?
    `).all(maximumSamples + 1);
    if (rows.length > maximumSamples) return { integrityVerified: false, samples: [] };
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      const sample = {
        ...
        trustScore: finiteInteger(row.trust_score, { maximum: 100 }),
        coverage: finiteInteger(row.coverage, { maximum: 100 }),
        trustState: String(row.trust_state || ""),
        databaseBytes: row.database_bytes === null ? null : finiteInteger(row.database_bytes),
        storageFreeBytes: row.storage_free_bytes === null ? null : finiteInteger(row.storage_free_bytes),
      ...
      const validShape = sample.recordedAt === sample.intervalKey && sample.trustScore !== null
        ...
        && /^[a-f0-9]{64}$/.test(String(row.sample_hash || ""));
      const validLink = index === 0 || sample.previousHash === previousHash;
      if (!validShape || !validLink || sampleHash(sample) !== row.sample_hash) {
        return { integrityVerified: false, samples: [] };
      }
      previousHash = row.sample_hash;
      samples.push(sample);
    }
    return { integrityVerified: true, samples };
  }
