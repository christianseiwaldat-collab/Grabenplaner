  ...
  return async function detail(articleNumber, context) {
    ...
    try {
      return await access.transaction(async tx => {
        ...
        let source = {}, stocks = [], stockAt = null;
        if (protection && article.sourceSystem === 'tradefoto.artikel_stamm') {
          ...
          source = await reader.byKey(tx, 'ARTIKEL_STAMM', [article.sourceArticleKey], fields) || {};
        ...
        const purchaseNet = article.sourceSystem === 'tradefoto.artikel_stamm'
          ? decimal(source.DurchschnittEK) : usablePrice(article, 'average_purchase', 'net');
        const vatPercent = vatRates.get(String(source.MWST)) ?? null;
        return { articleNumber: article.articleNumber, description: article.description,
          ...
          calculation: { purchaseNet, vatPercent, basis: 'Durchschnitts-EK netto', available: purchaseNet !== null && Number(purchaseNet) >= 0 && vatPercent !== null },
