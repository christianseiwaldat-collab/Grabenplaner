...
class IntegrationCache {
  constructor(options = {}) {
    ...
    this.maxEntryBytes = Math.max(1024, Number(options.maxEntryBytes || 16 * 1024 * 1024));
    this.maxBytes = Math.max(this.maxEntryBytes, Number(options.maxBytes || 64 * 1024 * 1024));
  ...
  create(actor, kind, value) {
    ...
    let byteSize;
    try {
      byteSize = Buffer.byteLength(JSON.stringify(value), "utf8");
    } catch {
      throw new IntegrationCacheError("Die Importvorschau konnte nicht sicher zwischengespeichert werden.", "INTEGRATION_SESSION_VALUE_INVALID", 422);
    }
    const currentBytes = [...this.entries.values()].reduce((sum, entry) => sum + Number(entry.byteSize || 0), 0);
    if (byteSize > this.maxEntryBytes || currentBytes + byteSize > this.maxBytes) {
      throw new IntegrationCacheError("Die Importvorschau ist für den geschützten Zwischenspeicher zu groß.", "INTEGRATION_SESSION_SIZE_LIMIT", 413);
    }
    const id = crypto.randomUUID();
    const expiresAt = Date.now() + this.ttlMs;
    const entry = { id, actor: normalizedActor, kind: String(kind), value, byteSize, expiresAt, consumed: false, timer: null };
