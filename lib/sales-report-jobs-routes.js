...
function registerSalesReportJobRoutes(app, { jobs, templates = null, requireSession, refreshSession, assertCsrf }) {
  const route = (work, download = false) => async (request, response) => {
    ...
    try {
      ...
      const session = await fresh(), signature = C.canonical(reportAuthority(session));
      const result = await work(session, request);
      if (C.canonical(reportAuthority(await fresh())) !== signature) C.fail('IMPORT_FORBIDDEN', 403);
      if (download) {
        const pdf = Buffer.isBuffer(result);
        response.set({ 'Content-Type': pdf ? 'application/pdf' : 'text/html; charset=utf-8', 'Content-Disposition': `attachment; filename="Verkaufsanalyse.${pdf ? 'pdf' : 'html'}"`, 'Content-Security-Policy': `default-src 'none'; style-src 'unsafe-inline'; sandbox${pdf ? ' allow-downloads' : ''}` });
  ...
  app.get('/api/sales-report-jobs/context', route(session => jobs.context(session)));
  ...
  if (templates) {
    app.get('/api/sales-report-templates', route(session => templates.list(session)));
    app.get('/api/sales-report-templates/:id', route((session, request) => templates.get(session, request.params.id)));
    app.post('/api/sales-report-templates', route((session, request) => templates.save(session, request.body)));
    app.put('/api/sales-report-templates/:id', route((session, request) => templates.save(session, request.body, request.params.id)));
    app.delete('/api/sales-report-templates/:id', route((session, request) => templates.remove(session, request.params.id, request.body)));
  }
}
