  ...
  const route = (work, download = false) => async (request, response) => {
    ...
    try {
      ...
      if (download) {
        ...
        response.set({ 'Content-Type': pdf ? 'application/pdf' : 'text/html; charset=utf-8', 'Content-Disposition': `attachment; filename="Verkaufsanalyse.${pdf ? 'pdf' : 'html'}"`, 'Content-Security-Policy': "default-src 'none'; style-src 'unsafe-inline'; sandbox" });
