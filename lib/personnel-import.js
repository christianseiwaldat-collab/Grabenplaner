...
const PERSONNEL_IMPORT_FIELDS = Object.freeze([
  ...
  { id: "homeLocationId", label: "Standort-ID", group: "Zuordnung", legacy: true },
  { id: "homeLocationName", label: "Standort (Name)", group: "Zuordnung", legacy: true },
...
function publicPersonnelImportFields() {
  return PERSONNEL_IMPORT_FIELDS
    .filter((field) => field.legacy !== true)
    .map(({ legacy: _legacy, ...field }) => ({ ...field }));
}
