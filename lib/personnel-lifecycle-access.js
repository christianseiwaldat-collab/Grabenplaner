...
const APPLICATION_LIST_PROJECTION_KEYS = Object.freeze([
  "id",
  "candidateId",
  "status",
  "desiredLocationId",
...
const APPLICATION_DETAIL_PROJECTION_KEYS = Object.freeze([
  "id",
  "candidateId",
  "status",
  "desiredPositionId",
...
function structuredApplicationProjection(application, snapshot, { detail = true } = {}) {
  const projected = assignKnown(
    {},
    application || {},
    detail ? APPLICATION_DETAIL_PROJECTION_KEYS : APPLICATION_LIST_PROJECTION_KEYS,
  );
  if (detail) {
    projected.canWrite = snapshot?.canWriteApplications === true
      && snapshot.canWriteApplication(application);
  }
  return projected;
}
...
function minimalProfileProjection(profile, { includeContact = false } = {}) {
  ...
  if (includeContact) {
    assignKnown(projected, profile || {}, [
      "birthDate",
      "citizenships",
      "email",
      "phone",
    ]);
    const address = profile && typeof profile.address === "object" && !Array.isArray(profile.address)
      ? profile.address
      : null;
    if (address && (address.postalCode || address.city)) {
      projected.residence = assignKnown({}, address, ["postalCode", "city"]);
    }
  }
  return projected;
}

function candidatePhotoProjection(photo) {
  if (!photo || photo.visibility !== "scoped_leadership"
    || photo.status !== "active" || Number(photo.currentVersion) < 1) return null;
  const current = Array.isArray(photo.versions)
    ? photo.versions.find((version) => Number(version.versionNumber) === Number(photo.currentVersion))
    : null;
  if (!current || current.mediaType !== "image/jpeg"
    || Number(current.sizeBytes) < 1 || Number(current.sizeBytes) > 512 * 1024) return null;
  return {
    id: photo.id,
    currentVersion: Number(photo.currentVersion),
    mediaType: current?.mediaType || "image/jpeg",
...
function projectedCandidate(candidate, snapshot, { detail = false } = {}) {
  ...
  if (detail && snapshot.global && snapshot.capabilities.canReadConfidential) {
    const projected = cloneData(candidate);
    if (Object.prototype.hasOwnProperty.call(candidate, "photo")) {
      projected.photo = candidatePhotoProjection(candidate.photo);
    }
    return projected;
  }
  ...
  projection.applications = visibleApplications
    .map((application) => structuredApplicationProjection(application, snapshot, { detail }));
  if (Object.prototype.hasOwnProperty.call(candidate, "photo")) {
    projection.photo = candidatePhotoProjection(candidate.photo);
  }
  return projection;
}
...
function projectPersonnelLifecycleApplication(application, sessionOrSnapshot) {
  ...
  return structuredApplicationProjection(application, snapshot);
}
