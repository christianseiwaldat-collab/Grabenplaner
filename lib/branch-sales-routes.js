...
const { normalizeSalesArticleSearch, normalizeSalesArticleNumber, SalesArticleCatalogError } = require("./sales-article-catalog");
...
function registerBranchSalesRoutes(app, { catalog, articleDetail, images, receipts, requireSession, refreshSession, assertCsrf }) {
  const route = (permission, work, pdf = false) => async (request, response) => {
    ...
    try {
      ...
      await fresh();
      if (pdf === "image") response.set({ "Content-Type": "image/webp", "Content-Security-Policy": "default-src 'none'; sandbox",
        "Content-Disposition": 'inline; filename="artikelbild.webp"' }).send(result);
      else if (pdf) response.type("application/pdf").attachment("Beleginformation-keine-Rechnung.pdf").send(result);
      ...
      const messages = {
        BRANCH_RECEIPT_LOCATIONS: "Bitte mindestens eine verfügbare Filiale auswählen.",
        BRANCH_RECEIPT_SELLERS: "Bitte bis zu 20 vollständige Personalnummern durch Beistriche getrennt eingeben.",
  ...
  app.get(base + '/branch-articles/detail', route(ARTICLES, ({ request, context }) => {
    C.exact(request.query, ['articleNumber']);
    return articleDetail(normalizeSalesArticleNumber(request.query.articleNumber), context);
  }));
  app.get(base + '/branch-articles/image', route(ARTICLES, async ({ request }) => {
    C.exact(request.query, ['articleNumber', 'v']);
    const number = normalizeSalesArticleNumber(request.query.articleNumber);
    if (!await catalog.getByArticleNumber(number)) C.fail('BRANCH_ARTICLE_NOT_FOUND', 404);
    const value = await images.content(number);
    if (!value) C.fail('BRANCH_ARTICLE_IMAGE_NOT_FOUND', 404);
    return value.buffer;
  }, 'image'));
  ...
  app.post(base + "/branch-receipts/search", receiptWork(({ request, context, workspace }) => {
    C.exact(request.body, ["dateFrom", "dateTo", "query", "receipt", "cursor", "locations", "sellers", "customer"]);
    const { locations, sellers = '', ...filters } = request.body;
    let sellerIds;
    try { sellerIds = require('../public/branch-receipt-filters').sellers(sellers); } catch { C.fail('BRANCH_RECEIPT_SELLERS', 422); }
    return workspace.receipts.search({ ...filters, sourceId: "compact-cash", kind: "receipts",
      sourceLocationIds: locations, sellerIds, sort: "date", direction: "desc", limit: 20 });
  }));
