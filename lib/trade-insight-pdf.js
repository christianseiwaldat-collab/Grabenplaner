...
function createTradeInsightPdf(snapshot,{sort='',direction='asc'}={}){
 const columns=M.columns(snapshot.kind,snapshot.projection);
 ...
 return new Promise((resolve,reject)=>{
  ...
  const weights=columns.map(c=>['label','reason','state','classification'].includes(c.key)?1.9:['date','sourceLocation','positions','customerNumber'].includes(c.key)?.85:1.15),total=weights.reduce((a,b)=>a+b,0),widths=weights.map(w=>width*w/total);
  function tableRow(values,header=false,index=0){
   ...
   let x=left;values.forEach((value,i)=>{font(header,8).text(clean(value),x+5,y+5,{width:widths[i]-10,align:['number','money'].includes(columns[i].type)?'right':'left'});x+=widths[i];});
  ...
  try{
   ...
   const labels={query:'Suche',locationId:'Filiale',supplier:'Lieferant',dateFrom:'Von',dateTo:'Bis',days:'Beobachtung (Tage)',group:'Sortiment',wgr:'Warengruppe',customer:'Kundennummer',serial:'Seriennummer'};
   const filters=Object.entries(snapshot.query).filter(([,v])=>v).map(([k,v])=>`${labels[k]||k}: ${v}`).join('  ·  ');
   if(filters)text(filters);
   if(!snapshot.query.locationId)text('Filialauswahl: Alle freigegebenen Filial-IDs zusammen');
