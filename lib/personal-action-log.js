...
function salesArticleCatalogPayload(value) {
  exactKeys(value, ["productId", "restoreRevision"]);
  if (!Object.hasOwn(value, "restoreRevision")) invalidInput();
  const productId = identifier(value.productId, 36).toLowerCase();
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(productId)) {
    invalidInput();
  }
  return Object.freeze({
    productId,
    restoreRevision: value.restoreRevision === null
      ? null
      : nullablePositiveRevision(value.restoreRevision),
  });
}

function salesArticleCatalogImportPayload(value) {
  exactKeys(value, ["snapshotId"]);
  const snapshotId = boundedText(value.snapshotId, 64).toLowerCase();
  if (!/^[a-f0-9]{64}$/.test(snapshotId)) invalidInput();
...
function normalizedUndoPayload(compensatorKey, value) {
  ...
  if (compensatorKey === "sales.article-catalog.restore.v1") {
    return salesArticleCatalogPayload(value);
  }
  if (compensatorKey === "sales.article-catalog.import.restore.v1") {
    return salesArticleCatalogImportPayload(value);
  }
  invalidInput();
}
