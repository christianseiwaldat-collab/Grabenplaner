...
const EXCLUDED_TABLES = Object.freeze({
  ...
  bestell:Object.freeze(B.metadata.tables.filter(t=>!t.included).map(t=>t.name)),
  weum:E.physical.weum.excludedTables,
  inventur:E.physical.inventur.excludedTables,
});
function definitions(kind) {
  if (!SOURCE_KINDS.includes(kind)) C.fail('IMPORT_SOURCE_KIND_INVALID');
  ...
  const history = H.TRADEFOTO_HISTORY_METADATA.tables.filter(t => (t.sourceFile || t.source) === kind)
    .map(t => ({ ...t, master: false, profile: H.profileFor(t.source, t.name) }))
...
async function readTradeFotoFullSource({ buffer, kind, password = '', send, readerFactory }) {
  ...
  if(['weum','inventur'].includes(kind))return require('./tradefoto-supplement-reader').readSupplement({reader,kind,fileSha256,bytes:buffer.length,send});
  ...
  for (const def of defs) {
    ...
    const checkpoint=await send({ type: 'table', name: def.name, expectedRows, declaredRows: table.rowCount });
    // The trusted consumer binds this offset to the complete file SHA-256,
    // parser/profile manifest and persisted run. No browser offset is accepted.
    const resumeAfter=checkpoint?.resumeAfter??0;
    C.integer(resumeAfter,0,expectedRows);
    ...
    for (let pageOffset=resumeAfter; pageOffset<expectedRows; pageOffset+=READ_PAGE_ROWS) {
      ...
      for (let offset=0; offset<rows.length; offset+=C.LIMITS.batch) {
        const batch = rows.slice(offset,offset+C.LIMITS.batch).map((row,i) => def.master
          ...
          : H.prepareTradeFotoHistoryRow(def.source, def.name, row, { fileSha256, rowNumber: pageOffset+offset+i+1 }));
