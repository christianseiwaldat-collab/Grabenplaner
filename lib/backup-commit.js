...
const { BACKUP_RETENTION_DAYS, planDailyBackups } = require('./backup-retention');
...
function pruneCommittedBackups(backupDirectory, keep = BACKUP_RETENTION_DAYS, options = {}) {
  const validBackups = listCommittedBackups(backupDirectory, options);
  const plan = planDailyBackups(validBackups.map(b => ({ id: b.snapshot, time: b.marker.committedAt })), { days: Number(keep), now: options.now || new Date() });
  const deletionCandidates = validBackups.filter(b => plan.removeIds.includes(b.snapshot));
  let removed = 0;
  for (const backup of deletionCandidates) {
    try {
      const root = fs.realpathSync(backupDirectory);
      for (const target of [backup.markerPath, backup.databasePath, backup.protectedDirectory]) {
        if (path.dirname(fs.realpathSync(target)) !== root) throw new Error('BACKUP_RETENTION_DELETE_SCOPE_INVALID');
      }
      verifyCommittedBackup(backupDirectory, path.basename(backup.markerPath), options);
