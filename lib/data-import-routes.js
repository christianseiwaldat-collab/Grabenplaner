...
const {SOURCE_KINDS}=require('./data-import-source-catalog');
function registerDataImportRoutes(app,{runtime,jobs=null,mappings=null,cashPublications=null,requireSession,refreshSession,assertCsrf,
  ...
  function failure(response,error) {
    ...
    const messages={IMPORT_NOT_ACTIVATED:'Die produktive Übernahme ist noch nicht freigegeben.',
      ...
      IMPORT_SOURCE_ROW_COUNT_MISMATCH:'Quellzähler und lesbare Zeilen stimmen nicht überein. Bitte eine konsistente Kopie bereitstellen.',
      IMPORT_INVENTORY_HEAD_INVALID:'Ein Inventurkopf ist mehrfach vorhanden oder ohne eindeutigen Schlüssel. Bitte die Quelle prüfen.',
      IMPORT_INVENTORY_HEAD_MISSING:'Eine Inventurposition hat keinen passenden Inventurkopf. Bitte die Quelle prüfen.',
    ...
    messages.IMPORT_DELETE_ALREADY_APPLIED='Diese Quelle wurde bereits ganz oder teilweise übernommen oder als Kassenstand freigegeben. Sie kann hier nicht gelöscht werden.';
    messages.IMPORT_DELETE_IN_PROGRESS='Diese Quelle wird endgültig gelöscht. Bitte die Löschung fortsetzen.';
  ...
  app.post('/api/data-import/sources/search',route(async(r,get)=>{
    ...
    if(jobs?.enqueueContentDate&&buildDataImportProjection(await get()).prepare) {
      for(const candidate of result.items.filter(s=>s.complete&&!s.active&&s.status!=='deleting'&&s.contentDate?.status!=='complete')) {
  ...
  app.get('/api/data-import/overview',route(async(_r,get)=>{
    const result=await runtime.overview(get);
    return jobs?{...result,items:await Promise.all(result.items.map(s=>jobs.overlay(s)))}:result;
  }));
  ...
  for(const action of ['delete-preview','delete'])app.post(`/api/data-import/sources/:id/${action}`,route((r,get)=>jobs
    ?jobs.deleteSource(get,r.params.id,r.body||{},action==='delete-preview')
    :runtime.sourceOperation(get,r.params.id,action,r.body||{})));
  ...
  app.post('/api/data-import/upload/:kind',async(request,response)=> {
    ...
    try {
      ...
      if(!SOURCE_KINDS.includes(request.params.kind)) C.fail('IMPORT_SOURCE_KIND_INVALID');
