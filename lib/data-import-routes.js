...
function registerDataImportRoutes(app,{runtime,jobs=null,mappings=null,cashPublications=null,requireSession,refreshSession,assertCsrf,
  ...
  app.post('/api/data-import/sources/search',route(async(r,get)=>{const result=await runtime.list(get,r.body||{});return jobs?{...result,items:await Promise.all(result.items.map(s=>jobs.overlay(s)))}:result;}));
  ...
  app.post('/api/data-import/upload/:kind',async(request,response)=> {
    ...
    try {
      ...
      if(jobs){
        const source=await jobs.enqueue(getSession,{buffer:request.body,kind:request.params.kind,password,signal});
        response.status(202).json({id:source.id,status:source.status,background:source.background});
      }else await runtime.upload(getSession,{buffer:request.body,kind:request.params.kind,password,
