...
function registerDataImportRoutes(app,{runtime,mappings=null,cashPublications=null,requireSession,refreshSession,assertCsrf}) {
  let uploading=false;
  ...
  const route=work=>async(request,response)=> {
    ...
    try {
      ...
      response.json(await work(request,getSession));
    }catch(error){failure(response,error);}
  };
  app.get('/api/data-import/context',route((_r,get)=>runtime.context(get)));
  ...
  app.post('/api/data-import/sources/search',route((r,get)=>runtime.list(get,r.body||{})));
  app.get('/api/data-import/sources/:id',route((r,get)=>runtime.sourceOperation(get,r.params.id,'read')));
  for(const action of ['review','apply','undo','events','rows','undo-preview']) {
    app.post(`/api/data-import/sources/:id/${action}`,route((r,get)=>runtime.sourceOperation(get,r.params.id,action,r.body||{})));
  }
  app.post('/api/data-import/upload/:kind',async(request,response)=> {
    headers(response);
    let admitted=false,password='';
    try {
      ...
      if(!['trade','cash'].includes(request.params.kind)) C.fail('IMPORT_SOURCE_KIND_INVALID');
      ...
      await new Promise((resolve,reject)=>express.raw({type:()=>true,limit:MAX_BYTES,inflate:false})(request,response,error=>error?reject(new C.DataImportError('IMPORT_SOURCE_SIZE_INVALID',413)):resolve()));
      ...
      await runtime.upload(getSession,{buffer:request.body,kind:request.params.kind,password,
        onStarted:source=>response.status(202).json({id:source.id,status:source.status})});
    }catch(error){failure(response,error);}
    finally{password='';if(Buffer.isBuffer(request.body))request.body.fill(0);if(admitted)uploading=false;}
  });
}
