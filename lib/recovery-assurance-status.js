...
const EVENT_TYPES = new Set([
  ...
  "application-smoke-not-run", "oauth-policy-passed",
]);
...
function recentRunSummaries(events, maximum = MAX_RETURNED_RUNS) {
  ...
  for (const event of events) {
    ...
    const state = runs.get(payload.runId) || {
      ...
      phases: Object.fromEntries(PHASE_EVENTS.map((id) => [id, { id, status: "pending", occurredAt: null }])),
    };
    if (payload.eventType === "full-assurance-started") state.startedAt = payload.occurredAt;
    else if (PHASE_EVENT_SET.has(payload.eventType)) {
      state.phases[payload.eventType] = {
        id: payload.eventType,
        status: payload.eventType === "application-smoke-not-run" ? "not_run" : "passed",
        occurredAt: payload.occurredAt,
      };
    } else {
  ...
  return [...runs.values()]
    ...
    .map((run) => ({
      ...
      durationSeconds: run.completedAt
        ...
        : null,
      phases: PHASE_EVENTS.map((id) => run.phases[id]),
    }))
