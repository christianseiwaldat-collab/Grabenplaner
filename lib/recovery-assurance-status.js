...
function recentRunSummaries(events, maximum = MAX_RETURNED_RUNS, reportId = null) {
  const runs = new Map();
  for (const event of events) {
    const payload = event.payload;
  ...
  return [...runs.values()]
    .filter((run) => run.startedAt && (!reportId || run.reportId === reportId))
    .map((run) => ({
      ...run,
      ...
      phases: PHASE_ORDER.map((id) => {
        ...
        if (run.status !== "failed" || phase.status !== "pending") return phase;
        const failedPhase = { CONFIGURATION_VERIFY_FAILED: "oauth-policy", PREPARE_FAILED: "backup", UPLOAD_FAILED: "backup",
          FULL_CHECK_FAILED: "repository-check", REPOSITORY_CHECK_FAILED: "repository-check", RESTORE_TEST_FAILED: "restore-test" }[run.errorCode];
        return { ...phase, status: id === failedPhase ? "failed" : "not_run" };
      }),
    }))
...
function readRecoveryAssuranceStatus(options = {}) {
  ...
  try {
    ...
    const recentRuns = recentRunSummaries(events, MAX_RETURNED_RUNS, options.reportId || null);
