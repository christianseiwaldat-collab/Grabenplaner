...
function defineDataImportProfile(input) {
  ...
  freeze(profile); PROFILES.add(profile); return profile;
}
function assertProfile(profile) { if (!PROFILES.has(profile)) fail("IMPORT_PROFILE_UNTRUSTED"); return profile; }
function normalizeDataImportRow(profile, row) {
  assertProfile(profile); exact(row, [...profile.fields.map(field => field.source), ...profile.excludedFields]);
  const source = {}, data = {};
  ...
  if (Buffer.byteLength(canonical(result)) > LIMITS.rowBytes) fail("IMPORT_ROW_TOO_LARGE", 413);
  return result;
}
