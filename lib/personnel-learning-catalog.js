...
function normalizePersonnelLearningTemplateInput(value, {
  ...
  const assessment = require("./personnel-learning-assessment").normalizeAssessment(value.assessment);
  const estimatedMinutes = Number(value.estimatedMinutes);
  const learningTarget = require("./personnel-learning-targets").normalizeLearningTarget(value.learningTarget);
  const article = require("./personnel-learning-knowledge").normalizeKnowledgeArticle(value.article);
  if (article && ((moduleType ?? value.moduleType) !== "knowledge" || learningTarget || assessment)) {
    invalid("PERSONNEL_LEARNING_KNOWLEDGE_INVALID", "Wissenseinträge werden getrennt von Schulungen und Zielkompetenzen geführt.");
  }
  if (learningTarget && verificationMode === "self_confirmation") {
    invalid("PERSONNEL_LEARNING_TARGET_INVALID", "Eine Schulung mit Zielkompetenz benötigt eine persönliche fachliche Abschlussprüfung.");
  }
  ...
  const content = Object.freeze({
    ...
    ...(article ? { catalogEntity:"article", article } : {}),
