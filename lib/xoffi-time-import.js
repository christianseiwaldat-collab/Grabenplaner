...
const XOFFI_OCR_ENGINE_VERSION = "local-tesseract-v7-deu-xoffi-2";
...
function isoWeekMonday(year, weekNumber) {
  ...
  if (!Number.isInteger(calendarYear) || calendarYear < 2000 || calendarYear > 2200 || !Number.isInteger(week) || week < 1 || week > 53) {
    return "";
  }
  const januaryFourth = new Date(Date.UTC(calendarYear, 0, 4, 12));
  const januaryFourthDay = januaryFourth.getUTCDay() || 7;
  januaryFourth.setUTCDate(januaryFourth.getUTCDate() - januaryFourthDay + 1 + (week - 1) * 7);
  const result = januaryFourth.toISOString().slice(0, 10);
  const thursday = new Date(`${result}T12:00:00Z`);
  thursday.setUTCDate(thursday.getUTCDate() + 3);
  return thursday.getUTCFullYear() === calendarYear ? result : "";
}
...
function completeWeekFromDates(dates) {
  const uniqueDates = new Set((Array.isArray(dates) ? dates : []).filter((date) => {
    const match = String(date || "").match(/^(20\d{2})-(\d{2})-(\d{2})$/);
    return match && isoDate(match[1], match[2], match[3]) === date;
  }));
  const completeMondays = [...uniqueDates].filter((date) => (
    mondayFor(date) === date
    && Array.from({ length: 7 }, (_unused, index) => addDays(date, index)).every((day) => uniqueDates.has(day))
  ));
  if (completeMondays.length !== 1) return null;
  return { weekStart: completeMondays[0], weekEnd: addDays(completeMondays[0], 6) };
}

function weekFromText(text) {
  const normalized = cleanText(text);
  const dates = datesFromText(normalized);
  const datedWeek = completeWeekFromDates(dates);
  if (datedWeek) return datedWeek;
  const range = normalized.match(/(\d{1,2})[.\/]\s*(\d{1,2})[.\/]\s*(20\d{2})\s*[-]\s*(\d{1,2})[.\/]\s*(\d{1,2})[.\/]\s*(20\d{2})/);
  if (range) {
    const first = isoDate(range[3], range[2], range[1]);
    const last = isoDate(range[6], range[5], range[4]);
    if (first && last && addDays(first, 6) === last && mondayFor(first) === first) return { weekStart: first, weekEnd: last };
  }
  const calendarWeekSegment = normalized.match(/Kalenderwoche.{0,28}/i)?.[0] || "";
  const bracketedWeek = calendarWeekSegment.match(/[\[(]\s*(\d{1,2})\s*[\])]/)?.[1];
  const labeledWeek = bracketedWeek
    || calendarWeekSegment.match(/Kalenderwoche\s*[:.]?\s*(\d{1,2})(?!\s*\?)/i)?.[1];
  const headerYear = dates[0]?.slice(0, 4) || normalized.slice(0, 400).match(/\b(20\d{2})\b/)?.[1];
  const weekStart = isoWeekMonday(headerYear, labeledWeek);
  if (weekStart) return { weekStart, weekEnd: addDays(weekStart, 6) };
  const first = dates.find((date) => mondayFor(date) === date);
  return first ? { weekStart: first, weekEnd: addDays(first, 6) } : null;
}
...
function parseXoffiRecognition({ text = "", blocks = [], width, height, headerDates = [], employees = [] } = {}) {
  ...
  const headerText = lines
    .filter((line) => line.y <= pageHeight * 0.2)
    .map((line) => line.text)
    .join(" ");
  const week = completeWeekFromDates(headerDates)
    || completeWeekFromDates(datesFromText(headerText))
    || weekFromText(completeText);
  if (!week) throw new XoffiTimeImportError("XOFFI_WEEK_NOT_DETECTED");
  const summaryBoundary = pageWidth * 0.925;
...
async function recognizeHeaderDates(worker, prepared, blocks) {
  try {
    ...
    for (let index = 0; index < dayCenters.length; index += 1) {
      ...
      if (recognizedDates.length !== 1) return [];
      headerDates.push(recognizedDates[0]);
    }
    return completeWeekFromDates(headerDates) ? headerDates : [];
  } catch {
    return [];
  }
}
...
async function inspectXoffiImageBuffer(buffer, { fileName = "xoffi.png", employees = [] } = {}) {
