...
const XOFFI_OCR_ENGINE_VERSION = "local-tesseract-v7-deu-xoffi-1";
...
function weekFromText(text) {
  ...
  const weekCandidates = [...calendarWeekSegment.matchAll(/\d{1,2}/g)]
    .map((match) => Number(match[0]))
    .filter((value) => value >= 1 && value <= 53);
  const headerYear = normalized.slice(0, 400).match(/\b(20\d{2})\b/)?.[1];
  const weekStart = isoWeekMonday(headerYear, bracketedWeek || weekCandidates.at(-1));
  if (weekStart) return { weekStart, weekEnd: addDays(weekStart, 6) };
  const dates = [...normalized.matchAll(/(\d{1,2})[.\/]\s*(\d{1,2})[.\/]\s*(20\d{2})/g)]
...
function parseXoffiRecognition({ text = "", blocks = [], width, height, employees = [] } = {}) {
  ...
  const week = weekFromText(completeText);
  ...
  const employeeRows = nameLines.map((line, index) => {
    ...
    const days = Array.from({ length: 7 }, (_unused, dayIndex) => {
      ...
      const intervals = [...new Set(dayLines.flatMap((item) => intervalsFromText(item.text)))];
      const totalLine = dayLines.find((item) => /gesamt\s*[:.]?/i.test(item.text));
      const totalMatch = totalLine?.text.match(/gesamt\s*[:.]?\s*([0-9Oo]+(?:[.,][0-9]+)?)/i);
      const valuedMinutes = totalMatch ? minutesFromDecimal(totalMatch[1]) : null;
      const actualMinutes = intervalMinutes(intervals);
      const absence = dayLines.some((item) => /\burlaub\b/i.test(item.text)) ? "vacation"
        : dayLines.some((item) => /\bkrank\b/i.test(item.text)) ? "sick" : "";
      return {
        workDate: addDays(week.weekStart, dayIndex),
        intervals,
        actualMinutes,
        valuedMinutes: valuedMinutes ?? actualMinutes,
        surchargeMinutes: Math.max(0, (valuedMinutes ?? actualMinutes) - actualMinutes),
        absence,
