...
const PRODUCT_READINESS_STANDARD = Object.freeze({
  ...
  security: {
    ...
    url: "https://nest.owasp.org/projects/application-security-verification-standard",
  },
});
...
function buildProductReadiness({
  ...
  const technicalGates = [
    {
      id: "security",
      label: "Technische Sicherheitsprüfungen",
      state: securityState,
      repairFactorId: ["server", "database", "tls"].find((id) => technicalCardState(trustIndex, id) !== "pass") || "server",
      standard: PRODUCT_READINESS_STANDARD.security,
      detail: "Bewertet werden die unten aufgeführten Server-, Datenbank- und HTTPS-Nachweise. Dies ist kein vollständiges OWASP-Sicherheitsaudit.",
    },
    {
      id: "recovery",
      label: "Recovery-Drill",
      state: recoveryState,
      repairFactorId: "recovery",
