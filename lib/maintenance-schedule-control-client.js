...
function parseTask(value) {
  if (!exactKeys(value, TASK_KEYS) || !/^[a-z][a-z0-9-]{2,40}$/.test(String(value.id || ""))
    ...
    || !["interval", "weekly", "monthly", "quarterly", "unsupported"].includes(value.cadence)
    || ![null, "not-installed", "different-target", "additional-triggers", "calendar-not-supported"].includes(value.unsupportedReason)
    ...
    || (value.intervalMinutes !== null && ![5, 10, 15, 20, 30, 60].includes(value.intervalMinutes))
  ...
  if ((!intervalValid && !weeklyValid && !calendarValid && !unsupportedValid)
