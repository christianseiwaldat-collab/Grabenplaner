...
const LEGACY_STATUS_SCHEMA_VERSION = 1;
...
// Daily 03:00 schedule plus time for the serialized backup/recovery checks.
const DEFAULT_MAXIMUM_AGE_HOURS = 30;
...
const CHECK_IDS = Object.freeze([
  ...LEGACY_CHECK_IDS.slice(0, 8),
  "frameOptions",
  ...LEGACY_CHECK_IDS.slice(8, 9),
  "permissionsPolicy",
  "crossOriginOpenerPolicy",
  "crossOriginResourcePolicy",
  "permittedCrossDomainPolicies",
  ...LEGACY_CHECK_IDS.slice(9),
]);
...
function parseServerMonitorStatus(value) {
  ...
  if (value.format !== STATUS_FORMAT
    || ![LEGACY_STATUS_SCHEMA_VERSION, STATUS_SCHEMA_VERSION].includes(value.schemaVersion)) {
  ...
  const checkIds = value.schemaVersion === LEGACY_STATUS_SCHEMA_VERSION ? LEGACY_CHECK_IDS : CHECK_IDS;
  assertExactKeys(value.checks, new Set(checkIds), "Die Monitorpruefungen");
  const checks = {};
...
function diagnosticsFromStatus(status, options = {}) {
  ...
  const maximumAgeHours = Number.isFinite(Number(options.maximumAgeHours))
    ? Math.min(48, Math.max(0.1, Number(options.maximumAgeHours)))
  ...
  const checkIds = status.schemaVersion === LEGACY_STATUS_SCHEMA_VERSION ? LEGACY_CHECK_IDS : CHECK_IDS;
  const failedChecks = checkIds.filter((id) => !status.checks[id]);
