...
const LEGACY_TOP_LEVEL_KEYS = new Set(
  [...TOP_LEVEL_KEYS].filter((key) => key !== "providerId"),
);
...
function parseOffsiteBackupStatus(value) {
  const schemaKeys = value?.schemaVersion === STATUS_SCHEMA_VERSION
    ? TOP_LEVEL_KEYS
    : value?.schemaVersion === LEGACY_STATUS_SCHEMA_VERSION
      ? LEGACY_TOP_LEVEL_KEYS
      : null;
  if (!schemaKeys) {
    throw statusError("STATUS_SCHEMA_INVALID", "Die Offsite-Statusversion ist ungueltig.");
  }
  assertExactKeys(value, schemaKeys, "Der Offsite-Backup-Status");
  for (const key of schemaKeys) {
    if (!Object.hasOwn(value, key)) throw statusError("STATUS_SCHEMA_INVALID", "Der Offsite-Backup-Status ist unvollstaendig.");
  }
  if (value.format !== STATUS_FORMAT || typeof value.configured !== "boolean") {
  ...
  if (value.schemaVersion === STATUS_SCHEMA_VERSION
    && ((value.configured && !PROVIDER_IDS.has(value.providerId))
      || (!value.configured && value.providerId !== null))) {
    throw statusError("STATUS_SCHEMA_INVALID", "Die gebundene Offsite-Providerkennung ist ungueltig.");
  }

  const normalized = {
    format: STATUS_FORMAT,
    schemaVersion: value.schemaVersion,
    providerId: value.schemaVersion === STATUS_SCHEMA_VERSION ? value.providerId : null,
