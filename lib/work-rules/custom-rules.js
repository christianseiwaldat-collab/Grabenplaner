...
const { hydrateWorkRuleProfileVersionRow } = require("./store");
const {
  assertCustomWorkRulesRepository,
} = require("../persistence/repositories/custom-work-rules");
...
async function normalizeScope(repository, scopeTypeValue, scopeKeyValue, ruleType) {
  ...
  if (scopeType === "business_unit") {
    const row = await repository.getActiveBusinessUnit(scopeKey);
  ...
  if (scopeType === "location") {
    const row = await repository.getActiveLocation(scopeKey);
  ...
  if (scopeType === "department") {
    const row = await repository.getActiveDepartment(scopeKey);
    if (!row) throw new TypeError("Die ausgewählte aktive Abteilung wurde nicht gefunden.");
    return {
      type: scopeType,
      key: String(row.id),
      label: `${row.locationId} · ${row.locationName} · ${row.name}`,
    };
  }
...
async function normalizeDefinition(repository, value = {}) {
  ...
  const scope = await normalizeScope(
    assertCustomWorkRulesRepository(repository),
    value.scopeType,
    value.scopeKey,
    ruleType,
  );
...
function withRepositoryTransaction(repository, work) {
  const persistence = assertCustomWorkRulesRepository(repository);
  return typeof persistence.transaction === "function"
    ? persistence.transaction(work)
    : work(persistence);
}

async function readStoredWorkRuleProfileVersion(repository, versionId) {
  const persistence = assertCustomWorkRulesRepository(repository);
  const normalizedVersionId = String(versionId || "");
  if (!normalizedVersionId) return null;
  const row = await persistence.getProfileVersion(normalizedVersionId);
  return row ? hydrateWorkRuleProfileVersionRow(row) : null;
}

async function insertDraftVersion(repository, profile, revisionNumber, value, actor) {
  const persistence = assertCustomWorkRulesRepository(repository);
  const definition = await normalizeDefinition(persistence, value);
  ...
  await persistence.insertProfileVersion({
    id: versionId,
    ...
    validFrom: definition.validFrom,
    validTo: definition.validTo,
    rules: prepared.storedRules,
    sources: prepared.sources,
    contentSha256: prepared.contentSha256,
    createdBy: String(actor || ""),
    publishedAt: null,
  });
  const profileStatus = (await persistence.getProfileStatus(profile.id))?.status;
  if (profileStatus === "active") {
    await persistence.touchActiveProfile(profile.id, String(actor || ""));
  } else {
    await persistence.updateDraftProfile({
      id: profile.id,
      name: definition.title,
      description: definition.description,
      sector: definition.topic,
      currentVersionId: versionId,
      updatedBy: String(actor || ""),
    });
  }
  const verified = await readStoredWorkRuleProfileVersion(persistence, versionId);
...
async function createCustomWorkRuleDraft(repository, value, actor = "") {
  ...
  return withRepositoryTransaction(repository, async (transaction) => {
    if (await transaction.profileExists(profileId)) {
      throw new TypeError("Dieses Regelkürzel wird bereits verwendet.");
    }
    await transaction.insertProfile({
      id: profileId,
      name: text(value.title, "Regelbezeichnung", 180, { required: true }),
      description: text(
        value.description,
    ...
    await insertDraftVersion(transaction, { id: profileId, code }, 1, value, actor);
    return getCustomWorkRuleDraft(transaction, profileId);
  });
}

async function addCustomWorkRuleDraftRevision(repository, profileIdValue, value, actor = "") {
  const profileId = text(profileIdValue, "Regel-ID", 180, { required: true });
  return withRepositoryTransaction(repository, async (transaction) => {
    const profile = await transaction.getProfileForRevision(profileId);
    ...
    const revisionNumber = await transaction.countDraftVersions(profile.id) + 1;
    ...
    return getCustomWorkRuleDraft(transaction, profile.id);
  });
}

async function serializeDraftVersion(repository, row) {
  const version = await readStoredWorkRuleProfileVersion(repository, row.id);
...
async function getCustomWorkRuleDraft(repository, profileIdValue) {
  const persistence = assertCustomWorkRulesRepository(repository);
  const profileId = String(profileIdValue || "");
  if (!profileId) return null;
  const profile = await persistence.getCustomProfile(profileId);
  if (!profile) return null;
  const versionRows = await persistence.listProfileVersionMetadata(profile.id);
  const versions = (await Promise.all(
    versionRows.map((row) => serializeDraftVersion(persistence, row)),
  )).sort((left, right) => (
      String(right.createdAt || "").localeCompare(String(left.createdAt || ""))
      || right.revisionNumber - left.revisionNumber
      || right.id.localeCompare(left.id)
    ));
  const publicationByDraftVersionId = await persistence.publicationRegistryAvailable()
    ? new Map((await persistence.listPublications(profile.id))
      .map((publication) => [publication.sourceProfileVersionId, publication]))
    : new Map();
  for (const version of versions) {
    const publication = publicationByDraftVersionId.get(version.id);
    version.releasedAsVersionId = publication?.releasedProfileVersionId || null;
    version.publicationId = publication?.id || null;
    version.publishedAt = publication?.publishedAt || null;
  }
  ...
  const publishedVersion = versions.find((version) => (
    version.id === profile.currentVersionId && version.status === "published"
  ...
  return {
    ...
    effectiveVersionId: profile.currentVersionId,
    publishedVersion,
...
async function customWorkRuleDefinitionFromVersion(repository, versionIdValue) {
  const persistence = assertCustomWorkRulesRepository(repository);
  const versionId = String(versionIdValue || "");
  const row = versionId ? await persistence.getVersionMetadata(versionId) : null;
  if (!row) throw new TypeError("Die ausgewählte eigene Regelfassung wurde nicht gefunden.");
  const version = await serializeDraftVersion(persistence, row);
...
async function publishCustomWorkRuleDraft(repository, versionIdValue, actor = "") {
  return withRepositoryTransaction(repository, async (transaction) => {
    const source = await customWorkRuleDefinitionFromVersion(transaction, versionIdValue);
    ...
    const releaseNumber = await transaction.countReleaseVersions(profileId) + 1;
    ...
    await transaction.insertProfileVersion({
      ...
      layer: prepared.layer,
      status: "published",
      validFrom: source.definition.validFrom,
      validTo: source.definition.validTo,
      rules: prepared.storedRules,
      sources: prepared.sources,
      contentSha256: prepared.contentSha256,
      createdBy: String(actor || ""),
    ...
    await transaction.activatePublishedProfile({
      id: profileId,
      name: source.definition.title,
      description: source.definition.description,
      sector: source.definition.topic,
      currentVersionId: releaseVersionId,
      updatedBy: String(actor || ""),
    });
    const verified = await readStoredWorkRuleProfileVersion(transaction, releaseVersionId);
    if (!verified || verified.contentSha256 !== prepared.contentSha256) {
...
async function restoreCustomWorkRuleVersionAsDraft(repository, versionIdValue, actor = "") {
  return withRepositoryTransaction(repository, async (transaction) => {
    const source = await customWorkRuleDefinitionFromVersion(transaction, versionIdValue);
    ...
    const profileId = source.id.split("@")[0];
    const revisionNumber = await transaction.countDraftVersions(profileId) + 1;
...
async function listCustomWorkRuleDrafts(repository) {
  const persistence = assertCustomWorkRulesRepository(repository);
  const rows = await persistence.listCustomProfileIds();
  return Promise.all(rows.map((row) => getCustomWorkRuleDraft(persistence, row.id)));
}

async function simulateCustomWorkRuleDraft(repository, value) {
  const definition = await normalizeDefinition(repository, value);
  return {
    metric: definition.metric,
    operator: definition.operator,
    threshold: definition.threshold,
