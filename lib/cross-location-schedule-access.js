...
function staffAssignmentRequestPeriod(today, sameDay = null) {
  const minimum = validIsoDate(today);
  if (!minimum) throw new TypeError("Das heutige Datum ist ungültig.");
  const date = new Date(minimum + "T12:00:00Z");
  const day = date.getUTCDate();
  date.setUTCDate(1);
  date.setUTCMonth(date.getUTCMonth() + 6);
  const lastDay = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth() + 1, 0)).getUTCDate();
  date.setUTCDate(Math.min(day, lastDay));
  const period = { minimum, maximum: date.toISOString().slice(0, 10) };
  if (!sameDay) return period;
  const localTime = String(sameDay.localTime || "");
  if (!/^(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d)?$/.test(localTime)) throw new TypeError("Die aktuelle Uhrzeit ist ungültig.");
  const closingTime = sameDay.open === true && TIME_PATTERN.test(String(sameDay.closingTime || ""))
    ? sameDay.closingTime : null;
  const closingMinutes = closingTime ? Number(closingTime.slice(0, 2)) * 60 + Number(closingTime.slice(3, 5)) : null;
  const deadlineMinutes = closingMinutes === null ? -1 : closingMinutes - 180;
  const cutoffTime = deadlineMinutes >= 0
    ? String(Math.floor(deadlineMinutes / 60)).padStart(2, "0") + ":" + String(deadlineMinutes % 60).padStart(2, "0") : null;
  const nowSeconds = Number(localTime.slice(0, 2)) * 3600 + Number(localTime.slice(3, 5)) * 60 + Number(localTime.slice(6, 8) || 0);
  const allowed = cutoffTime !== null && nowSeconds <= deadlineMinutes * 60;
  return { ...period, minimum: allowed ? minimum : addIsoDays(minimum, 1), today: minimum,
    sameDay: { allowed, closingTime, cutoffTime, reason: allowed ? null : closingTime ? "cutoff" : "closed" } };
}
