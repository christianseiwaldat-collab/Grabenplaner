...
const RETRYABLE=new Set(['IMPORT_SOURCE_READ_TIMEOUT','IMPORT_SOURCE_READ_INTERRUPTED','IMPORT_SOURCE_READ_FAILED',
  ...
  'PERSISTENCE_TIMEOUT','PERSISTENCE_UNAVAILABLE','PERSISTENCE_UNKNOWN']);
...
// session token, stored permission grant or automatic business apply is used.
function createDataImportJobs({directory,vault,runtime,resolvePrincipal,lifecycle,now=Date.now,onError=()=>{},store=createDataImportJobStore({directory,vault,now}),retryDelays=RETRY_DELAYS}) {
  ...
  const owner=session=>{if(!buildDataImportProjection(session).prepare)C.fail('IMPORT_FORBIDDEN',403);return C.id(String(session.employeeNumber));};
  ...
  const summary=j=>j?{status:j.status,phase:j.phase,retries:j.retries,maxRetries:RETRY_DELAYS.length,
  ...
  async function execute(job){
    ...
    const getSession=async()=>{if(job.expires<=now())C.fail('IMPORT_JOB_EXPIRED',409);const session=await resolvePrincipal(job.owner);if(owner(session)!==job.owner)C.fail('IMPORT_FORBIDDEN',403);return session;};
    let buffer;
    try{
      ...
      while(!stopped&&!controller.signal.aborted&&job.status!=='paused'){
        ...
        if(source.complete&&['ready','needs_review','applied'].includes(source.status)){
          job.status='completed';job.password='';await store.save(job);await store.remove(job.id);jobs.delete(job.id);return;
        }
        if(!source.complete||source.status!=='reviewing')C.fail('IMPORT_STATE_CONFLICT',409);
        await runtime.sourceOperation(getSession,job.id,'review',{expectedRevision:source.revision});
  ...
  return {enqueue,overlay,action,tick,start,stop,beginUpload};
}
