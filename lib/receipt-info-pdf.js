  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      function line(text, { size = 9, bold = false, color = '#213d38', gap = 7 } = {}) {
        doc.font(bold ? 'Receipt-Bold' : 'Receipt-Regular').fontSize(size).fillColor(color);
      ...
      function table(rows, columns) {
        ...
        function header() {
          ...
          columns.forEach((c, i) => { doc.font('Receipt-Bold').fontSize(8).fillColor('#213d38').text(c.label, x + 5, y + 7, { width: colWidth[i] - 10 }); x += colWidth[i]; }); y += 30;
        }
        if (y + 70 > bottom) page(true); header();
        for (const row of rows) {
          doc.font('Receipt-Regular').fontSize(8.5);
          const texts = columns.map(c => clean(c.value(row)));
          ...
          if (y + height > bottom) { page(true); header(); }
          let x = left;
          texts.forEach((t, i) => { doc.font('Receipt-Regular').fontSize(8.5).fillColor('#243633').text(t, x + 5, y + 4, { width: colWidth[i] - 10 }); x += colWidth[i]; });
      ...
      for (const [index, item] of items.entries()) {
        ...
        if (Object.hasOwn(item, 'personnel')) line(`Personalnummer (Belegverkäufer): ${item.personnel || 'Zuordnung offen'}`);
        ...
        if (item.kind === 'receipts') {
          ...
          const columns = [{ label: 'Artikel / Bezeichnung', weight: 5, value: l => [l.article, l.description].filter(Boolean).join('\n') },
            { label: 'Menge', weight: 1, value: l => number(l.quantity) },
            ...(Object.hasOwn(item, 'personnel') ? [{ label: 'Personalnr.', weight: 1.4, value: l => l.personnel || '-' }] : []),
            { label: 'Quellpreis', weight: 1.5, value: l => money(l.sourcePrice) },
            { label: 'Brutto', weight: 1.5, value: l => l.gross == null ? 'Prüfung offen' : money(l.gross) },
            { label: 'Status', weight: 1.5, value: l => ({ sale: 'Verkauf', return: 'Rückgabe', excluded: 'Ausgeschlossen', review: 'Prüfung offen' }[l.status] || 'Prüfung offen') }];
          table(item.lines, columns);
          line(item.gross == null ? `Quellbetrag: ${money(item.sourceAmount)} · Belegabgleich offen` : `Geprüfter Betrag: ${money(item.gross)} ${item.currency}`, { size: 11, bold: true });
          if (item.gross == null) line('Der Betrag wird unverändert aus der Quelle gezeigt; daraus wird keine geprüfte Umsatzsumme gebildet.', { size: 8 });
        } else {
