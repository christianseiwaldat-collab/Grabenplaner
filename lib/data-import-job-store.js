...
function createDataImportJobStore({directory,vault,now=Date.now}) {
  ...
  async function read(id){
    ...
    if(job?.version!==1||job.id!==id||!SOURCE_KINDS.includes(job.kind)||typeof job.owner!=='string'||!job.owner
      ||!['queued','reading','reviewing','retrying','paused','failed','completed'].includes(job.status)
      ||!Number.isSafeInteger(job.created)||!Number.isSafeInteger(job.expires)
      ||job.expires-job.created<RETENTION_MS||job.expires-job.created>RETENTION_MS+1000
      ||!Number.isSafeInteger(job.retries)||job.retries<0||job.retries>3||!Number.isFinite(job.nextAt)
      ||job.operation!==undefined&&(!['apply','content-date'].includes(job.operation)||job.phase!=='reviewing'||job.blob!==null||job.password!==''||!Number.isSafeInteger(job.requestedRevision)||job.requestedRevision<1)
    ...
    // Older writers sampled the clock separately for creation and expiry.
    // Accept their bounded clock drift, then retain exactly the original 72 h.
    job.expires=job.created+RETENTION_MS;
    return job;
  }
