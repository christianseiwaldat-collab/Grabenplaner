...
function createPersonnelWorkflowAccessSnapshot(session = {}) {
  ...
  const canStartInstances = canRead && has(PERSONNEL_WORKFLOW_PERMISSIONS.PUBLISH);
  const canStartScope = (value) => {
    if (!canStartInstances) return false;
    if (global) return true;
    return scopeMatches(
      role,
      allowedScopesByPermission[PERSONNEL_WORKFLOW_PERMISSIONS.PUBLISH],
      value,
    );
  };
  snapshot.capabilities = Object.freeze({
    canRead,
    canReadInstances: canRead,
    canStartInstances,
    canWriteDrafts,
