...
const palette = ['#245448', '#4878a0', '#bf7543', '#8c638c', '#818a45', '#667d86'];
...
function createSalesAnalysisPdf({ title, query, metadata, report, completedAt }) {
  return new Promise((resolve, reject) => {
    ...
    const left = 40, width = 515, bottom = 765;
    ...
    function text(value, { size = 9, bold = false, color = '#203b34', gap = 7 } = {}) {
      ...
      if (h > 650) C.fail('IMPORT_REPORT_PDF_LIMIT', 413);
      if (y + h > bottom) page();
    ...
    function table(columns, rows, section) {
      ...
      function row(values, header) {
        ...
        const heights = values.map((v, i) => doc.heightOfString(clean(v), { width: widths[i] - 12 }));
        const height = Math.max(15, ...heights) + 12;
        if (height > 640) C.fail('IMPORT_REPORT_PDF_LIMIT', 413);
        ...
        let x = left;
        values.forEach((v, i) => { font(header ? 8 : 8.5, header).text(clean(v), x + 6, y + 5, { width: widths[i] - 12, align: columns[i].numeric ? 'right' : 'left' }); x += widths[i]; });
    ...
    function chart(metric) {
      const entries = report.rows.map(row => ({ label: row.dimensions.map(d => d.label).join(' / '), value: row.metrics[metric.id].current, previous: row.metrics[metric.id].previous }))
        .filter(r => r.value !== null).sort((a, b) => Number(b.value) - Number(a.value));
      if (!entries.length) return;
      page(metric.label + ' · Übersicht');
      const shares = entries.length <= 6 && entries.every(r => Number(r.value) >= 0) && entries.some(r => Number(r.value) > 0)
      ...
      if (shares) {
        ...
        text('Aktuell: Grün · Vergleich: Grau. Vollständige Werte stehen in den Tabellen.', { color: '#65746f' });
        ...
        for (const entry of visible) {
          font(8); const h = Math.max(37, doc.heightOfString(entry.label, { width: 202 }) + 8);
          if (y + h > bottom) page(metric.label + ' · Übersicht (Fortsetzung)');
          font(8).text(entry.label, left, y + 2, { width: 202 });
    ...
    try {
      ...
      text(`Filialen: ${query.locationIds.map(id => metadata.locations.find(l => l.id === id)?.label || id).join(', ')}`);
      text(`Aufschlüsselung: ${query.groupBy.map(id => metadata.dimensions.find(d => d.id === id)?.label || id).join(' > ')}`);
      for (const [label, ids, options] of [['WGR / Sortiment', query.productGroupIds, metadata.productGroups], ['Hersteller / Marke', query.manufacturerIds, metadata.manufacturers], ['MA', query.sellerIds, metadata.sellers]]) {
        text(`${label}: ${ids.length ? ids.map(id => options.find(o => o.id === id)?.label || id).join(', ') : 'Alle im gewählten Umfang'}`, { size: 8.5 });
      }
      text(`Quelle: ${metadata.source.label} · ${metadata.source.coverageLabel}`, { size: 8, color: '#65746f' });
      text(`Erstellt: ${completedAt.slice(0, 19).replace('T', ' ')} UTC · ${report.processed} Positionen geprüft`, { size: 8, color: '#65746f' });
      ...
      const cells = (m, result) => [display(result.current, m.unit), display(result.previous, m.unit),
      ...
      text('Historische Kassenpositionen; Hersteller und Sortiment entsprechen dem gespeicherten Verkaufsstand. Fehlende Tage sind keine bestätigten Nullumsätze. Bei einem leeren Quellzeitraum ist kein Vorjahresvergleich verfügbar.', { size: 8 });
      text('Retouren fließen mit ihrem Vorzeichen ein. Ungeprüfte Belege sperren betroffene Geldsummen. Prozentänderung: Differenz / Betrag des Vergleichswerts; bei Vergleichswert null nicht verfügbar.', { size: 8 });
      if (query.metrics.some(id => ['customerCount', 'revenuePerCustomer', 'marginPerCustomer'].includes(id))) text('Kundenanzahl zählt unterschiedliche bekannte Kundenkonten. Anonyme Belege bleiben getrennt. Werte je Kunde verwenden ausschließlich Umsätze bzw. Roherträge dieser bekannten Konten. Kunden- und Belegzahlen verschiedener Gruppen dürfen nicht addiert werden.', { size: 8 });
      if (query.metrics.some(id => ['grossMargin', 'marginPerCustomer', 'marginRate'].includes(id)) && metadata.marginStatus !== 'confirmed') text('Rohertrag: Die Bedeutung des historischen Kassenfelds ist noch nicht bestätigt. Deshalb wird kein Rohertrag aus heutigen Einkaufspreisen oder ungeklärten Quellwerten berechnet.', { size: 8 });
      if (query.metrics.some(id => ['grossMargin', 'marginPerCustomer', 'marginRate'].includes(id)) && metadata.marginStatus === 'confirmed') text('Rohertrag: historischer Kassen-Rohertrag je Stück × verkaufte Menge, je Position auf Cent gerundet. Fehlt ein Quellwert, bleiben die betroffenen Rohertragskennzahlen nicht verfügbar.', { size: 8 });
      if (query.metrics.includes('marginRate')) text('Absolute Änderungen der Rohertragsquote sind Prozentpunkte.', { size: 8 });
      for (const [key, label] of [['current', 'Aktuell'], ['comparison', 'Vergleich']]) {
        const c = report.coverage[key];
        text(`${label}: ${c.selectedRecords} passende Positionen, ${c.review} ungeprüft, ${c.excluded} ausgeschlossen${metadata.projection.customers ? `, ${c.anonymousReceipts} Belege ohne bekanntes Kundenkonto` : ''}.`, { size: 8 });
      }
      ...
      for (const metric of selected) {
        ...
        table([{ ...columns[0], label: 'Gruppe' }, ...columns.slice(1)], report.rows.map(row => [row.dimensions.map(d => d.label).join(' / '), ...cells(metric, row.metrics[metric.id])]), metric.label);
        text(`Gesamt: ${display(report.total.metrics[metric.id].current, metric.unit)}`, { bold: true });
        if (!report.rows.length) text('Keine passenden Positionen in den gewählten Quellzeiträumen.');
      }
      const range = doc.bufferedPageRange();
      for (let i = 0; i < range.count; i++) {
        doc.switchToPage(i); font(7, false, '#65746f').text(`Grabenplaner · Verkaufsanalyse · Seite ${i + 1} / ${range.count}`, left, 790, { width, lineBreak: false });
      }
      doc.end();
