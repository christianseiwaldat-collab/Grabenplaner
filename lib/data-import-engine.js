...
function createDataImportEngine({ repository, protection, profiles, writers = {}, sourceTolerances = [], sharedPayloads = false, getActor, authorize, clock = () => new Date().toISOString() }) {
  ...
  async function makePlan(tx, executor, run, profile, row) {
    const decoded = await decodeRow(tx, run, row), writer = writerRegistry.get(profile.entity);
  ...
  const engine = {
    ...
    async recheck(runId, expectedRevision) {
      return inRun(runId, "review", async (tx, _executor, run) => {
        ...
        await tx.resetReview({ runId });
        return summary(tx, await changeRun(tx, run, "reviewing", "import.recheck"));
      });
    },
    ...
    async apply(runId, expectedRevision) {
      return inRun(runId, "apply", async (tx, executor, run, profile) => {
        ...
        for (const row of await tx.pendingApply({ runId, limit: C.LIMITS.batch })) {
          const plan = await decodeRow(tx, run, row);
