...
const CRM_CUSTOMER_TYPES = Object.freeze(["private", "business"]);
const CRM_CUSTOMER_SORT_KEYS = Object.freeze([
  "customerNumber", "lastName", "firstName", "customerType", "companyName",
  "postalCode", "city", "country", "phone", "email", "website", "vatId", "birthDate",
]);
const CRM_CUSTOMER_COLUMN_IDS = Object.freeze([
  "photo", "customerNumber", "name", "customerType", "companyName", "address",
  "postalCode", "city", "country", "phone", "email", "website", "vatId", "birthDate",
]);
...
function normalizeCrmCustomerInput(value, { update = false } = {}) {
  ...
  for (const field of CUSTOMER_FIELDS) {
    if (!Object.hasOwn(value, field)) invalid(`Das Feld ${field} fehlt.`);
  }
  ...
  if (!result.companyName && !result.firstName && !result.lastName) {
    invalid("Bitte einen Namen oder Firmennamen angeben.", "CRM_NAME_REQUIRED");
  }
  if (update) {
