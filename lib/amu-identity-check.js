...
const { extractAumIdentity } = require("../public/amu-ocr-parser");
...
async function derivedIdentityFromDocuments(documents = []) {
  if (process.env.NODE_ENV === "test" && Object.prototype.hasOwnProperty.call(process.env, "GRABENPLANER_TEST_AMU_IDENTITY_TEXT")) {
    return identityFromTexts([process.env.GRABENPLANER_TEST_AMU_IDENTITY_TEXT]);
  }
  if (process.env.NODE_ENV === "test") return { status: "not_detected", candidate: "" };
  ...
  const result = identityFromTexts([...pdfTexts, ...imageResults]);
  pdfTexts.fill("");
  imageResults.fill("");
  return result;
}

async function evaluateAumIdentity(documents, { profileConfigured = false, compareCandidate } = {}) {
  const checkedAt = new Date().toISOString();
  if (!profileConfigured) return { status: "profile_missing", checkedAt, engineVersion: ENGINE_VERSION };
  const derived = await derivedIdentityFromDocuments(documents);
  if (derived.status !== "detected" || !derived.candidate) {
    return { status: "not_detected", checkedAt, engineVersion: ENGINE_VERSION };
  }
  const matched = typeof compareCandidate === "function" && compareCandidate(derived.candidate) === true;
  derived.candidate = "";
  return { status: matched ? "matched" : "mismatch", checkedAt, engineVersion: ENGINE_VERSION };
}
