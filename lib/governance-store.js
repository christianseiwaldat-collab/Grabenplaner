...
function createGovernanceStore(options = {}) {
  ...
  function payrollHandoffRow(id) {
    const row = db.prepare("SELECT * FROM payroll_handoffs WHERE id = ?").get(String(id || ""));
    if (!row) return null;
    ...
    const events = db.prepare(`
      SELECT * FROM payroll_handoff_events
      WHERE handoff_id = ?
      ORDER BY occurred_at, id
    `).all(row.id).map((eventRow) => {
  ...
  function persistPayrollHandoffEvent(handoff, input) {
    ...
    `).run(
      ...
      event.handoffId,
      event.eventType,
      protect(event, context("payroll-handoff-event", event.eventId, "system")),
      event.receiptSha256,
      event.actorId,
      event.occurredAt,
    );
  ...
  function createPayrollHandoffRecord({ month, scope, statements, actor }) {
    monthRange(month);
    const locationId = String(scope?.locationId || "");
    const departmentId = String(scope?.departmentId || "");
    const previousRow = db.prepare(`
      SELECT * FROM payroll_handoffs
      WHERE period_month = ? AND location_id = ? AND department_id = ?
      ORDER BY revision DESC LIMIT 1
    `).get(month, locationId, departmentId);
    const previous = previousRow ? payrollHandoffRow(previousRow.id) : null;
    ...
    try {
      ...
      `).run(
        handoff.handoffId,
        handoff.month,
        handoff.scope.locationId,
        handoff.scope.departmentId,
        handoff.revision,
        protect(handoff, context("payroll-handoff", handoff.handoffId, "system")),
        handoff.receiptSha256,
        previous?.row.id || null,
        handoff.createdBy,
        handoff.createdAt,
      );
      persistPayrollHandoffEvent(handoff, { eventType: "created", actorId: handoff.createdBy });
      if (previous && previous.state !== "superseded") {
        persistPayrollHandoffEvent(previous.handoff, {
          eventType: "superseded",
    ...
    return payrollHandoffRow(handoff.handoffId);
  }

  function appendPayrollHandoffEvent(id, input, actor) {
    const value = payrollHandoffRow(id);
    ...
    if (eventType === "external_transfer_marked" && !value.events.some((event) => event.eventType === "exported")) {
    ...
    if (eventType === "protocol_recorded" && !value.events.some((event) => event.eventType === "external_transfer_marked")) {
    ...
    if (eventType === "protocol_recorded" && value.events.some((event) => event.eventType === "protocol_recorded")) {
    ...
    if (eventType !== "protocol_recorded" && value.events.some((event) => event.eventType === eventType)) {
      return value;
    }
    persistPayrollHandoffEvent(value.handoff, {
      eventType,
      actorId: String(actor || "system"),
      protocolResult: input?.protocolResult || "",
      protocolNumber: input?.protocolNumber || "",
    ...
    return payrollHandoffRow(id);
  }

  function listPayrollHandoffs({ month = "", locationId = "", includeSuperseded = false } = {}) {
    const where = [];
    const values = [];
    if (month) {
      monthRange(month);
    ...
    if (locationId) {
      ...
      values.push(String(locationId));
    }
    const rows = db.prepare(`
      SELECT id FROM payroll_handoffs
      ${where.length ? `WHERE ${where.join(" AND ")}` : ""}
      ORDER BY period_month DESC, location_id, department_id, revision DESC
    `).all(...values);
    return rows.map((row) => payrollHandoffRow(row.id))
      .filter((value) => includeSuperseded || value.state !== "superseded")
      .map(payrollHandoffSummary);
  }

  function verifyPayrollHandoffIntegrity() {
    const rows = db.prepare(`
      SELECT id, supersedes_id FROM payroll_handoffs
      ORDER BY period_month, location_id, department_id, revision
    `).all();
    const values = new Map(rows.map((row) => {
      const value = payrollHandoffRow(row.id);
      return [String(row.id), { ...value, supersedesId: String(row.supersedes_id || "") }];
    }));
    let verified = 0;
    ...
    const eventCount = Number(db.prepare("SELECT COUNT(*) AS count FROM payroll_handoff_events").get().count || 0);
