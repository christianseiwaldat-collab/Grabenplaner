...
function detectDocumentType(buffer, originalName = "Dokument") {
  if (!Buffer.isBuffer(buffer) || buffer.length === 0) {
    throw storageError("Die Dokumentdatei ist leer oder ungültig.", "AMU_DOCUMENT_INVALID");
  }
  if (buffer.length > MAX_DOCUMENT_BYTES) {
    throw storageError("Die Dokumentdatei darf höchstens 10 MiB groß sein.", "AMU_DOCUMENT_TOO_LARGE");
  }
  ...
  throw storageError("Nur PDF-, JPG- und PNG-Dateien werden unterstützt.", "AMU_DOCUMENT_TYPE_UNSUPPORTED");
}
...
function encryptDocument(buffer, keyId, key) {
  const keyIdBuffer = Buffer.from(keyId, "utf8");
  const iv = crypto.randomBytes(IV_BYTES);
  const cipher = crypto.createCipheriv("aes-256-gcm", key, iv);
  const encrypted = Buffer.concat([cipher.update(buffer), cipher.final()]);
  const tag = cipher.getAuthTag();
  const header = Buffer.alloc(ENCRYPTED_FILE_MAGIC.length + 1 + IV_BYTES + TAG_BYTES + keyIdBuffer.length);
  let offset = 0;
  ENCRYPTED_FILE_MAGIC.copy(header, offset); offset += ENCRYPTED_FILE_MAGIC.length;
  ...
  return { encrypted: Buffer.concat([header, encrypted]), iv, tag };
}

function decryptDocument(encrypted, keys) {
  const minimum = ENCRYPTED_FILE_MAGIC.length + 1 + IV_BYTES + TAG_BYTES;
  if (!Buffer.isBuffer(encrypted) || encrypted.length <= minimum || !encrypted.subarray(0, ENCRYPTED_FILE_MAGIC.length).equals(ENCRYPTED_FILE_MAGIC)) {
    throw storageError("Die AMU-Datei hat ein ungültiges verschlüsseltes Format.", "AMU_ENCRYPTED_FILE_INVALID");
  }
  let offset = ENCRYPTED_FILE_MAGIC.length;
  ...
  try {
    ...
    return { buffer: Buffer.concat([decipher.update(encrypted.subarray(offset)), decipher.final()]), keyId, iv, tag };
  } catch (error) {
    throw storageError("Die AMU-Datei ist beschädigt oder wurde verändert.", "AMU_DOCUMENT_INTEGRITY_FAILED", error);
  }
}

function createAmuStorage({ rootDirectory, encryptionKeys, activeKeyId, scanner = null, requireScanner = false } = {}) {
  ...
  if (fs.existsSync(keyCheckPath)) {
    ...
    for (const file of listEncryptedFiles(root).files) decryptDocument(fs.readFileSync(file.path), keys);
  ...
  async function saveBuffer({ buffer, originalName } = {}) {
    const type = detectDocumentType(buffer, originalName);
    ...
    try {
      ...
      let scan;
      try {
        scan = await runConfiguredScanner(scanner, plainTemp, Boolean(requireScanner));
        scannerHealth = { checked: true, available: scan.available !== false, clean: scan.clean, engine: scan.engine || null, error: "" };
      } catch (error) {
  ...
  function readBuffer(metadata = {}) {
    ...
    const decrypted = decryptDocument(encrypted, keys);
...
function syncEncryptedFilesBackup({ sourceDirectory, targetDirectory, manifestMetadata = {} } = {}) {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (!content.subarray(0, ENCRYPTED_FILE_MAGIC.length).equals(ENCRYPTED_FILE_MAGIC)) {
...
function validateEncryptionKeyForStorage({ sourceDirectory, encryptionKeys, activeKeyId } = {}) {
  ...
  const { files } = listEncryptedFiles(sourceDirectory);
  for (const file of files) decryptDocument(fs.readFileSync(file.path), keys);
  return { ok: true, fileCount: files.length };
}

function readAndVerifyBackup(backupDirectory) {
  ...
  for (const raw of manifest.files) {
    ...
    if (content.length !== Number(raw.byteSize) || sha256(content) !== String(raw.sha256 || "").toLowerCase()
      || !content.subarray(0, ENCRYPTED_FILE_MAGIC.length).equals(ENCRYPTED_FILE_MAGIC)) {
      throw storageError(`Die Integritätsprüfung für ${storageKey} ist fehlgeschlagen.`, "AMU_BACKUP_INTEGRITY_FAILED");
    }
    verified.push({ storageKey, content, byteSize: content.length, sha256: raw.sha256 });
  }
...
function restoreEncryptedFilesBackup({ backupDirectory, targetDirectory } = {}) {
  ...
  const { manifest, verified, keyCheckContent } = readAndVerifyBackup(backupDirectory);
  safeMkdir(path.join(prepared, "blobs"));
  try {
    writeExclusiveAndSync(path.join(prepared, KEY_CHECK_FILE), keyCheckContent);
    for (const file of verified) {
      const targetPath = path.join(prepared, "blobs", ...file.storageKey.split("/"));
      safeMkdir(path.dirname(targetPath));
      writeExclusiveAndSync(targetPath, file.content);
    }
    atomicReplaceDirectory(prepared, target);
