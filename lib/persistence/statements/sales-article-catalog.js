"use strict";
const SEARCH_PARAMETERS = Object.fromEntries(Array.from({ length: 10 }, (_, i) => ["query" + i, "text"]));
...
const ARTICLE_SEARCH_COLUMNS = Object.freeze({
  ...
  sourceSystem: "text",
  currentRevision: "safe_integer",
  ...Object.fromEntries(PRICE_SEARCH_FIELDS.map(p => [p.id, nullable('decimal_string')])),
});
...
const SALES_ARTICLE_CATALOG_STATEMENTS = Object.freeze({
  searchProjectionDirty: queryOne("search-projection.dirty", {}, { productId: "text" }),
  searchProjectionSource: queryOne("search-projection.source", { productId: "text" }, {
    ...ARTICLE_SEARCH_COLUMNS, sourceSnapshotId: "text",
  }),
  upsertSearchProjection: execute("search-projection.upsert", {
    ...ARTICLE_SEARCH_COLUMNS, sourceSnapshotId: "text", searchText: "text", articleNumberSort: "text", descriptionSort: "text",
    ...Object.fromEntries(PRICE_SEARCH_FIELDS.map(p => [p.id + 'Sort', nullable('text')])),
  }),
  clearSearchProjectionDirty: execute("search-projection.clear-dirty", { productId: "text" }),
  listActiveByNumber: queryAll("articles.list-active-by-number", { active: "boolean", limit: "safe_integer", offset: "safe_integer" }, ARTICLE_SEARCH_COLUMNS),
  countActive: queryOne("articles.count-active", { active: "boolean" }, { total: "safe_integer" }),
