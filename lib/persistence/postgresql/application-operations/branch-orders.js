...
function createPostgresqlBranchOrderOperations(database) {
  ...
  async function normalizeConfiguration(locationId, input = {}) {
    ...
    const existingUnitIds = await idsForLocation("branch_order_units");
    const existingItemIds = await idsForLocation("branch_order_catalog_items");
    const existingGroupIds = await idsForLocation("branch_order_groups");
