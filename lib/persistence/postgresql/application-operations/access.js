...
async function openCoreOperations(options,{pool:suppliedPool}={}){
 // Keep the qualified connection between the 30-second diagnostics polls.
 // New/replaced connections still run the complete environment/schema checks.
 const config=configuration(options),pool=suppliedPool||new Pool({...config,max:1,min:1}),context=new AsyncLocalStorage(),initialized=new WeakSet();let savepoint=0;
