...
function matchingOpeningParenthesisPosition(tokens, significant, closePosition) {
  let depth = 0;
  for (let position = closePosition; position >= 0; position -= 1) {
    const token = tokens[significant[position]];
    if (tokenIsSymbol(token, ")")) depth += 1;
    if (tokenIsSymbol(token, "(")) {
      depth -= 1;
...
function portableCaseNormalizedLikeAt(tokens, significant, position) {
  const rightFunction = tokens[significant[position + 1]];
  const rightOpen = tokens[significant[position + 2]];
  const rightParameter = tokens[significant[position + 3]];
  const rightClose = tokens[significant[position + 4]];
  const escape = tokens[significant[position + 5]];
  const escapeCharacter = tokens[significant[position + 6]];
  const close = tokens[significant[position - 1]];
  if (lowerToken(rightFunction) !== "lower"
    || !tokenIsSymbol(rightOpen, "(")
    || rightParameter?.type !== "parameter"
    || !tokenIsSymbol(rightClose, ")")
    || lowerToken(escape) !== "escape"
    || escapeCharacter?.type !== "string"
    || escapeCharacter.text !== "'!'"
    || !tokenIsSymbol(close, ")")) return false;
  const openPosition = matchingOpeningParenthesisPosition(tokens, significant, position - 1);
  const leftFunction = lowerToken(tokens[significant[openPosition - 1]]);
  return openPosition > 0
    && ["lower", "gp_unicode_casefold"].includes(leftFunction);
}
...
function detectBlockingFeatures(tokens, significant) {
  ...
  for (let position = 0; position < significant.length; position += 1) {
    ...
    if (word === "like" && !portableCaseNormalizedLikeAt(tokens, significant, position)) {
      features.add("sqlite.like-operator");
    }
...
function sqliteApplicationFunctionReplacements(tokens, significant) {
  const replacements = new Map();
  let usesUnicodeCasefold = false;
  for (let position = 0; position < significant.length; position += 1) {
    if (!functionCallAt(tokens, significant, position, "gp_unicode_casefold")) continue;
    replacements.set(significant[position], "LOWER");
    usesUnicodeCasefold = true;
  }
  return Object.freeze({ replacements, usesUnicodeCasefold });
}
...
function compilePostgresqlDialectEntry(options = {}) {
  ...
  const usesCamelcaseAliasQuoting = replacements.size > 0
    || Boolean(jsonCompilation?.supported && jsonCompilation.replacements.size > 0);
  const applicationFunctions = sqliteApplicationFunctionReplacements(tokens, significant);
  for (const [index, replacement] of applicationFunctions.replacements) {
    if (replacements.has(index)) {
  ...
  if (applicationFunctions.usesUnicodeCasefold) {
    coveredFeatures.push("postgresql.sqlite-unicode-casefold");
  }
