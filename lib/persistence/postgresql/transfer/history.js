...
async function connect(domain,url,staging=null) {
  ...
  try {
    ...
    if(domain==='core')await verifyCoreSchema(client);
      ...
      if(JSON.stringify(rows.map(r=>r.stage))!=='[5,6,7,8]'||rows.some(r=>r.source_sha256!==source.schemaSha256||r.plan_sha256!==createSalesSchemaPlan(r.stage).digest)||rows.at(-1).target_sha256!==await schemaFingerprint(client,SCHEMAS))throw new Error('MIGRATION_SALES_SCHEMA_DRIFT');
    }
    return client;
