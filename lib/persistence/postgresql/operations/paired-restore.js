...
const RESTORE_TOOL_TIMEOUT_MS=45*60*1000;
const quote=value=>{if(!/^[a-z][a-z0-9_]{0,62}$/.test(value))throw new Error('PG_PAIR_RESTORE_IDENTIFIER');return '"'+value+'"';};

function waitForRestoreTool(child,{name,commandNumber}) {
 // Restoring the sales domain includes index builds and foreign-key scans.
 // The isolated worker reserves additional time for the other domain and
 // application verification beyond this bounded native-tool allowance.
 const timeoutMs=name==='pg_restore'?RESTORE_TOOL_TIMEOUT_MS:15*60*1000;
 return new Promise((resolve,reject)=>{
  let expired=false;
  const timer=setTimeout(()=>{expired=true;child.kill('SIGTERM');},timeoutMs);
  child.once('error',()=>{clearTimeout(timer);reject(new Error('PG_PAIR_RESTORE_TOOL_START'));});
  child.once('close',code=>{
   clearTimeout(timer);
   code===0&&!expired?resolve():reject(Object.assign(new Error('PG_PAIR_RESTORE_TOOL_FAILED'),{tool:name,commandNumber}));
  });
 });
}
...
async function restorePair({bundle,commitMarker,workRoot,onRestored,privateApplicationNetwork=false}={}){
 ...
 const verified=await verifyPairBundle(bundle,commitMarker);
 ...
 try{
  ...
  // This short-lived restored cluster shares one CPU with application checks.
  ...
  for(const d of config.domains){
   ...
   const before=performance.now();await native('pg_restore',['--exit-on-error','--single-transaction','--dbname='+d.database,bundle+'/'+d.domain+'.dump'],{database:d.database});timings.push({domain:d.domain,milliseconds:Math.round(performance.now()-before)});
  ...
  if(onRestored)result.application=await onRestored({config,connect,socket,workRoot,bundle});
...
module.exports={restorePair,protectedRecords,waitForRestoreTool,RESTORE_TOOL_TIMEOUT_MS};
