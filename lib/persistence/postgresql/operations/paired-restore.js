...
const {assertRestoreCapacity}=require('./restore-capacity');
...
async function restorePair({bundle,commitMarker,workRoot,onRestored,onProgress=()=>{},privateApplicationNetwork=false,performanceProfile='parallel'}={}){
 ...
 const currentCapacity=fs.statfsSync(workRoot);
 const capacityPlan=assertRestoreCapacity(verified.manifest,currentCapacity.bavail*currentCapacity.bsize);
