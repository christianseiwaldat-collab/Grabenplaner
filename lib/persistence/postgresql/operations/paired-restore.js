...
async function restorePair({bundle,commitMarker,workRoot,onRestored,privateApplicationNetwork=false}={}){
 ...
 async function native(name,args,{database='postgres'}={}){
  ...
  await new Promise((resolve,reject)=>{let expired=false;const timer=setTimeout(()=>{expired=true;child.kill('SIGTERM');},900000);child.once('error',()=>{clearTimeout(timer);reject(new Error('PG_PAIR_RESTORE_TOOL_START'));});child.once('close',code=>{clearTimeout(timer);code===0&&!expired?resolve():reject(Object.assign(new Error('PG_PAIR_RESTORE_TOOL_FAILED'),{tool:name,commandNumber}));});});
 }
...
module.exports={restorePair,protectedRecords};
