...
async function captureCheckpoint(client,domain){
 ...
 const counts=[];
 for(const t of tables){const name=quote(t.schemaname)+'.'+quote(t.tablename);counts.push({schema:t.schemaname,table:t.tablename,rows:(await client.query('SELECT count(*)::text AS n FROM '+name)).rows[0].n});}
 ...
 return {schemaSha256:await schemaFingerprint(client,schemas),tables:counts,sequences};
}
