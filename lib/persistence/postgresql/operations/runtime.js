...
const {safeRoot,safePath}=require('./paired-bundle');
...
function copyRecoveryFiles(sourceRoot,directory){
 const volatileRoot=path.join(sourceRoot,'private','amu','tmp');
 const include=source=>source!==volatileRoot&&!source.startsWith(volatileRoot+path.sep);
 for(const name of ['private','branding-kits']){
  const source=path.join(sourceRoot,name);if(fs.existsSync(source))fs.cpSync(source,path.join(directory,name),{recursive:true,errorOnExist:true,force:false,dereference:false,filter:include});
 }
}
function recoveryFilesPreflight(sourceRoot){
 const volatileRoot=path.join(sourceRoot,'private','amu','tmp');
 let files=0,bytes=0;
 function inspect(target){
  if(target===volatileRoot||target.startsWith(volatileRoot+path.sep))return;
  const relative=path.relative(sourceRoot,target).split(path.sep).join('/');
  safePath(sourceRoot,relative);
  const stat=fs.lstatSync(target);
  if(stat.isSymbolicLink())throw new Error('PG_PAIR_COMPONENT_TYPE');
  if(stat.isDirectory()){
   for(const name of fs.readdirSync(target).sort())inspect(path.join(target,name));
   return;
  }
 ...
 for(const name of ['private','branding-kits']){
  const source=path.join(sourceRoot,name);if(fs.existsSync(source))inspect(source);
 }
 return {verified:true,files,bytes,excluded:['private/amu/tmp']};
}
 ...
 try{
  ...
  for(const domain of config.domains){
   ...
   try{
    ...
    if(domain.domain==='sales'&&await require('../sales/import-delete').target(client,(await client.query('SELECT target_sha256 FROM gp.sales_migration_history ORDER BY stage DESC LIMIT 1')).rows[0]?.target_sha256)!==digest)throw new Error('PG_OPERATIONS_SCHEMA');
...
async function createBackup(config){
 const started=performance.now(),preparationTimings=[],measure=phaseTimer(entry=>preparationTimings.push(entry));
 await measure('source-file-preflight',()=>recoveryFilesPreflight(config.sourceFiles));
 const identity=await measure('database-identity',()=>inspectPair(config)),runId=require('node:crypto').randomUUID();
 ...
 try{const result=await createPairedSnapshot({backupDirectory:config.backupDirectory,domains,
  ...
  async readCheckpoint({domain,client,onTiming}){return {...identity[domain],restore:await require('./paired-checkpoint').captureCheckpoint(client,domain,{onTiming})};},
  async runDump({domain,database,snapshotId,targetFile,onTiming}){
   const measure=phaseTimer(onTiming);
   const account=domains.find(d=>d.domain===domain);
   await measure('dump',()=>nativeTool(config,'pg_dump',{account,database,argv:['--format=custom','--compress=gzip:1','--snapshot='+snapshotId,'--role=gp_'+domain+'_owner']},{outputFile:targetFile,errorFile:config.workDirectory+'/'+runId+'-'+domain+'-dump.error'}));
   await measure('dump-catalog',()=>nativeTool(config,'pg_restore',{account,database,argv:['--list',targetFile]},{outputFile:config.workDirectory+'/'+runId+'-'+domain+'-catalog',errorFile:config.workDirectory+'/'+runId+'-'+domain+'-catalog.error'}));
  },
  async captureRecoveryFiles({directory,onTiming}){
   const measure=phaseTimer(onTiming);
   await measure('file-copy',()=>{
   recoveryFilesPreflight(config.sourceFiles);
   copyRecoveryFiles(config.sourceFiles,directory);
   ...
   await measure('roles-dump',()=>nativeTool(config,'pg_dumpall',{account:config.administrator,database:'postgres',argv:['--roles-only','--no-role-passwords']},{outputFile:directory+'/roles.sql',errorFile:config.workDirectory+'/'+runId+'-roles.error'}));
  }
 });
 if(config.mode==='productive'){
 ...
 return {ok:true,...result,phaseTimings:[...preparationTimings,...result.phaseTimings],operationMilliseconds:Math.round(performance.now()-started),path:result.bundle,amuBackup:result.bundle+'/private/amu'};}finally{await Promise.allSettled(domains.map(d=>d.pool.end()));}
}
...
module.exports={FORMAT,loadConfiguration,inspectPair,createBackup,monitor,nativeTool,url,latestBackup,copyRecoveryFiles,recoveryFilesPreflight};
