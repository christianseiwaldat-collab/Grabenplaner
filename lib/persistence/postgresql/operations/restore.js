...
function verifyRestoredProtectedDocuments({
  ...
  const verifyFile = (target, expectedContent) => {
    const stat = fs.lstatSync(target);
    if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1
      || stat.size !== expectedContent.length
      || crypto.createHash("sha256").update(fs.readFileSync(target)).digest("hex")
        !== crypto.createHash("sha256").update(expectedContent).digest("hex")) {
      throw restoreError(POSTGRESQL_RESTORE_ERROR_CODES.EVIDENCE_MISMATCH);
    }
  };
  verifyFile(path.join(restoredDirectory, "key-check.amu"), backup.keyCheckContent);
