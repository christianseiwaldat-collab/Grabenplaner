...
async function createPairedSnapshot({backupDirectory,domains,withQuiescedWrites,runDump,captureRecoveryFiles,readCheckpoint}={}){
 ...
 return withQuiescedWrites(async gate=>{
  ...
  fs.mkdirSync(directory,{mode:0o700});const clients=[],checkpoints={},databaseBytes={},snapshots={},timings=[],phaseTimings=[];const started=performance.now();
  ...
  try{
   ...
   for(const domain of domains){
    ...
    assertConnections();timings.push({domain:domain.domain,milliseconds:Math.round(performance.now()-before)});
   }
   await measure('recovery-files',()=>captureRecoveryFiles({directory,checkpoints,onTiming:recordTiming}));
   assertConnections();
   for(const client of [...clients].reverse()){await client.query('COMMIT');assertConnections();}
   const result=await measure('hash-and-seal',()=>sealPairBundle(directory,{snapshotId,databases:domains.map(d=>({domain:d.domain,database:d.database,file:d.domain+'.dump'})),checkpoint:{quiescence:'application-and-files-drained;both-databases-share-locked',domains:checkpoints,databaseBytes}}));
