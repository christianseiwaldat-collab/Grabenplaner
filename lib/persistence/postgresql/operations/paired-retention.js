...
function configuredPairedRetention(config){
 const policy=config.localBackupRetention;
 if(policy===undefined)return null;
 if(!policy||typeof policy!=='object'||Array.isArray(policy)||Object.keys(policy).join(',')!=='keepCount'
  ||!Number.isInteger(policy.keepCount)||policy.keepCount<2||policy.keepCount>1000)throw new Error('PG_PAIR_RETENTION_POLICY');
 return {keepCount:policy.keepCount};
}
async function prunePairedSnapshots(root,{keepDays=20,keepCount,now=Date.now()}={}){
 safeRoot(root);
 const countMode=keepCount!==undefined;
 if(!Number.isFinite(now)||(countMode
  ?!Number.isInteger(keepCount)||keepCount<2||keepCount>1000
  :!Number.isInteger(keepDays)||keepDays<1||keepDays>1000))throw new Error('PG_PAIR_RETENTION_POLICY');
 const points=[];
 ...
 for(const point of points)if(countMode?keep.size<keepCount:!days.has(point.day)&&days.size<keepDays){days.add(point.day);keep.add(point.directory);}
 const removed=[];
 ...
 return {providerId:'postgresql-pair',policy:countMode?'latest-count':'daily',verifiedPoints:points.length,retainedPoints:keep.size,retainedDays:days.size,removed};
}
module.exports={prunePairedSnapshots,configuredPairedRetention};
