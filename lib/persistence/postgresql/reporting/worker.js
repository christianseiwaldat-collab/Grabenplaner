...
parentPort.on('message',async({id,input})=>{
  ...
  try{
    ...
    if(input.operation==='trade-insights'){const result=await env.insights.run(()=>resolve(input.session?.employeeNumber),input.kind,input.query,{sourceRevision:input.sourceRevision});parentPort.postMessage({id,result});return;}
