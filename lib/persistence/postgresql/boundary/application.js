...
async function openTwoDatabaseDevelopmentApplication({coreUrl,salesUrl,stage=7,authorize,autoDeliver=false,onInitializationPhase,...options}){
  ...
  if(stage>=8)for(const entry of require('../sales/import-delete-catalog').CATALOG)owners.set(entry.statement.id,'sales');
  ...
  for(const entry of require('../core/import-delete-catalog').CATALOG)owners.set(entry.statement.id,'core');
