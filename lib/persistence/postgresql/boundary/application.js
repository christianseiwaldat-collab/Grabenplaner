...
const {createCoreCatalog}=require('../core/catalog');
const {createSalesCatalog}=require('../sales/catalog');
...
async function openTwoDatabaseDevelopmentApplication({coreUrl,salesUrl,stage=7,authorize,autoDeliver=false,...options}){
  ...
  try{core=await openCoreDevelopmentApplication({...settings,databaseUrl:coreUrl,boundary:true});sales=await openSalesDevelopmentApplication({...settings,databaseUrl:salesUrl,stage});}
  catch(e){await core?.close();await locks.end();throw e;}
  const owners=new Map();
  for(const [domain,entries] of [['core',[...createCoreCatalog().entries,...CORE_BOUNDARY_CATALOG]],['sales',[...createSalesCatalog(stage).entries,...SALES_BOUNDARY_CATALOG,...receipts.CATALOG,...(stage>=8?require('../reporting/catalog').REPORTING_BOUNDARY_CATALOG:[])]]])for(const e of entries){
  ...
  async function lock(){
    ...
    catch(e){client.release(true);throw e;}
  ...
  async function begin(options){
    ...
    async function query(statement,parameters){
      ...
      if(statement===S.dependencies){
        ...
        const [local,remote]=await Promise.all([salesTx.queryOne(statement,parameters),coreTx.queryOne(B.masterDependents,{id:parameters.id})]);
        touched.add(statement.id);return [{count:local.count+remote.count}];
      }
