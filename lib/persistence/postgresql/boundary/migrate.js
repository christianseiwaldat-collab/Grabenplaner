...
async function verifyCoreSchema(client){
  ...
  expected=await require('../core/trade-annotations').target(client,expected);
  expected=await require('../core/personnel-learning-runs').target(client,expected);
  expected=await require('../core/xoffi-snapshots').target(client,expected);
