...
async function openSalesDevelopmentApplication(options={}){
  ...
  const config=configuration({...options,domain:'sales'});
  // Retain qualified connections up to the existing connection limit; hourly
  // replacement and environment/schema validation remain unchanged.
  ...
  async function prepareClient(client){
    if(!initialized.has(client)){
      ...
      if(rows.at(-1)?.stage!==stage||rows.some(r=>r.plan_sha256!==createSalesSchemaPlan(r.stage).digest||r.source_sha256!==createSalesSchemaPlan(r.stage).sourceSchemaSha256)||await require('./import-delete').target(client,rows.at(-1).target_sha256)!==await schemaFingerprint(client,SCHEMAS))throw new Error('Sales application schema contract mismatch');
  ...
  try{
    ...
    if(stage>=8)entries.push(...require('./import-recheck-catalog').IMPORT_RECHECK_CATALOG);
    if(stage>=8)entries.push(...require('./cash-publication-batches').CASH_PUBLICATION_BATCH_CATALOG);
    if(stage>=8)entries.push(...require('./import-batches').IMPORT_BATCH_CATALOG);
    if(stage>=8)entries.push(...require('./import-delete-catalog').CATALOG);
    if(stage>=8)entries.push(...require('./import-reader-batches').IMPORT_READER_BATCH_CATALOG);
    const provider=createPostgresqlPersistenceProvider({pool:boundedPool,prepareClient,catalog:require('./import-work-queues').indexedImportWorkQueues(entries),poolOwnership:'provider'});
    if(stage>=8)require('../../repositories/import-batch-support').enableImportBatches(provider);
