...
async function migrateSalesDevelopment(client,stage=5){
  ...
  try{
    ...
    if(exists){
      ...
      if(await require('./import-delete').target(client,previous.at(-1).target_sha256)!==await schemaFingerprint(client,SCHEMAS))throw new Error('Sales target schema drift');
