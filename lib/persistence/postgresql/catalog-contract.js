...
const POSTGRESQL_FULL_APPLICATION_STATEMENT_COUNT = 891;
...
function closedApplicationAcceptanceError() {
  return invalidCatalog(
    "A complete branded PostgreSQL application catalog is unavailable "
      + "because the acceptance gate is closed at 0/891 live/parity receipts.",
  );
}
...
function definePostgresqlApplicationCatalog(options = {}) {
  ...
  if (fullApplicationCatalog
    ...
    throw invalidCatalog("A full PostgreSQL application catalog must cover exactly 891 statements.");
  }
  if (!fullApplicationCatalog
    && normalizedEntries.length >= POSTGRESQL_FULL_APPLICATION_STATEMENT_COUNT) {
    throw invalidCatalog(
      "A partial PostgreSQL development catalog cannot contain all 891 statements.",
    );
  }
