...
function mapPostgresqlError(error, {
  ...
  if (driverCode === "23505" || driverCode === "23P01") {
    ...
    // pg 8.22 emits its configured client read timeout without an error code.
    // Match only that exact driver error so import jobs can retain/retry their
    // checkpoint; unknown application/schema failures remain non-retryable.
    || (!driverCode && error?.message === "Query read timeout")) {
...
function createPostgresqlPersistenceProvider({
  ...
  if (!["external", "provider"].includes(poolOwnership)
    ...
    || (prepareClient !== undefined && typeof prepareClient !== "function")
  ...
  async function acquireClient(operation) {
    ...
    try {
      ...
      // The acquisition deadline measures waiting for a connection. A newly
      // acquired connection may still need environment/schema verification;
      // those queries retain their own server and driver deadlines.
      clearTimeout(timer);
      if (prepareClient) {
        try { await prepareClient(client); }
        catch (error) { try { client.release(true); } catch {} throw error; }
      }
      return client;
    } catch (error) {
