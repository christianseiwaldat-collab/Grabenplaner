...
function annotations({protection,scopeId}){
 ...
 async function read(tx,kind,key){const row=await Q.read(tx,A.get,{scopeId,kind,id:id(kind,key)});return row?{revision:row.revision,value:protection.open(row.payload,context(row))}:{revision:0,value:null};}
