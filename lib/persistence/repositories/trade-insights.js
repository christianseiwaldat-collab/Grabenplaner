...
function createTradeInsightRuntime({access,vault,scopeId=scopeIdDefault,cashBackendFactory,today=()=>new Date().toISOString().slice(0,10)}){
 async function run(getSession,operation,input={}){
  ...
  try{
   if(!protection)return {available:false,projection:p,locations:[]};
   const result=await access.transaction(async tx=>{
    const epoch=await tx.queryOne(S.epoch,{scopeId}),reader=createSalesMasterReader({protection,scopeId});
    ...
    function history(prefetched){return createImportHistoryService({access,executor:tx,protection,prefetched,getActor:()=>({scopeId,ownerId:String(session.employeeNumber)}),
     authorize:v=>['history.read','history.scope','history.reference'].includes(v.action)
      &&(!v.sourceInstance||['tradefoto-bestell','tradefoto-trade'].includes(v.sourceInstance))
    ...
    if(operation==='context'){
     ...
     return {available:true,projection:p,locations:rows.filter(l=>l.active&&(p.company||p.locationIds.includes(l.id))).map(l=>({id:l.id,label:l.label})),sources,today:today()};
    }
    if(['inventory','prices','stock-metadata','classification','classification-save'].includes(operation)){
