...
function methodsFor(access) {
  const methods = {};
  for (const [name, statement] of Object.entries(WIFI_AUTOMATION_STATEMENTS)) {
...
const createWifiAutomationRepository = Object.freeze(function createWifiAutomationRepository(access) {
  ...
  const repository = Object.freeze({
    ...
    ...(typeof access.transaction === "function" ? {
      transaction(work) {
        ...
        return access.transaction((executor) => work(createWifiAutomationRepository(executor)));
      },
    } : {}),
  });
...
function assertWifiAutomationRepository(repository) {
  if (!REPOSITORIES.has(repository)) {
    throw new PersistenceError(PERSISTENCE_ERROR_CODES.CONTRACT_VIOLATION, {
      operation: "wifi-automation-repository",
    });
  }
  return repository;
}
...
module.exports = {
  assertWifiAutomationRepository,
