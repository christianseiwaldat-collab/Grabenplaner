...
async function refreshArticleSearchProjection(operations, productId) {
  const source = await operations.queryOne(S.searchProjectionSource, { productId });
  if (!source) throw retryable("sales-article-search-source-incomplete");
  await operations.execute(S.upsertSearchProjection, articleSearchProjection(source));
  await operations.execute(S.clearSearchProjectionDirty, { productId });
}
...
async function persistSnapshot(
  ...
  for (let index = 0; index < snapshot.articles.length; index += 1) {
    ...
    await refreshArticleSearchProjection(operations, productId);
  }
...
async function persistImportUndo(
  ...
  for (const { impact, target, active } of restorations) {
    ...
    await refreshArticleSearchProjection(operations, impact.productId);
  }
...
async function persistManagedRevision(operations, value) {
  ...
  await refreshArticleSearchProjection(operations, value.productId);
  const auditId = await insertManualAudit(operations, {
    ...value,
    action: value.action,
    detail: {
...
const createSalesArticleCatalogRepository = Object.freeze(function createSalesArticleCatalogRepository(
  ...
  const transact = (work) => (typeof access.transaction === "function"
    ...
    : work(direct));
  return Object.freeze({
    async search(value = {}, projection = {}) {
      const operation = "sales-article-catalog-search";
      const input = normalize(operation, () => normalizeSalesArticleSearch(value));
      const priceSort = PRICE_SEARCH_FIELDS.find(p => p.id === input.sort);
      if (priceSort && !projection[priceSort.permission]) throw invalidInput(operation);
      const parameters = {
      ...
      const read = async operations => {
        if (await operations.queryOne(S.searchProjectionDirty, {})) {
          throw retryable("sales-article-search-projection-not-current");
        }
        const simple = input.active !== null && input.sourceSystem === null && input.identifier === ""
          && Object.values(searchParameters(input.query)).every(value => value === "%");
        const ordered = simple && input.sort === "articleNumber" && input.direction === "asc";
        const [items, count] = await Promise.all([
          operations.queryAll(ordered ? S.listActiveByNumber : S.search, ordered ? {
            active: input.active, limit: input.limit, offset: input.offset,
          } : {
            ...parameters,
            sort: input.sort,
          ...
          operations.queryOne(simple ? S.countActive : S.countSearch, simple ? { active: input.active } : parameters),
        ]);
        return Object.freeze({
          items: Object.freeze(items.map(row => Object.fromEntries(Object.entries(row).filter(([key]) => {
            const price = PRICE_SEARCH_FIELDS.find(p => p.id === key); return !price || projection[price.permission] === true;
          })))),
          total: count?.total || 0,
          limit: input.limit,
          offset: input.offset,
      ...
      return typeof access.transaction === "function"
        ? access.transaction(tx => read(primitiveOperations(tx)), { isolation: "serializable", readOnly: true })
        : read(direct);
    },
