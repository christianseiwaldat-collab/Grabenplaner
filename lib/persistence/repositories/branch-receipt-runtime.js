  ...
  return Object.freeze({ async run(getSession, work) {
    ...
    try {
      ...
      if (protection) {
        ...
        if (publication) {
          const backend = cashBackendFactory({ publication, publications, scopeId });
          ...
          const receipts = dispatchRead ? Object.freeze(Object.fromEntries(["search", "documents"].map(operation => [operation, async query => {
            ...
            resultStore, summaryStore, projectionFor: branchReceiptProjection });
          workspace = Object.freeze({ receipts, sourceRevision, context: () => ({ available: true, today: today(),
            ...
            location: { id: context.locationId, name: backend.source.locations.find(l => l.id === context.locationId)?.label || context.locationId } }) });
        }
      }
