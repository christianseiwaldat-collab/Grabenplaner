...
function createCashHistoryBackend({ publication, publications, scopeId, prefetchRows = null }) {
  ...
  return Object.freeze({
    ...
    policy,
    async searchReportOnline(tx, p) {
      const table = TABLES.find(t => t.name === 'Umsatz_Kasse_Details'), result = [];
      for (const sourceId of ONLINE_LOCATION.sourceIds) {
        const rows = await tx.queryAll(S.reportSourceSearch, { datasetSlot: dataset.row.slot,
          locationKey: sourceId === '0' ? null : publications.referenceKey('FILIALEN', sourceId),
          dateFrom: p.dateFrom, dateTo: p.dateTo, afterDate: p.afterDate,
          afterRow: p.afterId === '~' ? C.LIMITS.rows + 1 : parseId(p.afterId, table), limit: p.limit });
        result.push(...rows.map(r => ({ id: rowId(table, r.sourceRow), businessDate: r.businessDate, onlineCandidate: true })));
      }
      return result;
    },
    async isReportOnline(tx, id) {
      const table = TABLES.find(t => t.name === 'Umsatz_Kasse_Details');
      return isOnlineSource((await decoded(tx, table, parseId(id, table))).normalized.source.Filialid);
    },
    async reportSellerOptions(tx) {
      await publications.verifyFilterBindings(tx, publication, 'MITARBEITER');
      const bindings = await tx.queryAll(S.bindings, { publicationId: pub.id, kind: 'MITARBEITER', limit: 10001 });
      return [...new Set(bindings.map(b => b.targetId))].sort((a, b) => a.localeCompare(b, 'de-AT', { numeric: true }))
        .map(id => ({ id, label: `MA ${id}` }));
    },
    async epoch(tx) {
      const current = await publications.state(tx);
      if (current.data.active !== pub.id) C.fail('IMPORT_HISTORY_RESULTS_CHANGED', 409);
      const fresh = await publications.load(tx, pub.id);
      if (fresh.dataset.row.revision !== dataset.row.revision || fresh.row.payload !== pub.payload) C.fail('IMPORT_HISTORY_RESULTS_CHANGED', 409);
      return [pub.id, current.row.revision, dataset.row.revision, policy.fingerprint, C.fingerprint(pub.payload)];
    },
    async search(tx, p) {
      ...
      const rows = await tx.queryAll((p.unassigned ? S.search : S.searchAssigned)[table.name], { publicationId: pub.id, datasetSlot: dataset.row.slot,
    ...
    service(tx, authorize) {
      const cache = new Map(), referenceCache = new Map(), bindingCache = new Map(), childrenCache = new Map();
      ...
      async function references(entry) {
        if (referenceCache.has(entry)) return referenceCache.get(entry);
        ...
        if (entry.table.name === 'Umsatz_Kasse_Details') {
          const table = TABLES.find(t => t.name === 'Umsatz_KASSE'), parent = await loadId(rowId(table, entry.row.parentRow));
        ...
        for (const r of requests) {
          const value = r.key?.[0] ?? null, key = C.canonical([r.table, value]);
          if (!bindingCache.has(key)) bindingCache.set(key, await publications.reference(tx, pub.id, r.table, value));
          refs.push({ role: r.role, dataClass: r.dataClass, targetKind: kinds[r.table], ...bindingCache.get(key) });
        }
        referenceCache.set(entry, refs);
        return refs;
      }
      async function childEntries(head) {
        if (!childrenCache.has(head)) {
          const table = TABLES.find(t => t.name === 'Umsatz_Kasse_Details');
          ...
          const entries = rows.map(row => {
            const id = rowId(table, row.sourceRow);
            if (!cache.has(id)) cache.set(id, { table, row, ...reader.decode(dataset, table, row) });
            return cache.get(id);
          });
          childrenCache.set(head, entries);
        }
        return childrenCache.get(head);
      }
      async function scope(entry, refs) {
        if (!await allowed(entry.table, 'history.scope', [], { locations: refs.filter(r => r.targetKind === 'location'), sourceLocationId: entry.normalized.source.Filialid })) C.fail('IMPORT_FORBIDDEN', 403);
      }
      return Object.freeze({
        ...(prefetchRows ? { async prefetch(ids, childrenIds=ids) {
          // Only this transaction owns these caches. Authorization still runs
          // through detail/children/receipt before any value leaves the service.
          const heads = TABLES.find(t => t.name === 'Umsatz_KASSE');
          if (!Array.isArray(ids) || ids.length > 20) C.fail('IMPORT_COMPOSITION_INVALID');
          const ordinals = ids.map(id => parseId(id, heads));
          if (!ordinals.length) return;
          const rows = await prefetchRows(tx, 'heads', dataset.row.slot, ordinals, 20);
          for (const row of rows) cache.set(rowId(heads,row.sourceRow), {table:heads,row,...reader.decode(dataset,heads,row)});
          if(!Array.isArray(childrenIds)||childrenIds.some(id=>!ids.includes(id)))C.fail('IMPORT_COMPOSITION_INVALID');
          const childOrdinals=childrenIds.map(id=>parseId(id,heads));if(!childOrdinals.length)return;
          const details = TABLES.find(t => t.name === 'Umsatz_Kasse_Details');
          const children = await prefetchRows(tx, 'children', dataset.row.slot, childOrdinals, 2001);
          // A bounded partial preload is never mistaken for complete coverage.
          // Oversized batches use the existing per-receipt bound and validation.
          if (children.length >= 2001) return;
          const grouped = new Map(childOrdinals.map(n => [n,[]]));
          for (const row of children) {
            if (!grouped.has(row.parentRow)) C.fail('IMPORT_HISTORY_INTEGRITY');
            grouped.get(row.parentRow).push(row);
          }
          for (const [ordinal, values] of grouped) {
            if(values.length > 1000) continue;
            const head=cache.get(rowId(heads,ordinal));if(!head)continue;
            childrenCache.set(head,values.map(row=>{
              const id=rowId(details,row.sourceRow),entry={table:details,row,...reader.decode(dataset,details,row)};
              cache.set(id,entry);return entry;
            }));
          }
        } } : {}),
        async children(id) {
          ...
          return (await childEntries(head)).map(entry => rowId(entry.table, entry.row.sourceRow));
        },
        async detail(id) {
        ...
        async receipt(id) {
          ...
          await scope(head, await references(head));
          const table = TABLES.find(t => t.name === 'Umsatz_Kasse_Details');
          const lines = (await childEntries(head)).map(entry => ({ source: entry.normalized.source, parentRevision: 1 }));
