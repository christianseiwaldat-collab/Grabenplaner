...
function createDataImportRuntime({access,vault,scopeId='grabenplaner-main',allowApply=false,sharedPayloads=false,compactCash=false,
  ...
  const operationBatchSize=access.getCapabilities?.().providerId==='postgresql'?25:C.LIMITS.batch;
  ...
  async function withContext(getSession,action,create,work) {
    ...
    const engine=createDataImportEngine({repository,protection,profiles:PROFILES,sourceTolerances,sharedPayloads,operationBatchSize,
      operationBudgetMs:access.getCapabilities?.().providerId==='postgresql'?1000:Infinity,getActor:()=>actor,authorize:authorized,clock,
  ...
  function reserve(getSession,{buffer,kind}) {return lifecycle.run(async()=>{
    ...
    return withContext(getSession,'prepare',true,async({actor,protection,sourceId,get,summarize})=>{
      ...
      const data={kind,fileSha256,bytes:buffer.length,status:'interrupted',complete:false,error:'IMPORT_SOURCE_INCOMPLETE',tables:[],excludedTableNames:[],
      ...
      return summarize(await get(id));
    });
  });}
  async function uploadSource(getSession,{buffer,kind,password='',onStarted=()=>{},signal}) {
    try {
      interrupted(signal);
      assertSourceBytes(buffer); definitions(kind);
          ...
          try {
            await readSource({buffer,kind,password,signal,consumeBuffer:true,onMessage:async message=> {
              ...
              if(message.type==='manifest') {
                ...
                else for(let offset=0;offset<message.rows.length;offset+=operationBatchSize) {
                  current.run=await engine.stage(current.run.id,{expectedRevision:current.run.revision,startRow:message.startRow+offset,rows:message.rows.slice(offset,offset+operationBatchSize)});
                  interrupted(signal);
                }
