...
function createDataImportRuntime({access,vault,scopeId='grabenplaner-main',allowApply=false,sharedPayloads=false,compactCash=false,
  ...
  function reserve(getSession,{buffer,kind,fileName}) {return lifecycle.run(async()=>{
    ...
    return withContext(getSession,'prepare',true,async({actor,protection,sourceId,get,summarize})=>{
      ...
      const data={kind,fileSha256,fileName,bytes:buffer.length,status:'interrupted',complete:false,error:'IMPORT_SOURCE_INCOMPLETE',tables:[],excludedTableNames:[],
    ...
    try {
      ...
      return await withContext(getSession,'prepare',true,async context=> {
        ...
        return exclusive(id,async()=> {
          ...
          const data={kind,fileSha256,fileName,bytes:buffer.length,status:'reading',complete:false,error:'',tables:[],excludedTableNames:[],
            ...(kind==='cash'&&compactCash?{storage:CASH_SNAPSHOT_FORMAT,scope:'full'}:{})};
          await access.execute(S.insertSource,{...row,payload:protection.seal(data,sourceContext(row))});
