...
const {SOURCE_CATALOG,SOURCE_KINDS}=require('../../data-import-source-catalog');
const {buildSalesArticleCatalogProjection}=require('../../sales-article-catalog-access');
const PROFILES = SOURCE_KINDS.flatMap(kind=>definitions(kind)).map(d=>d.profile);
          ...
          try {
            await readSource({buffer,kind,password,signal,consumeBuffer:true,onMessage:async message=> {
              ...
              if(message.type==='manifest') {
                ...
                if(['weum','inventur'].includes(kind)) {
                  if(message.selection?.version!==1||!Array.isArray(message.selection.tables)||message.selection.tables.length!==known.length)C.fail('IMPORT_SOURCE_PROTOCOL_INVALID');
                  for(const [index,table] of message.selection.tables.entries()) {
                    if(table.name!==known[index].name||table.selectedRows!==message.tables[index].declaredRows)C.fail('IMPORT_SOURCE_PROTOCOL_INVALID');
                    C.integer(table.sourceRows,table.selectedRows);C.integer(table.selectedRows);
                  }
                  d.selection=message.selection;
                }
  ...
  return Object.freeze({
    ...
    async context(getSession) {
      ...
      return {available,projection,activationEnabled:allowApply===true,maxBytes:512*1024*1024,
        sourceKinds:SOURCE_KINDS,sourceCatalog:SOURCE_CATALOG,
    ...
    async overview(getSession) {
      return withContext(getSession,'read',false,async c=> {
        if(!c.protection)return {items:[],complete:true};
        const latest=new Map();
        let beforeAt='9999-12-31T23:59:59.999Z',beforeId='f'.repeat(64);
        // kind lives in authenticated encrypted payloads. Do not infer the
        // latest source from the first history page or expose another owner.
        for(let page=0;page<40;page++) {
          await c.check();
          const rows=await access.queryAll(S.sources,{...c.actor,beforeAt,beforeId,limit:50});
          for(const row of rows) {
            const source=await c.get(row.id),kind=source.data.kind;
            if(SOURCE_KINDS.includes(kind)&&!latest.has(kind))latest.set(kind,c.summarize(source));
          }
          if(rows.length<50||SOURCE_KINDS.every(kind=>latest.has(kind)))return {items:[...latest.values()],complete:true};
          const last=rows.at(-1);beforeAt=last.createdAt;beforeId=last.id;
        }
        return {items:[...latest.values()],complete:false};
      });
    },
