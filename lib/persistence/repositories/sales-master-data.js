...
function createSalesMasterReader({ protection, scopeId, sourceInstance = 'tradefoto-trade', batch=false }) {
  ...
  const reader = {
    async byKey(tx, table, key, names) {
      const record = await Q.read(tx,S.find, { scopeId, identityHash: M.masterIdentity(protection, { scopeId, sourceInstance }, table, key) });
    ...
    async list(tx, table, names, max = 2000) {
      ...
      while (true) {
        ...
        if(batch)await Q.prefetch(tx,S.segments,records.map(r=>({recordId:r.id})));
    ...
    async dictionary(tx, table, names, max = 2000) {
      ...
      const allowed = { ARTIKEL_Sortimente: ['Sortiment', 'Bezeichnung', 'Warengruppe'], ARTIKEL_Warengruppen: ['Warengruppe', 'Bezeichnung'], Marken: ['Marke'] };
