...
async function loadVersion(tx, protection, record, revision = record.revision, prefetched = null) {
  // Externally supplied reporting prefetches retain their own read contract.
  if (!prefetched) return Q.verified(tx,protection,'history',{...record,versionRevision:revision},() => authenticateVersion(tx,protection,record,revision));
  return authenticateVersion(tx,protection,record,revision,prefetched);
}
async function authenticateVersion(tx, protection, record, revision, prefetched = null) {
  const profile = checkHeader(record), version = prefetched?.versions.get(record.id + ':' + revision) || await Q.read(tx, S.version, { recordId: record.id, revision });
  ...
  const segments = prefetched?.segments.get(record.id + ':' + revision) || await Q.read(tx, S.segments, { recordId: record.id, revision });
  ...
  const indices = prefetched?.references.get(record.id + ':' + revision) || await Q.read(tx, S.references, { recordId: record.id, revision });
...
function createImportHistoryWriters({ protection, authorize, resolveMasterSourceInstance }) {
  ...
  return Object.freeze(Object.fromEntries(H.TRADEFOTO_HISTORY_PROFILES.map(profile => {
    ...
    async function header(tx, id, value) {
      context(value); C.id(id); const record = await Q.read(tx, S.get, { id, scopeId: value.scopeId });
    ...
    async function dependencies(tx, data, value) {
      ...
      if (parentRequest) {
        if (parentRequest.key.some(key => key === null || key === '')) issue = 'HISTORY_PARENT_KEY_MISSING';
          ...
          const record = await Q.read(tx, S.find, { scopeId: value.scopeId, identityHash: H.historyIdentity(protection, value, source, parentRequest.name, parentRequest.key) });
          if (!record) issue = 'HISTORY_PARENT_REQUIRED';
            ...
            if(profile.sourceTable==='Inventurdetails'&&(loaded.version.fileSha256!==value.fileSha256||loaded.version.snapshotAt!==value.snapshotAt))issue='HISTORY_SOURCE_PAIR_CHANGED';
    ...
    async function review(tx, row, value) {
      ...
      const record = await Q.read(tx, S.find, { identityHash: identity(value, row.key), scopeId: value.scopeId });
      const current = record ? await loadVersion(tx, protection, record) : null;
      // Inventory is a selective, complete snapshot. Even an unchanged head or
      // exception must retain membership in the newly imported source file.
      const inventorySnapshot=['Inventur','Inventurdetails'].includes(profile.sourceTable)&&current&&(current.version.fileSha256!==value.fileSha256||current.version.snapshotAt!==value.snapshotAt);
      return { issue: checked.issue, token: checked.token, rewrite: !!current && (current.meta.dependencyToken !== checked.token||!!inventorySnapshot) };
    }
    ...
    return [profile.entity, Object.freeze({
      read, review,
      async prepareRead(tx, rows, value) {
        context(value);
        const requests = rows.map(row => ({scopeId:value.scopeId,identityHash:identity(value,row.key)}));
        for (const row of rows) {
          const parent = H.historyParentRequest(source, profile.sourceTable, row.data);
          if (parent && parent.key.every(key => key !== null && key !== '')) requests.push({scopeId:value.scopeId,identityHash:H.historyIdentity(protection,value,source,parent.name,parent.key)});
        }
        const records = (await Q.prefetch(tx, S.find, requests)).filter(Boolean);
        for (const record of records) Q.put(tx, S.get, {id:record.id,scopeId:value.scopeId}, record);
        const versions = records.map(record => ({recordId:record.id,revision:record.revision}));
        for (const statement of [S.version,S.segments,S.references]) await Q.prefetch(tx, statement, versions);
        const masterSourceInstance = C.id(await resolveMasterSourceInstance(Object.freeze({scopeId:value.scopeId,sourceInstance:value.sourceInstance,source})));
        await readReference.prefetch(tx, {...value,sourceInstance:masterSourceInstance}, rows.flatMap(row => H.historyReferenceRequests(source,profile.sourceTable,row.data)));
      },
      async findExisting(tx, row, value) { context(value); const record = await Q.read(tx, S.find, { scopeId: value.scopeId, identityHash: identity(value, row.key) }); return record ? [(await loadVersion(tx, protection, record)).target] : []; },
      ...
      async createBatch(tx, inputs, value) {
        context(value); C.integer(inputs.length,1,C.LIMITS.batch);
        const records=[], versions=[], segments=[], references=[], holds=[];
        for (const {id,data} of inputs) {
          C.id(id);
          const checked=await dependencies(tx,data,value); if(checked.issue) C.fail('IMPORT_'+checked.issue,409);
          const record={id,scopeId:value.scopeId,sourceInstance:value.sourceInstance,source,sourceTable:profile.sourceTable,
            profileHash:profile.fingerprint,identityHash:identity(value,H.historyKey(source,profile.sourceTable,data)),revision:1};
          const version={recordId:id,revision:1,fileSha256:value.fileSha256,snapshotAt:value.snapshotAt,importedBy:value.ownerId,
            importedAt:value.at,runId:value.runId,masterSourceInstance:checked.masterSourceInstance,
            businessDate:historyBusinessDate(source,profile.sourceTable,data,H),parentId:checked.parent?.id||null,parentRevision:checked.parent?.revision||null};
          const meta={references:checked.references,dependencyToken:checked.token,dataHash:protection.digest(data),restoredFromRevision:null};
          records.push(record); versions.push({...version,payload:protection.seal(meta,versionContext(record,version))});
          for(const segment of H.historySegments(source,profile.sourceTable,data)) segments.push({recordId:id,revision:1,dataClass:segment.dataClass,
            payload:protection.seal(segment.data,segmentContext(record,version,segment.dataClass))});
          for(const ref of checked.references) references.push({recordId:id,revision:1,role:ref.role,dataClass:ref.dataClass,
            masterRecordId:ref.recordId,lookupHash:referenceIndex(protection,record.scopeId,ref)});
          for(const recordId of new Set(checked.references.map(ref=>ref.recordId).filter(Boolean))) holds.push({recordId,historyId:id,revision:1});
        }
        for(const [statement,rows] of [[B.historyRecords,records],[B.historyVersions,versions],[B.historySegments,segments],[B.historyReferences,references]])
          if(rows.length && (await tx.execute(statement,{rows})).rowsAffected!==rows.length) C.fail('IMPORT_CONCURRENT_CHANGE',409);
        if(holds.length && (await tx.queryOne(B.historyHolds,{rows:holds})).count!==holds.length) C.fail('IMPORT_HISTORY_INTEGRITY');
        // Epochs are opaque invalidation tokens. One change per atomic packet
        // invalidates exactly the same readers as repeated writes in that packet.
        await expectOne(tx,S.advanceEpoch,{scopeId:value.scopeId,token:crypto.randomUUID()});
        records.forEach(record=>Q.invalidate(tx,record));
        await Q.prefetch(tx,S.get,records.map(record=>({id:record.id,scopeId:value.scopeId})));
        const requests=records.map(record=>({recordId:record.id,revision:1}));
        for(const statement of [S.version,S.segments,S.references]) await Q.prefetch(tx,statement,requests);
        const results=[]; for(const record of records) results.push(await read(tx,record.id,value));
        return results;
      },
      ...
      async remove(tx, { id, expectedRevision }, value) {
        ...
        await expectOne(tx, S.advanceEpoch, { scopeId: value.scopeId, token: crypto.randomUUID() }); Q.invalidate(tx, record); return true;
      },
    })];
  })));
}
