...
async function loadVersion(tx, protection, record, revision = record.revision) {
  const profile = checkHeader(record), version = await tx.queryOne(S.version, { recordId: record.id, revision });
  ...
  const segments = await tx.queryAll(S.segments, { recordId: record.id, revision });
  ...
  const indices = await tx.queryAll(S.references, { recordId: record.id, revision });
  ...
  return Object.freeze(Object.fromEntries(H.TRADEFOTO_HISTORY_PROFILES.map(profile => {
    ...
    return [profile.entity, Object.freeze({
      ...
      async remove(tx, { id, expectedRevision }, value) {
        ...
        await tx.execute(S.clearVersions, { recordId: id }); await expectOne(tx, S.remove, { id, scopeId: value.scopeId, expectedRevision }); return true;
      },
    })];
  })));
}
// Protected, bounded backend reads for Block 5. No route/role is registered here.
function createImportHistoryService({ access, protection, getActor, authorize, executor = null }) {
  ...
  async function load(tx, who, id, revision) {
    ...
    const record = await tx.queryOne(S.get, { id, scopeId: who.scopeId }); if (!record) C.fail('IMPORT_HISTORY_NOT_FOUND', 404);
    ...
    const loaded = await loadVersion(tx, protection, record, revision);
