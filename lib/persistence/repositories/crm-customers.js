...
const createCrmCustomersRepository = Object.freeze(function createCrmCustomersRepository(access) {
  ...
  return Object.freeze({
    async search(value) {
      ...
      const directoryEmpty = !count?.total && !(await direct.queryOne(S.countSearch, {
        ...searchParameters(''), customerType: null,
      }))?.total;
      return Object.freeze({
        items,
        directoryEmpty,
    ...
    async update(value) {
      ...
      return atomic(async (operations) => {
        ...
        const { expectedRevision, ...patch } = customer;
        const { actor: ignoredActor, timestamp: ignoredTime, ...base } = customerWriteParameters(before, actor, timestamp);
        try { customer = { ...normalizeCrmCustomerInput({ ...base, customFields: before.customFields, ...patch }), expectedRevision }; }
        catch { throw invalidInput(operation); }
