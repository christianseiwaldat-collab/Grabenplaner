...
async function branchPage(env,active,locationId,after,branches){
 ...
 const found=new Map();
 // Read bounded index batches, merge by the same stable history id used by
 // continuation cursors. This also supports more than 32 source branches.
 for(let offset=0;offset<Math.max(1,ids.length);offset+=32){
 const chunk=ids.slice(offset,offset+32);
 const candidates=await tx.queryAll(require('../statements/branch-article-stock').branchStock,{scopeId,snapshot:active.manifest.fileSha256,
  ...
  ...Object.fromEntries(Array.from({length:32},(_,i)=>['master'+i,chunk[i]||'']))});
 for(const row of candidates)found.set(row.id,row);
 }
 const candidates=[...found.values()].sort((a,b)=>a.id<b.id?-1:a.id>b.id?1:0).slice(0,PAGE+1);
...
async function stockSummary(env,input){
 ...
 const all=!q.locationId;
 const sourceSelection=q.locationId.startsWith('trade-source:');
 if(!all&&!sourceSelection&&!p.company&&!p.locationIds.includes(q.locationId))C.fail('IMPORT_FORBIDDEN',403);
 const branches=(await stockLocations(env)).filter(b=>all||(sourceSelection?b.id===q.locationId:b.locationId===q.locationId));
 ...
 for(const record of records.slice(0,PAGE)){
  ...
  if(all||sourceSelection?(!branches.some(b=>b.sourceId===text(s.FilialID))||!await env.visible(s.FilialID)):!await env.visible(s.FilialID,q.locationId))continue;
 ...
 const project=row=>{
  const value={...row,positiveQuantity:D.add(row.confirmedQuantity,row.unclassifiedQuantity),positivePositions:row.confirmedPositions+row.unclassified};
  return p.costs?value:Object.fromEntries(Object.entries(value).filter(([key])=>!['provisionalNet','confirmedNet','valued','missingCost','zeroCost'].includes(key)));
 };
