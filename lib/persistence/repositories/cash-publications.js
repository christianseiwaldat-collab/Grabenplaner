...
function createCashPublications({ access, protection, scopeId, policies = [], clock = () => new Date().toISOString() }) {
  ...
  async function candidate(tx, id, ownerId) {
    ...
    // Count immutable rows before treating the previously verified complete
    // source as receipt membership evidence. No subset is promoted to coverage.
    for (const table of TABLES) same((await tx.queryOne(table.statements.count, { datasetSlot: dataset.row.slot })).count,
      dataset.data.tables.find(t => t.name === table.name).expectedRows);
  ...
  async function load(tx, id) {
    ...
    same((await tx.queryOne(S.bindingCount, { publicationId: id })).count, data.bindingCount);
    const policy = defineTradeFotoSalesPolicy(data.policy);
    same(policy.fingerprint, data.policyFingerprint);
    return { row, data, policy, ...result };
  }
  async function references(tx, dataset, reader, kind, after = '', limit = 100) {
    ...
    for (const record of records.slice(0, limit)) {
      ...
      const table = TABLES[record.tableIndex], row = await tx.queryOne(table.statements.row, { datasetSlot: dataset.row.slot, sourceRow: record.sourceRow });
  ...
  async function plan(tx, actor, input, projection, sessionSignature) {
    ...
    if (input.resolveArticles) for (const [sourceId] of sources.get('ARTIKEL_STAMM')) {
      ...
      const target = await tx.queryOne(M.articleBySource, { sourceSystem: TRADEFOTO_ARTICLE_SOURCE_SYSTEM, sourceArticleKey: sourceId });
    ...
    for (const m of requested) {
      ...
      const target = await tx.queryOne(m.kind === 'FILIALEN' ? M.location : m.kind === 'MITARBEITER' ? M.employee : m.kind === 'KUNDEN' ? M.crmGet : M.articleBySource,
        m.kind === 'ARTIKEL_STAMM' ? { sourceSystem: TRADEFOTO_ARTICLE_SOURCE_SYSTEM, sourceArticleKey: m.sourceId } : { id: m.targetId });
    ...
    const data = { label: input.label, createdBy: actor.ownerId, datasetRevision: dataset.row.revision, fileSha256: dataset.data.fileSha256,
      bindingCount: bindings.length, bindingCounts: Object.fromEntries(Object.keys(FIELDS).map(kind => [kind, bindings.filter(b => b.kind === kind).length])),
  ...
  return Object.freeze({ state, load, candidate, references, referenceKey,
    ...
    async activate(tx, actor, input, projection, signature, expectedPlan) {
      ...
      for (const binding of p.bindings) {
        ...
        await tx.execute(S.insertBinding, { ...b, payload: protection.seal(binding, bindingContext(b)) });
      }
