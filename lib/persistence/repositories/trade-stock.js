...
async function articleCash(env,cash,articleNumber,{from,to,after=0,stats=null,limit=201,strict=false}={}){
 ...
 for(const row of sourceRows.slice(0,limit-1)){
  ...
  if(strict&&String(raw.EAN)!==String(articleNumber)){state.ambiguousArticles=(state.ambiguousArticles||0)+1;continue;}
  ...
  const stockReference=strict?await publications.reference(env.tx,pub.row.id,'FILIALEN',f.Bestandsfilialid):null;
  const stockKey=strict?(allowed(stockReference)?stockReference.targetId:null):str(f.Bestandsfilialid).replace(/^0+(?=\d)/,'');if(!stockKey)state.unknownStock++;else{
...
async function stockCounts(env,article,active){
 ...
 if(ids.length>100)return null;const counts=new Map(),records=new Set(),items=[];
 for(const entry of ids){
  ...
  items.push({id:entry.id,sourceLocation:str(f.FilialID),stock:f.FBestand??null,ordered:f.Bestellt??null,incoming:f.im_Zulauf??null,sourceDate:value.provenance.snapshotAt});
 }
 return {counts,records,items};
}
async function stockOperation(env,operation,input){
 ...
 if(operation==='stock-summary')return require('./trade-stock-summary').stockSummary(env,input);
 if(['stock-metadata','stock-summary-metadata'].includes(operation)){
  const groups=await env.reader.dictionary(tx,'ARTIKEL_Sortimente',['Sortiment','Bezeichnung','Warengruppe']);
  const wgr=await env.reader.dictionary(tx,'ARTIKEL_Warengruppen',['Warengruppe','Bezeichnung']);const cash=operation==='stock-summary-metadata'?null:await cashContext(env);
  return {groups:groups.map(g=>({id:str(g.Sortiment),label:g.Bezeichnung})),wgr:wgr.map(g=>({id:str(g.Warengruppe),label:g.Bezeichnung})),cashPeriod:cash?.bounds||null,stockLocations:await require('./trade-stock-summary').stockLocations(env)};
 }
...
module.exports={stockOperation,cashKeys,classification,groupDefinition,articleCash,cashContext,inventoryMetrics,stockCounts};
