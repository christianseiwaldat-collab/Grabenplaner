...
function createManagedSalesHistoryRuntime({ access, vault, enabled = false, cashEnabled = false, scopeId = 'grabenplaner-main', sources = [],
  getPolicy, getCoverage, today = () => new Intl.DateTimeFormat('sv-SE', { timeZone: 'Europe/Vienna' }).format(new Date()), now }) {
  C.id(scopeId);
  ...
  return Object.freeze({ async run(getSession, work) {
    ...
    const identity = value => C.canonical([String(value?.employeeNumber || ''), value?.accountId || null, buildSalesHistoryProjection(value)]);
    ...
    try {
      ...
      if (cashEnabled) {
        ...
        const publication = await access.transaction(tx => publications.active(tx), { isolation: 'serializable' });
      ...
      const workspace = createSalesHistoryWorkspace({ access, protection, sources: backend ? [backend.source] : sources,
        getPolicy: backend ? () => backend.policy : getPolicy, getCoverage, today, now, analysisStore, backend,
      ...
      const receipts = backend ? createReceiptWorkspace({ access, protection, backend, getSession: () => session,
        getActor: () => ({ scopeId, ownerId: C.id(String(session.employeeNumber)) }), today, now, resultStore }) : null;
      const result = await work(Object.freeze({ ...workspace, receipts }));
      session = await getSession();
      if (identity(session) !== signature) C.fail('IMPORT_FORBIDDEN', 403);
      return result;
