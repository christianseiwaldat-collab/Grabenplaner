...
function createManagedSalesHistoryRuntime({ access, vault, enabled = false, cashEnabled = false, scopeId = 'grabenplaner-main', sources = [],
  getPolicy, getCoverage, today = () => new Intl.DateTimeFormat('sv-SE', { timeZone: 'Europe/Vienna' }).format(new Date()), now, retainCompletedAnalyses = true, cashBackendFactory = createCashHistoryBackend }) {
  C.id(scopeId);
  ...
  const checkpointStore = createSalesReportCheckpointStore();
  return Object.freeze({ async run(getSession, work) {
    ...
    try {
      ...
      if (cashEnabled) {
        ...
        if (publication) backend = cashBackendFactory({ publication, publications, scopeId });
      }
      if (!backend && !(enabled === true && sources.length)) return work(null);
      const articleReaders=new WeakMap();
      const articleResolverFactory=tx=>{
        if(!articleReaders.has(tx))articleReaders.set(tx,require('./trade-article-history').createTradeArticleReader({access,tx,protection,scopeId,ownerId:String(session.employeeNumber)}));
        return articleReaders.get(tx);
      };
      if(backend){const original=backend;backend={...original,epoch:async tx=>[await original.epoch(tx),await tx.queryOne(require('../statements/import-history').IMPORT_HISTORY_STATEMENTS.epoch,{scopeId})]};}
      const workspace = createSalesHistoryWorkspace({ access, protection, sources: backend ? [backend.source] : sources,
        ...
        getSession: () => session, getActor: () => ({ scopeId, ownerId: C.id(String(session.employeeNumber)) }), articleResolverFactory });
      const receipts = backend ? createReceiptWorkspace({ access, protection, backend, getSession: () => session,
        getActor: () => ({ scopeId, ownerId: C.id(String(session.employeeNumber)) }), today, now, resultStore, summaryStore, articleResolverFactory }) : null;
      const marginPolicy = backend ? reportMarginPolicyFor(backend.policy) : null;
      const reports = backend ? createSalesReportWorkspace({ access, protection, backend, getSession: () => session, scopeId, today, marginPolicy, checkpointStore }) : null;
