...
async function loadMaster(tx, protection, record) {
  return Q.verified(tx,protection,'master',record,async () => {
  const profile = checkHeader(record), data = {};
  const segments = await Q.read(tx, S.segments, { recordId: record.id });
...
function createImportMasterWriters({ protection }) {
  return Object.freeze(Object.fromEntries(M.TRADEFOTO_MASTER_PROFILES.map(profile => {
    ...
    async function header(tx, id, value) {
      ...
      const record = await Q.read(tx, S.get, { id, scopeId: value.scopeId });
    ...
    return [profile.entity, Object.freeze({
      ...
      async prepareRead(tx, rows, value) {
        context(value);
        const records = (await Q.prefetch(tx, S.find, rows.map(row => ({scopeId:value.scopeId, identityHash:M.masterIdentity(protection, value, profile.sourceTable, row.key)})))).filter(Boolean);
        for (const record of records) Q.put(tx, S.get, {id:record.id,scopeId:value.scopeId}, record);
        await Q.prefetch(tx, S.segments, records.map(record => ({recordId:record.id})));
      },
      async findExisting(tx, row, value) {
        context(value);
        const record = await Q.read(tx, S.find, { scopeId: value.scopeId, identityHash: M.masterIdentity(protection, value, profile.sourceTable, row.key) });
      ...
      async create(tx, { id, data }, value) {
        ...
        await expectOne(tx, S.insert, record); await saveParts(tx, protection, record, data); Q.invalidate(tx, record); return read(tx, id, value);
      },
      async createBatch(tx, inputs, value) {
        context(value); C.integer(inputs.length, 1, C.LIMITS.batch);
        const records = [], segments = [], relations = [];
        for (const {id, data} of inputs) {
          ...
          const record = {id, scopeId:value.scopeId, sourceInstance:value.sourceInstance, sourceTable:profile.sourceTable,
            ...
            revision:1, updatedBy:value.ownerId, updatedAt:value.at};
          records.push(record);
          for (const segment of M.masterSegments(profile.sourceTable,data)) segments.push({recordId:id,kind:segment.kind,dataClass:segment.dataClass,
            payload:protection.seal(segment.data,segmentContext(record,segment))});
          for (const relation of M.masterReferences(protection,record,profile.sourceTable,data)) relations.push({recordId:id,...relation});
        }
        for (const [statement,rows] of [[B.masterRecords,records],[B.masterSegments,segments],[B.masterRelations,relations]])
        ...
        // Read the persisted headers and ciphertext back; input values are never
        // used as a substitute for the writer's authenticated result.
      ...
      async remove(tx, { id, expectedRevision }, value) {
        ...
        await expectOne(tx, S.remove, { id, scopeId: value.scopeId, expectedRevision }); Q.invalidate(tx, record); return true;
      },
    })];
  })));
}
...
function createImportMasterReferenceReader({ protection, authorize }) {
  ...
  async function readReference(tx, context, name, key) {
    ...
    const record = await Q.read(tx, S.find, { scopeId: context.scopeId, identityHash: M.masterIdentity(protection, context, name, key) });
    ...
    const binding = await Q.read(tx, S.getBinding, { recordId: record.id });
    ...
    const target = kind === "sales_article"
      ? await Q.read(tx, S.articleBySource, { sourceSystem: TRADEFOTO_ARTICLE_SOURCE_SYSTEM, sourceArticleKey: M.masterSource(name, master.data).EAN })
      : await Q.read(tx, kind === "crm_customer" ? S.crmGet : kind === "employee" ? S.employee : S.location, { id: binding.targetId });
  ...
  readReference.prefetch = async (tx, context, requests) => {
    const parameters = requests.filter(request => request.key && !(request.key[0] === '0' && ['KUNDEN','MITARBEITER'].includes(request.table)))
      .map(request => ({scopeId:context.scopeId,identityHash:M.masterIdentity(protection, context, request.table, request.key)}));
    const records = (await Q.prefetch(tx, S.find, parameters)).filter(Boolean);
    await Q.prefetch(tx, S.segments, records.map(record => ({recordId:record.id})));
    if(Q.supportsReferences(tx))await Q.prefetch(tx,S.getBinding,records.map(record=>({recordId:record.id})));
  };
