...
const CUSTOMER_MAP = Object.freeze({ KUND_NR: "customerNumber", NACHNAME: "lastName", VORNAME: "firstName", "STRaße": "street",
...
function customerProjection(source, decision) {
  ...
  if (!["private", "business"].includes(decision.customerType)) C.fail("IMPORT_CUSTOMER_TYPE_REVIEW_REQUIRED");
  const corrections = decision.corrections || {}; C.exact(corrections, Object.values(CUSTOMER_MAP).filter(key => key !== "customerNumber"));
  ...
  Object.assign(input, corrections, { customerType: decision.customerType, companyName: decision.companyName || "", customFields: [] });
  let normalized;
...
function createImportMasterService({ access, protection, getActor, authorize, clock = () => new Date().toISOString() }) {
  ...
  async function planCustomer(tx, who, input) {
    ...
    if (!decision) C.fail("IMPORT_CUSTOMER_TYPE_REVIEW_REQUIRED");
    ...
    if (beforeBinding) {
      ...
      if (before.customerNumber !== proposed.customerNumber) C.fail("IMPORT_CUSTOMER_NUMBER_MISMATCH", 409);
      action = "link"; // First binding never overwrites an existing CRM card.
    } else if (await tx.queryOne(S.crmFind, { number: proposed.customerNumber })) C.fail("IMPORT_CUSTOMER_MATCH_CONFIRMATION_REQUIRED", 409);
    await allowed(who, "customer.sync", ["customer_restricted"], { targetKind: "crm_customer", targetId: before?.id || null });
    const after = before ? customerData(before) : { ...Object.fromEntries(CUSTOMER_FIELDS.map(key => [key, key === "birthDate" ? null : ""])), ...proposed };
