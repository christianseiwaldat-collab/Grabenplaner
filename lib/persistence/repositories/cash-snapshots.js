...
function createCashSnapshotStore({ access, protection, actor, check = async () => {}, sourceTolerances = [], clock = () => new Date().toISOString() }) {
  ...
  async function transaction(action, work) {
    ...
    await check(action);
    const result = await access.transaction(work, { isolation: 'serializable' });
  ...
  return Object.freeze({
    ...
    async reverify(id) {
      return transaction('prepare', async tx => {
        ...
        for (const state of dataset.data.tables) { state.verified = false; state.verifiedRows = 0; state.verifiedHash = EMPTY; }
