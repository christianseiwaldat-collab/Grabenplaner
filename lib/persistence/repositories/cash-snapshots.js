...
function createCashSnapshotStore({ access, protection, actor, check = async () => {}, sourceTolerances = [], clock = () => new Date().toISOString() }) {
  ...
  const packets = require('./import-batch-support').supportsImportBatches(access);
  const identity = (table, key) => Buffer.from(protection.digest([FORMAT, 'source-key', actor.scopeId, table.name, key]), 'hex');
  function normalize(table, row, dataset, ordinal, immutable = false) {
    const result = C.normalizeDataImportRow(table.profile, row, immutable);
    if (!table.keys) {
  ...
  function decode(dataset, table, row) {
    ...
    // Stored values already use the canonical scalar representation produced by
    // the importer. Raw Access conversions would copy and rewalk every field.
    // Reapply the full profile validation directly, including synthetic keys.
    if (!table.keys) { source._source_snapshot_sha256 = C.sha(dataset.data.fileSha256); source._source_row = C.integer(row.sourceRow, 1); }
    const normalized = normalize(table, source, dataset.data, row.sourceRow, true);
    same(hex(row.sourceKey), hex(identity(table, normalized.key)));
    // Both sides are validated scalars, so exact value equality is equivalent
    // to the previous canonical-array comparison (including -0 versus 0).
    if (values.some((value, index) => value !== normalized.source[table.columns[index].name])) C.fail('IMPORT_SOURCE_INTEGRITY');
  ...
  return Object.freeze({
    ...
    async append(id, name, startRow, rows) {
      ...
      return transaction('prepare', async tx => {
        ...
        const prepared = rows.map((row, i) => normalize(table, row, dataset.data, startRow + i));
        const parents = new Map(), pending = [];
        if (packets) {
          const requests = new Map();
          for (let i = 0; i < prepared.length; i++) {
            if (startRow + i <= state.receivedRows) continue;
            const request = H.historyParentRequest('cash', name, prepared[i].data);
            if (!request) continue;
            const parent = tableFor(request.name), key = hex(identity(parent, request.key));
            if (!requests.has(parent)) requests.set(parent, new Set());
            requests.get(parent).add(key);
          }
          for (const [parent, keys] of requests) {
            const records = await tx.queryAll(B.cashKeys[TABLES.indexOf(parent)], { datasetSlot, keys: [...keys] });
            for (const record of records) {
              decode(dataset, parent, record);
              parents.set(parent.name + ':' + hex(record.sourceKey), record);
            }
          }
        }
        let changed = false;
        for (let i = 0; i < rows.length; i++) {
          const sourceRow = startRow + i, normalized = prepared[i];
          ...
          if (request) {
            const parent = tableFor(request.name), sourceKey = identity(parent, request.key);
            const record = packets ? parents.get(parent.name + ':' + hex(sourceKey))
              : await tx.queryOne(parent.statements.key, { datasetSlot, sourceKey });
            if (!record) C.fail('IMPORT_HISTORY_PARENT_MISSING', 409);
            if (!packets) decode(dataset, parent, record);
            row.parentRow = record.sourceRow;
          }
          ...
          const stored = { ...row, payload: protection.seal(values, rowContext(dataset.row, table, row)) };
          if (packets) pending.push(Object.fromEntries(Object.entries(stored).map(([key, value]) => [key, Buffer.isBuffer(value) ? hex(value) : value])));
          else await one(tx, table.statements.insert, stored);
          state.sourceHash = chain(state.sourceHash, sourceRow, values); state.receivedRows++; changed = true;
        }
        if (pending.length && (await tx.execute(B.cashInsert[TABLES.indexOf(table)], { rows: pending })).rowsAffected !== pending.length) C.fail('IMPORT_CONCURRENT_CHANGE', 409);
    ...
    async contentDate(id,name,{after=0,uploadedAt}={}) {
      const table=tableFor(name),dates=require('../../data-import-content-date');
      C.integer(after,0,C.LIMITS.rows);C.utc(uploadedAt);
      return transaction('read',async tx=>{
        const dataset=await load(tx,id),state=dataset.data.tables.find(t=>t.name===name);
        const rows=await tx.queryAll(table.statements.page,{datasetSlot:dataset.row.slot,after,limit:REVIEW_ROWS});
        const result=dates.empty();
        for(const row of rows) dates.accumulate(result,table.profile,[decode(dataset,table,row).normalized.source],uploadedAt);
        return {...result,after:rows.at(-1)?.sourceRow??after,complete:(rows.at(-1)?.sourceRow??after)>=state.receivedRows};
      });
    },
