...
const { createReceiptSummaryStore, readReceiptSummary, writeReceiptSummary } = require('../../receipt-summary-store');
...
const BranchFilters = require('../../../public/branch-receipt-filters');
...
const BATCH = 100;
function createReceiptWorkspace({ access, protection, backend, getSession, getActor, today, now = () => Date.now(),
  resultStore = createReceiptResultStore(), summaryStore = createReceiptSummaryStore(), projectionFor = buildSalesHistoryProjection,
  queryFor = Model.query, sourceCustomer = null, articleResolverFactory = null }) {
  function state() {
    const actor = getActor(), session = getSession(), p = projectionFor(session);
  ...
  function service(tx, s) {
    return backend.service(tx, value => {
      ...
      if (value.action === 'history.scope') {
        if (s.p.receiptSourceIds) return s.p.receiptSourceIds.includes(BranchFilters.location(value.sourceLocationId));
  ...
  async function document(reader, id, s, tx, headFilter = null, summaryEpoch = null) {
    ...
    const row = { id, kind, date: head.provenance.businessDate, receipt: h.Bonnr ?? h.ZBon ?? h.Beleg ?? '', invoice: h.RechnungsNr ?? h.Rechnungsnr ?? '',
      locationId: s.p.receiptSourceIds ? BranchFilters.location(h.Filialid) : linked(loc) ? loc.targetId : 'unassigned',
      location: s.p.receiptSourceIds ? s.locations.find(l => l.id === BranchFilters.location(h.Filialid))?.label || BranchFilters.location(h.Filialid)
        : linked(loc) ? s.locations.find(l => l.id === loc.targetId)?.label || loc.targetId : 'Filialzuordnung offen',
    ...
    if (s.p.sellers) row.personnel = s.p.receiptSourceIds ? String(h['VerkäuferID'] ?? '') : personnel(refs.header_seller, h['VerkäuferID']);
    if (s.p.customerPurchases) {
      // Zero is an anonymous sale, never a shared customer identity. Only an
      // authenticated source binding may contribute CRM data; the branch
      // adapter can separately read the narrow encrypted source customer.
      ...
      if (assigned && !s.customers.has(number)) {
        ...
        s.customers.set(number, { customer, source: sourceCustomer ? await sourceCustomer(tx, number, !!customer) : null });
      }
      const { customer, source } = s.customers.get(number) || {};
      ...
      if (source && !customer) Object.assign(row, source);
      else if (source) {
        row.customerPhone = [...new Set([row.customerPhone, source.customerPhone].filter(Boolean))].join(' · ');
        row.customerSourceAccount ||= source.customerSourceAccount;
      }
    }
    if (headFilter && !headFilter(row)) return { ...row, filtered: true };
    const summaryOptions = summaryEpoch ? { store: summaryStore, protection, now,
      context: ['receipt-position-summary-v1', s.signature, summaryEpoch, id] } : null;
    const cached = summaryOptions && readReceiptSummary(summaryOptions);
    if (cached) return { ...row, ...cached, sourceAmount: h.RechnungsBetrag };
    ...
    for (const childId of children) {
      ...
      if ((s.p.receiptSourceIds ? BranchFilters.location(f.Filialid) : linked(ref) ? ref.targetId : 'unassigned') !== row.locationId) C.fail('IMPORT_HISTORY_INTEGRITY');
      const metric = metrics.get(f.RepID);
      const articleDisplay=articleResolverFactory?await articleResolverFactory(tx).resolve(f.EAN,row.date):null;
      row.lines.push({ article: f.EAN, description: f.Artikelbezeichnung, quantity: f.VKMenge, sourcePrice: f.VK_Preis,
        ...(articleDisplay?{articleDisplay,displayDescription:f.Artikelbezeichnung||articleDisplay.label||''}:{}),
        gross: metric?.gross ?? null, status: metric?.status ?? 'review',
        ...(s.p.sellers ? { personnel: s.p.receiptSourceIds ? String(f['Verkäuferid'] ?? '') : personnel(line.references.find(r => r.role === 'line_seller'), f['Verkäuferid']) } : {}) });
    }
    Object.assign(row, { positions: row.lines.length, description: row.lines.map(l => l.displayDescription||l.description).filter(Boolean).slice(0, 3).join(' · '),
    ...
    if (summaryOptions) {
      const value = { positions: row.positions, description: row.description, gross: row.gross, state: row.state, issues: row.issues,
        lineSearch: row.lines.flatMap(l => [l.article ?? '', l.description ?? '',l.articleDisplay?.label||'']),
        linePersonnel: s.p.sellers ? row.lines.map(l => l.personnel ?? '') : [] };
      writeReceiptSummary({ ...summaryOptions, value });
      return { ...row, ...value, lines: [] };
    }
    return row;
  }
  async function scan(input) {
    const s = state(), q = queryFor(input, { today: today(), projection: s.p });
    ...
    const result = await access.transaction(async tx => {
      ...
      for (const locationId of q.sourceLocationIds || (q.locationId ? [q.locationId] : locations)) {
      ...
      if(q.kind==='receipts'&&reader.prefetch){
        const ids=candidates.filter(row=>row.receiptScopeAllowed!==false).slice(0,Math.min(20,q.limit)).map(row=>row.id);
        const missing=ids.filter(id=>!readReceiptSummary({store:summaryStore,protection,now,context:['receipt-position-summary-v1',s.signature,epoch,id]}));
        await reader.prefetch(ids,missing);
      }
      const matchesHead = row => Search.matches([row.receipt, row.invoice], q.receipt)
        ...
        && (!q.sellerIds?.length || q.sellerIds.includes(BranchFilters.seller(row.personnel)))
      ...
      for (const candidate of candidates.slice(0, BATCH)) {
        if (candidate.receiptScopeAllowed === false) { processed++; last = candidate; continue; }
        const row = await document(reader, candidate.id, s, tx, matchesHead, epoch); processed++; processedLines += row.positions || 0; last = candidate;
        ...
        const sellerValues = q.sellerRole === 'line_seller' ? row.linePersonnel || [] : [row.personnel];
        if (Search.matches([row.receipt, row.invoice], q.receipt) && Search.matches(sellerValues, q.seller)
          && Search.matches(Model.CUSTOMER_COLUMNS.map(k => row[k]), q.customer)
          && Search.matches([row.receipt, row.invoice, row.description, row.account, ...(row.lineSearch || [])], q.query)) {
          const { lines, lineSearch, linePersonnel, ...summary } = row; items.push(summary);
        }
  ...
  async function search(input) {
    ...
    const epoch = await access.transaction(async tx => protection.digest(await backend.epoch(tx)), { isolation: 'serializable', readOnly: true });
