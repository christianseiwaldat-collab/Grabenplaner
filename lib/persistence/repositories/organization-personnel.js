...
function executorMethods(access) {
  return Object.freeze({
    ...
    getScheduleManualLock(locationId, weekStart) {
      return access.queryOne(S.getScheduleManualLock, {
        locationId: text(locationId),
        weekStart: text(weekStart),
      });
    },
    ...
    listPermissionDefaults() { return access.queryAll(S.listPermissionDefaults, {}); },
    listPositionPermissionDefaults() { return access.queryAll(S.listPositionPermissionDefaults, {}); },
    updateRolePermissionDefaults(value) { return access.execute(S.updateRolePermissionDefaults, value); },
    updatePositionPermissionDefaults(value) { return access.execute(S.updatePositionPermissionDefaults, value); },
    listPortalRoles() {
    ...
    getPortalRoleProjection(id, positionId = "") {
      return access.queryOne(S.getPortalRoleProjection, { id: text(id), positionId: text(positionId) });
    },
    ...
    async upsertScheduleManualLock(value) {
      const prepared = record(value, "upsertScheduleManualLock");
      const result = await access.execute(S.upsertScheduleManualLock, {
        locationId: text(prepared.locationId),
        weekStart: text(prepared.weekStart),
        locked: boolean(prepared.locked),
        expectedRevision: safeInteger(
          prepared.expectedRevision,
          "upsertScheduleManualLock",
        ),
        actor: text(prepared.actor),
      });
      return result.returnedRows[0] || null;
    },
    ...
    insertPosition(position, sortOrder, actor) {
    ...
    updatePosition(id, name, actor) {
      return access.execute(S.updatePosition, { id: text(id), name: text(name), actor: text(actor) });
    },
    countPositionTypeAssignments(id) {
      return access.queryOne(S.countPositionTypeAssignments, { id: text(id) })
        .then((row) => row.count);
    },
    deletePositionTypeAssignments(id) {
      return access.execute(S.deletePositionTypeAssignments, { id: text(id) });
    },
    archivePosition(id, actor) {
      return access.execute(S.archivePosition, { id: text(id), actor: text(actor) });
    },
    listEmployees() {
    ...
    async applyAccessProfile(actor, profile, before = null) {
      ...
      // A personnel/profile edit is not a decision to restore personally denied
      // rights. Only the explicit central rights operation changes these denials.
