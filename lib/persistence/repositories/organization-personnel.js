...
function executorMethods(access) {
  return Object.freeze({
    ...
    deleteAccessScopes(employeeNumber) {
      return access.execute(S.deleteAccessScopes, { employeeNumber: text(employeeNumber) });
    },
    ...
    updateOrganizationAccountPassword(accountId, passwordHash) {
    ...
    async applyAccessProfile(actor, profile, before = null) {
      ...
      await this.deletePermissionGrants(employeeNumber);
      await this.deletePermissionDenials(employeeNumber);
      ...
      await this.deleteAccessScopes(employeeNumber);
      if (["location_planner", "manager", "department_manager"].includes(profile.role)) {
        await this.insertAccessScope({
          ...
          departmentId: profile.role === "department_manager" ? profile.preferredDepartmentId : 0,
