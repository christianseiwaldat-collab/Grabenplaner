...
function createSalesReportJobs({ access, vault, runtime, resolvePrincipal, batchWorker = null, scope = 'grabenplaner-main', now = Date.now, onError = () => {} }) {
  ...
  const elapsedSeconds = (startedAt, completedAt) => {
    const started = Date.parse(String(startedAt || '')), completed = Date.parse(String(completedAt || ''));
  ...
  const view = (row, data) => ({ id: row.id, status: row.status, created: row.created, updated: row.updated,
    ...
    durationSeconds: row.status === 'completed'
      ? elapsedSeconds(data.startedAt || row.created, data.completedAt || row.updated)
      : null });
    ...
    await protectedWork(async p => {
      ...
      try {
        ...
        if (complete) {
          ...
          data.artifact = data.format === 'pdf'
            ? (result.artifact || (await createSalesAnalysisPdf({ ...data, report: result.report, completedAt })).toString('base64'))
            : renderSalesReportDocument({ ...data, result, completedAt });
        }
