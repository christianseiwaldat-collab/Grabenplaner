...
function createSalesReportJobs({ access, vault, runtime, resolvePrincipal, scope = 'grabenplaner-main', now = Date.now, onError = () => {} }) {
  ...
  const matchingAuthority = (p, session, data) => p.digest(data.query.reportVersion === 2 ? authority(session)
    ...
    await protectedWork(async p => {
      ...
      try {
        ...
        const result = await runtime.run(getSession, async workspace => {
          const revision = data.query.reportVersion === 2 ? workspace?.reportSourceRevision : workspace?.legacyReportSourceRevision;
          if (!workspace || revision !== data.sourceRevision) C.fail('IMPORT_HISTORY_ANALYSIS_CHANGED', 409);
          const cursor = cursors.get(row.id);
          if (data.query.reportVersion === 2) return workspace.reports.step(data.query, data.metadata, cursor);
        ...
        if (complete) data.artifact = data.format === 'pdf'
          ? (await createSalesAnalysisPdf({ ...data, report: result.report, completedAt: stamp() })).toString('base64')
        ...
        await access.transaction(tx => save(tx, p, row, { ...data, artifact: null, error: ['IMPORT_FORBIDDEN', 'IMPORT_HISTORY_ANALYSIS_CHANGED', 'IMPORT_REPORT_GROUP_LIMIT', 'IMPORT_REPORT_PDF_LIMIT'].includes(error?.code) ? error.code : 'IMPORT_REPORT_FAILED' }, 'failed'));
      }
    });
  }
  function tick() {
