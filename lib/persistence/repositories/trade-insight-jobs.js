...
function createTradeInsightJobs({access,vault,runtime,resolvePrincipal,dispatchRead,scopeId='grabenplaner-main',now=Date.now,onError=()=>{}}){
 ...
 function validateKind(session,kind){
  ...
  const p=authority(session).projection,key=['inventory','stock-summary'].includes(kind)?'inventory':kind==='repairs'?'repairs':['customer-history','device-history'].includes(kind)?'customers':'purchasing';
 ...
 async function create(session,input){
  ...
  C.exact(input.query,['query','locationId','supplier','dateFrom','dateTo','days','group','wgr','customer','serial']);
 ...
 async function step(){
  ...
  await protectedWork(async p=>{
   ...
   try{
    ...
    const sourceDates=[...new Set([...(data.result?.sourceDates||[]),snapshot.sourceDate].filter(Boolean))];
