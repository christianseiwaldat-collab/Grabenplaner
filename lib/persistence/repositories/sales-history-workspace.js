...
function createSalesHistoryWorkspace({ access, protection, sources, getSession, getActor, getPolicy = () => null,
  ...
  analysisStore = createSalesHistoryAnalysisStore(), backend = null }) {
        ...
        for (const record of records) {
          ...
          if (advancing) {
            ...
            if (query.kind === 'sales') {
              ...
              if (row.metric) {
                counts.checked++;
                const countKey = row.metric.status === 'sale' ? 'sales' : row.metric.status === 'return' ? 'returns' : row.metric.status === 'adjustment' ? 'adjustments' : row.metric.status === 'payment' ? 'payments' : row.metric.status === 'voucher_issue' ? 'voucherIssues' : row.metric.status === 'uid_clearing' ? 'uidClearings' : 'excluded';
