...
function createSalesHistoryWorkspace({ access, protection, sources, getSession, getActor, getPolicy = () => null,
  ...
  async function execute(input, { customerId = null } = {}, analysisCursor = null) {
    ...
    try {
        ...
        for (const record of records) {
          ...
          if (advancing) {
            ...
            if (query.kind === 'sales') {
              ...
              if (row.metric) {
                counts.checked++; counts[row.metric.status === 'sale' ? 'sales' : row.metric.status === 'return' ? 'returns' : 'excluded']++;
      ...
      analysisStore.entries.set(analysisId, { ownerKey, expires: work.expires, payload: protection.seal(work.aggregate, aggregateContext(analysisId)) });
      return work.result;
