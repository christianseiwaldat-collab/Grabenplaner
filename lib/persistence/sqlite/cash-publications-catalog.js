...
const SQLITE_CASH_PUBLICATIONS_CATALOG = Object.freeze([
  ...
  ...Object.entries(S.search).map(([name, statement]) => {
    ...
    return entry(statement, `SELECT ${select(CASH_SNAPSHOT_COLUMNS.ROW, 'r.')} FROM ${t.sqlName} r
      ...
      AND (($unassigned=1 AND l.target_id IS NULL) OR ($unassigned=0 AND l.target_id=$locationId))
