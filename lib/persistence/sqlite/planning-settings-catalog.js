...
const EMPLOYEE_FIELDS = Object.freeze({
  ...
  position_employment_classification: "position.employment_classification",
});
...
const SQLITE_PLANNING_SETTINGS_CATALOG = Object.freeze([
  ...
  entry(S.listOverlappingWeekOptions, `
    ...
    WHERE employee_number = json_extract($payload, '$.employeeNumber')
      ...
      AND (
        json_extract($payload, '$.excludedGroupId') IS NULL
        OR group_id IS NULL
        OR group_id <> json_extract($payload, '$.excludedGroupId')
      )
  `),
