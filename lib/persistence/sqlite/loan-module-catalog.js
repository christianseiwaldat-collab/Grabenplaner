...
const ARTICLE_FIELDS = Object.freeze({
  ...
  source_product_number: "article.source_product_number",
  source_url: "article.source_url",
  source_fetched_at: "article.source_fetched_at",
...
const SQLITE_LOAN_MODULE_CATALOG = Object.freeze([
  ...
  entry(S.upsertLocationSetting, `
    INSERT INTO loan_location_settings
      (location_id, enabled, article_lookup_enabled, article_lookup_provider,
       ...
       photo_original_retention, created_by, updated_by, updated_at)
    VALUES (
  ...
  entry(S.getArticleByIdentifier, `
    SELECT ${jsonObject(ARTICLE_FIELDS)}
    FROM article_identifiers matched
    JOIN articles article ON article.article_number = matched.article_number
    WHERE matched.identifier_type = json_extract($payload, '$.identifierType')
      AND matched.identifier_value = json_extract($payload, '$.identifierValue')
  `),
  entry(S.searchArticles, `
    ...
    WHERE article.active = 1
      AND (
        ...
        OR article.description LIKE json_extract($payload, '$.descriptionPattern') COLLATE NOCASE
        OR EXISTS (
          SELECT 1
          FROM article_identifiers identifier
          WHERE identifier.article_number = article.article_number
    ...
    ORDER BY CASE
      ...
      ELSE 1
    END, article.description, article.article_number
    LIMIT 20
  `),
  entry(S.upsertArticle, `
    ...
    VALUES (
      ...
      json_extract($payload, '$.sourceProvider'),
      json_extract($payload, '$.sourceProductNumber'),
      json_extract($payload, '$.sourceUrl'),
      json_extract($payload, '$.sourceFetchedAt'),
      1,
      json_extract($payload, '$.actorEmployeeNumber'),
      json_extract($payload, '$.actorEmployeeNumber'),
      CURRENT_TIMESTAMP
    ...
    ON CONFLICT(article_number) DO UPDATE SET
      ...
      updated_at = CURRENT_TIMESTAMP
  `),
  entry(S.upsertArticleIdentifier, `
    ...
    VALUES (
      json_extract($payload, '$.articleNumber'),
      json_extract($payload, '$.identifierType'),
      json_extract($payload, '$.identifierValue'),
      json_extract($payload, '$.sourceProvider'),
      CURRENT_TIMESTAMP,
      json_extract($payload, '$.actorEmployeeNumber'),
      json_extract($payload, '$.actorEmployeeNumber'),
  ...
  entry(S.getActiveArticle, `
    ...
    FROM articles
    WHERE article_number = json_extract($payload, '$.articleNumber')
  `),
