...
const SQLITE_ORGANIZATION_PERSONNEL_CATALOG = Object.freeze([
  ...
  `),
  entry(S.getScheduleManualLock, `
    SELECT location_id, week_start, locked, revision, created_by, updated_by,
           created_at, updated_at
    FROM schedule_manual_locks
    WHERE location_id = $locationId
      AND week_start = $weekStart
  `),
  ...
  entry(S.listCostCenterTypePositions, `
    SELECT cctp.cost_center_type_id, p.id, p.name, p.builtin, p.active, p.is_default,
           p.employment_classification, p.sort_order, p.revision, p.created_by, p.updated_by,
           p.created_at, p.updated_at, p.archived_by, p.archived_at
  ...
  entry(S.listPositions, `
    SELECT p.id, p.name, p.builtin, p.active, p.is_default, p.employment_classification,
           p.sort_order, p.revision, p.created_by, p.updated_by, p.created_at, p.updated_at,
           p.archived_by, p.archived_at,
           (SELECT COUNT(*) FROM employees e WHERE e.position_id = p.id) AS employee_count,
           (SELECT COUNT(*) FROM employees e WHERE e.position_id = p.id AND e.active = 1) AS active_employee_count,
    ...
    GROUP BY p.id, p.name, p.builtin, p.active, p.is_default, p.employment_classification,
             p.sort_order, p.revision, p.created_by, p.updated_by, p.created_at, p.updated_at,
             p.archived_by, p.archived_at
  ...
  entry(S.listPortalPermissionDenials, `
    ...
    ORDER BY permission
  `),
  entry(S.getPersonnelLearningPermissionDenialAuthority, `
    SELECT employee_number, permission, authority_level, scope_location_id,
           denied_by, generation_id, created_at, updated_at, revision
  ...
  `),
  entry(S.listPermissionDefaults, `
    SELECT 'role' AS kind, r.id, r.name, r.permissions, r.permissions_revision AS revision,
      r.permissions_customized AS customized,
      (SELECT COUNT(*) FROM portal_users u JOIN employees e ON e.personnel_number=u.employee_number
        WHERE u.role=r.id AND u.active=1 AND e.active=1 AND NOT EXISTS (
          SELECT 1 FROM portal_position_permission_defaults p WHERE p.position_id=e.position_id AND p.permissions IS NOT NULL)) AS affected
    FROM portal_roles r
    UNION ALL
    SELECT 'position', p.id, p.name, d.permissions, COALESCE(d.revision,0), d.permissions IS NOT NULL,
      (SELECT COUNT(*) FROM portal_users u JOIN employees e ON e.personnel_number=u.employee_number
        WHERE e.position_id=p.id AND u.role<>'developer' AND u.active=1 AND e.active=1)
    FROM positions p LEFT JOIN portal_position_permission_defaults d ON d.position_id=p.id
    ORDER BY kind, name, id
  `),
  entry(S.updateRolePermissionDefaults, `
    UPDATE portal_roles SET permissions=$permissions, permissions_customized=$customized,
      permissions_revision=permissions_revision+1, updated_at=CURRENT_TIMESTAMP
    WHERE id=$id AND id<>'developer' AND permissions_revision=$revision
  `),
  entry(S.listPositionPermissionDefaults, `
    SELECT position_id AS id, permissions FROM portal_position_permission_defaults
    WHERE permissions IS NOT NULL ORDER BY position_id
  `),
  entry(S.updatePositionPermissionDefaults, `
    INSERT INTO portal_position_permission_defaults (position_id,permissions,revision,updated_by)
    VALUES ($id,$permissions,$revision+1,$actor)
    ON CONFLICT(position_id) DO UPDATE SET permissions=excluded.permissions,
      revision=excluded.revision, updated_by=excluded.updated_by, updated_at=CURRENT_TIMESTAMP
    WHERE portal_position_permission_defaults.revision=$revision
  `),
  ...
  entry(S.getPortalRoleProjection, `
    SELECT r.id, r.name, CASE WHEN r.id = 'developer' THEN r.permissions
      ELSE COALESCE(p.permissions, r.permissions) END AS permissions
    FROM portal_roles r
    LEFT JOIN portal_position_permission_defaults p ON p.position_id = $positionId
    WHERE r.id = $id
  `),
  ...
  entry(S.deleteScheduleNote, `
    ...
    WHERE location_id = $locationId
      ...
      AND week_start = $weekStart
  `),
  entry(S.upsertScheduleManualLock, `
    INSERT INTO schedule_manual_locks
      (location_id, week_start, locked, revision, created_by, updated_by,
       created_at, updated_at)
    SELECT $locationId, $weekStart, $locked, 1, $actor, $actor,
           CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
    WHERE $expectedRevision = 0
       OR EXISTS (
         SELECT 1 FROM schedule_manual_locks
         WHERE location_id = $locationId AND week_start = $weekStart
       )
    ON CONFLICT(location_id, week_start)
    DO UPDATE SET
      locked = excluded.locked,
      revision = schedule_manual_locks.revision + 1,
      updated_by = excluded.updated_by,
      updated_at = CURRENT_TIMESTAMP
    WHERE schedule_manual_locks.revision = $expectedRevision
    RETURNING location_id, week_start, locked, revision, created_by, updated_by,
              created_at, updated_at
  `, true),
  ...
  `),
  entry(S.countDepartmentLearningModuleVersionScopes, `
    SELECT COUNT(*) AS count
    FROM personnel_learning_module_versions
    WHERE scope_department_id = $id
  `),
  ...
  entry(S.listPersonnelDirectory, `
    SELECT
      ...
      p.employment_classification AS position_employment_classification,
  ...
  entry(S.insertPosition, `
    INSERT INTO positions
      (id, name, builtin, active, is_default, employment_classification, sort_order,
       revision, created_by, updated_by, updated_at)
    VALUES (
      $id, $name, 0, 1,
      CASE WHEN EXISTS (SELECT 1 FROM positions WHERE active = 1 AND is_default = 1) THEN 0 ELSE 1 END,
      'standard', $sortOrder, 1, $actor, $actor, CURRENT_TIMESTAMP
    )
  `),
  entry(S.getPosition, `
    SELECT p.id, p.name, p.builtin, p.active, p.is_default, p.employment_classification,
  ...
  entry(S.updatePosition, `
    UPDATE positions
    SET name = $name, builtin = 0, revision = revision + 1,
  ...
  `),
  entry(S.deletePositionTypeAssignments, `
    DELETE FROM cost_center_type_positions
    WHERE position_id = $id
  `),
  entry(S.archivePosition, `
    UPDATE positions
    SET active = 0, builtin = 0, is_default = 0, revision = revision + 1,
        updated_by = $actor, updated_at = CURRENT_TIMESTAMP,
        archived_by = $actor, archived_at = CURRENT_TIMESTAMP
  ...
  entry(S.deletePermissionDenial, `
    ...
    WHERE employee_number = $employeeNumber
      AND permission = $permission
  `),
  entry(S.upsertPersonnelLearningPermissionDenialAuthority, `
    ...
    VALUES (
      $employeeNumber, $permission, $authorityLevel, $scopeLocationId,
      $actor, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1
    )
    ON CONFLICT(employee_number, permission) DO UPDATE SET
      ...
      updated_at = CURRENT_TIMESTAMP,
      revision = personnel_learning_permission_denial_authorities.revision + 1
  `),
  entry(S.deletePersonnelLearningPermissionDenialAuthority, `
    DELETE FROM personnel_learning_permission_denial_authorities
    WHERE employee_number = $employeeNumber
      AND permission = $permission
  `),
