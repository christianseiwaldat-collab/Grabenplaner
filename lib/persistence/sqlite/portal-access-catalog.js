...
const EMPLOYEE_SESSION_FIELDS = Object.freeze({
  ...
  access_scope_assignment_count: `(
    ...
    WHERE assigned_scope.employee_number = s.employee_number
  )`,
...
const SQLITE_PORTAL_ACCESS_CATALOG = Object.freeze([
  ...
  entry(S.getEmployeeSessionByToken, `
    ...
    LIMIT 1
  `),
  entry(S.getReportPrincipal, `
    SELECT ${jsonObject(Object.fromEntries(Object.entries(EMPLOYEE_SESSION_FIELDS)
      .filter(([key]) => !['id', 'expires_at', 'revoked_at'].includes(key))
      .map(([key, expression]) => [key, expression.replaceAll('s.employee_number', 'u.employee_number')])))}
    FROM portal_users u
    JOIN employees e ON e.personnel_number=u.employee_number
    LEFT JOIN positions p ON p.id=e.position_id
    LEFT JOIN portal_user_roles r ON r.employee_number=u.employee_number
    LEFT JOIN amu_local_access_overrides amu ON amu.employee_number=u.employee_number
    WHERE u.employee_number=json_extract($payload, '$.employeeNumber') AND u.active=1 AND e.active=1
      AND TRIM(u.password_hash)<>''
    LIMIT 1
  `),
  entry(S.touchEmployeeSession, `
    ...
    SET last_seen_at = CURRENT_TIMESTAMP,
        expires_at = json_extract($payload, '$.expiresAt')
    WHERE id = json_extract($payload, '$.id')
  `),
  ...
  entry(S.revokeEmployeeSessionById, `
    ...
    WHERE id = json_extract($payload, '$.id')
  `),
  entry(S.listPasswordResetCandidates, `
    SELECT ${jsonObject({
      employee_number: "u.employee_number",
      email_target_fingerprint: "contact.email_target_fingerprint",
    })}
    FROM portal_users u
    JOIN employees e ON e.personnel_number = u.employee_number
    JOIN personal_notification_contacts contact ON contact.employee_number = u.employee_number
    WHERE u.active = 1
      AND e.active = 1
      AND TRIM(u.password_hash) <> ''
      AND contact.email_verified_at IS NOT NULL
      AND TRIM(contact.email_target_fingerprint) <> ''
      AND json_extract($payload, '$') IS NOT NULL
    ORDER BY u.employee_number
  `),
  entry(S.countRecentPasswordResetTokens, `
    SELECT json_object('count', COUNT(*)) AS data
    FROM portal_password_reset_tokens
    WHERE employee_number = json_extract($payload, '$.employeeNumber')
      AND julianday(created_at) >= julianday(json_extract($payload, '$.since'))
  `),
  entry(S.purgePasswordResetTokens, `
    DELETE FROM portal_password_reset_tokens
    WHERE julianday(expires_at) < julianday(json_extract($payload, '$.cutoff'))
      AND (used_at IS NOT NULL OR revoked_at IS NOT NULL OR expires_at < CURRENT_TIMESTAMP)
  `),
  entry(S.revokePasswordResetTokensForEmployee, `
    UPDATE portal_password_reset_tokens
    SET revoked_at = COALESCE(revoked_at, CURRENT_TIMESTAMP)
    WHERE employee_number = json_extract($payload, '$.employeeNumber')
      AND used_at IS NULL
      AND revoked_at IS NULL
      AND (json_extract($payload, '$.exceptId') IS NULL
        OR id <> json_extract($payload, '$.exceptId'))
  `),
  entry(S.insertPasswordResetToken, `
    INSERT INTO portal_password_reset_tokens
      (id, employee_number, token_hash, email_fingerprint, expires_at)
    SELECT
      json_extract($payload, '$.id'),
      json_extract($payload, '$.employeeNumber'),
      json_extract($payload, '$.tokenHash'),
      json_extract($payload, '$.emailFingerprint'),
      json_extract($payload, '$.expiresAt')
    FROM portal_users u
    JOIN employees e ON e.personnel_number = u.employee_number
    JOIN personal_notification_contacts contact ON contact.employee_number = u.employee_number
    WHERE u.employee_number = json_extract($payload, '$.employeeNumber')
      ...
      AND TRIM(u.password_hash) <> ''
      AND contact.email_verified_at IS NOT NULL
      AND contact.email_target_fingerprint = json_extract($payload, '$.emailFingerprint')
  `),
  entry(S.revokePasswordResetTokenById, `
    UPDATE portal_password_reset_tokens
    SET revoked_at = COALESCE(revoked_at, CURRENT_TIMESTAMP)
    WHERE id = json_extract($payload, '$.id')
      AND used_at IS NULL
  `),
  entry(S.getPasswordResetTokenByHash, `
    SELECT ${jsonObject({
      id: "reset.id",
      employee_number: "reset.employee_number",
      email_fingerprint: "reset.email_fingerprint",
      expires_at: "reset.expires_at",
    })}
    FROM portal_password_reset_tokens reset
    JOIN portal_users u ON u.employee_number = reset.employee_number
    JOIN employees e ON e.personnel_number = reset.employee_number
    JOIN personal_notification_contacts contact ON contact.employee_number = reset.employee_number
    WHERE reset.token_hash = json_extract($payload, '$.tokenHash')
      AND reset.used_at IS NULL
      AND reset.revoked_at IS NULL
      AND reset.expires_at > json_extract($payload, '$.now')
      AND u.active = 1
      AND e.active = 1
      AND TRIM(u.password_hash) <> ''
      AND contact.email_verified_at IS NOT NULL
      AND contact.email_target_fingerprint = reset.email_fingerprint
    LIMIT 1
  `),
  entry(S.consumePasswordResetToken, `
    UPDATE portal_password_reset_tokens
    SET used_at = json_extract($payload, '$.usedAt')
    WHERE id = json_extract($payload, '$.id')
      AND token_hash = json_extract($payload, '$.tokenHash')
      AND used_at IS NULL
      AND revoked_at IS NULL
      AND expires_at > json_extract($payload, '$.usedAt')
  `),
  entry(S.updatePasswordFromReset, `
    UPDATE portal_users
    SET password_hash = json_extract($payload, '$.passwordHash'),
        must_change_password = 0,
        password_changed_at = json_extract($payload, '$.changedAt'),
        failed_login_attempts = 0,
        locked_until = NULL,
        updated_at = json_extract($payload, '$.changedAt')
    WHERE employee_number = json_extract($payload, '$.employeeNumber')
      AND active = 1
  `),
  entry(S.revokeMobileSessionsForPasswordReset, `
