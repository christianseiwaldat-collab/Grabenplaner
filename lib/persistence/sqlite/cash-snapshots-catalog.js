...
const SQLITE_CASH_SNAPSHOTS_CATALOG = Object.freeze([
  ...
  ...TABLES.flatMap(t => [
    ...
    entry(t.statements.children, `SELECT ${select(C.ROW)} FROM ${t.sqlName} WHERE dataset_slot=$datasetSlot AND parent_row=$parentRow ORDER BY source_row LIMIT $limit`),
  ]),
]);
