...
const REQUEST_ROWS = Object.freeze({
  ...
  timeOff: Object.freeze([
    ...
    ["review_department_id", "r.review_department_id"],
...
const SQLITE_ABSENCE_MANAGEMENT_CATALOG = Object.freeze([
  ...
  entry(ABSENCE_MANAGEMENT_STATEMENTS.insertTimeOffRequest, `
    INSERT INTO time_off_requests
      (employee_number, location_id, origin_location_id, review_department_id,
       lending_id, request_date, date_from, date_to, all_day,
       start_time, end_time, note, approval_type, approval_stage, status,
       traffic_light, check_reason)
    VALUES (
      json_extract($data, '$.employeeNumber'),
      json_extract($data, '$.locationId'),
      json_extract($data, '$.originLocationId'),
      json_extract($data, '$.reviewDepartmentId'),
      json_extract($data, '$.lendingId'),
  ...
  entry(ABSENCE_MANAGEMENT_STATEMENTS.insertTimeOffChange, `
    INSERT INTO time_off_change_requests
      (employee_number, location_id, origin_location_id, review_department_id,
       lending_id, original_request_id, request_type,
  ...
  entry(ABSENCE_MANAGEMENT_STATEMENTS.updateTimeOffRequest, `
    ...
    SET location_id = json_extract($data, '$.locationId'),
        origin_location_id = json_extract($data, '$.originLocationId'),
        review_department_id = json_extract($data, '$.reviewDepartmentId'),
        lending_id = json_extract($data, '$.lendingId'),
        request_date = json_extract($data, '$.requestDate'),
        date_from = json_extract($data, '$.dateFrom'),
        date_to = json_extract($data, '$.dateTo'),
  ...
  entry(ABSENCE_MANAGEMENT_STATEMENTS.employeeReviewScope, `
    ...
    LIMIT 1
  `),
  entry(ABSENCE_MANAGEMENT_STATEMENTS.activeEmployeeLendingsForRange, `
    SELECT json_object(
      'id', lending.id,
    ...
    WHERE lending.employee_number = json_extract($data, '$.employeeNumber')
      AND lending.status = 'active'
      AND lending.date_from <= json_extract($data, '$.dateTo')
      AND lending.date_to >= json_extract($data, '$.dateFrom')
    ORDER BY lending.date_from, lending.start_time, lending.id
  `),
  entry(ABSENCE_MANAGEMENT_STATEMENTS.locationTimeOffConfiguration, `
    SELECT json_object(
      'id', location.id,
      'day_settings_json', location.day_settings_json,
      'min_staff', MAX(
        COALESCE(location.min_staff, 0),
        COALESCE(SUM(CASE WHEN department.active = 1 THEN department.min_staff ELSE 0 END), 0)
      )
    ) AS data
    FROM locations location
    LEFT JOIN departments department ON department.location_id = location.id
    WHERE location.id = json_extract($data, '$.locationId')
  ...
  entry(ABSENCE_MANAGEMENT_STATEMENTS.plannedShiftOnDate, `
    ...
    WHERE s.employee_number = json_extract($data, '$.employeeNumber')
      AND s.shift_date = json_extract($data, '$.date')
      AND (
        json_extract($data, '$.locationId') IS NULL
        OR s.location_id = json_extract($data, '$.locationId')
      )
    LIMIT 1
  `),
  ...
  entry(ABSENCE_MANAGEMENT_STATEMENTS.workRuleEvaluationEmployee, `
    SELECT json_object(
      ...
      'position_employment_classification', p.employment_classification,
      'home_location_id', e.home_location_id,
      'preferred_department_id', e.preferred_department_id
    ) AS data
    FROM employees e
  ...
  entry(ABSENCE_MANAGEMENT_STATEMENTS.replaceOriginalTimeOff, `
    ...
    SET location_id = json_extract($data, '$.locationId'),
        ...
        review_department_id = json_extract($data, '$.reviewDepartmentId'),
        lending_id = json_extract($data, '$.lendingId'),
        request_date = json_extract($data, '$.dateFrom'),
        date_from = json_extract($data, '$.dateFrom'),
        date_to = json_extract($data, '$.dateTo'),
        all_day = json_extract($data, '$.allDay'),
        start_time = json_extract($data, '$.startTime'),
