...
const ARTICLE_SEARCH_PROJECTION_SOURCE_SQL = `
  SELECT article.product_id AS productId, article.article_number AS articleNumber,
    ...
    ${PRICES.map(p => `(SELECT CASE WHEN COUNT(DISTINCT price.amount)=1 THEN MIN(price.amount) ELSE NULL END
      FROM sales_article_price_snapshots price WHERE price.product_id=article.product_id AND price.source_snapshot_id=revision.source_snapshot_id
        AND price.price_type='${p.type}' AND price.price_basis='${p.basis}' AND price.currency='EUR'
        AND price.quality_status IN ('confirmed','inferred')) AS ${p.id}`).join(',\n    ')}
...
const UPSERT_ARTICLE_SEARCH_PROJECTION_SQL = `
  INSERT INTO sales_article_search_projection
    (product_id, article_number, current_revision, description, active, source_snapshot_id,
     ...
     ${PRICES.flatMap(p => [p.column, p.column + '_sort']).join(',')})
  VALUES ($productId, $articleNumber, $currentRevision, $description, $active, $sourceSnapshotId,
    ...
    ${PRICES.flatMap(p => ['$' + p.id, '$' + p.id + 'Sort']).join(',')})
  ON CONFLICT(product_id) DO UPDATE SET
    ...
    ${PRICES.flatMap(p => [p.column, p.column + '_sort']).map(c => `${c}=excluded.${c}`).join(',')}
`;
