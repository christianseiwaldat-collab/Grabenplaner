...
const SEARCH_PREDICATE = `
  ...
  AND (
    customer.customer_number LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR customer.company_name LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR customer.first_name LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR customer.last_name LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR customer.street LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR customer.address_supplement LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR customer.postal_code LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR customer.city LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR customer.country LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR customer.phone LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR customer.email LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR customer.website LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR customer.vat_id LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
    OR EXISTS (
      SELECT 1 FROM crm_customer_custom_fields custom
      WHERE custom.customer_id = customer.id
        AND (
          custom.title LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
          OR custom.value LIKE $likeQuery ESCAPE '!' COLLATE NOCASE
        )
    )
  )
`;
...
const SQLITE_CRM_CUSTOMERS_CATALOG = Object.freeze([
  ...
  entry(S.insert, `
    INSERT INTO crm_customers (
      ...
      $id, $customerNumber, $customerType, $companyName, $firstName, $lastName,
