...
const { isPersistenceError, PERSISTENCE_MESSAGE_KEYS } = require("../errors");

const LEGACY_DATABASES = new WeakMap();
const SQLITE_REGISTERED_APPLICATION_FUNCTIONS = new WeakSet();
...
function persistenceError(code, operation, cause, messageKey = "") {
  return new PersistenceError(code, { operation, cause, messageKey });
}
...
function mapSqliteError(error, {
  ...
  if (code === PERSISTENCE_ERROR_CODES.CHECK_VIOLATION) {
    const sqliteMessage = String(error?.message || "");
    if (sqliteMessage.includes("STAFF_ASSIGNMENT_HOME_SHIFT_CONFLICT")) {
      messageKey = PERSISTENCE_MESSAGE_KEYS.STAFF_ASSIGNMENT_HOME_SHIFT_CONFLICT;
    } else if (sqliteMessage.includes("STAFF_ASSIGNMENT_COVERAGE_REQUIRED")) {
      messageKey = PERSISTENCE_MESSAGE_KEYS.STAFF_ASSIGNMENT_COVERAGE_REQUIRED;
    } else if (sqliteMessage.includes("STAFF_ASSIGNMENT_DIRECT_ABSENCE_CONFLICT")) {
      messageKey = PERSISTENCE_MESSAGE_KEYS.STAFF_ASSIGNMENT_DIRECT_ABSENCE_CONFLICT;
    }
  }
  return persistenceError(code, operation, error, messageKey);
}
...
function registerSqliteApplicationFunctions(database) {
  if (SQLITE_REGISTERED_APPLICATION_FUNCTIONS.has(database)) return;
  database.function(
    "gp_unicode_casefold",
    { deterministic: true, directOnly: true },
    (value) => (value === null ? null : String(value).toLocaleLowerCase("de-AT")),
  );
  SQLITE_REGISTERED_APPLICATION_FUNCTIONS.add(database);
}

function initializeSqliteConnection(database) {
  registerSqliteApplicationFunctions(database);
...
function openSqliteLegacyDatabase(databasePath, {
  ...
  try {
    ...
    else registerSqliteApplicationFunctions(database);
