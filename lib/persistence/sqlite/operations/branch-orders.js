...
function ensureSqliteBranchOrdersSchema(database) {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS branch_order_location_settings (
      ...
      catalog_content_version INTEGER NOT NULL DEFAULT 0
        CHECK (catalog_content_version >= 0)
    );
    ...
    CREATE TABLE IF NOT EXISTS branch_order_recipients (
      ...
      cc_email TEXT NOT NULL DEFAULT '' COLLATE NOCASE,
      primary_delivery_mode TEXT NOT NULL DEFAULT 'message'
        CHECK (primary_delivery_mode IN ('message', 'message_pdf', 'pdf_only')),
      cc_delivery_mode TEXT NOT NULL DEFAULT 'message'
        CHECK (cc_delivery_mode IN ('message', 'message_pdf', 'pdf_only')),
    ...
    CREATE TABLE IF NOT EXISTS branch_order_deliveries (
      ...
      recipient_role TEXT NOT NULL DEFAULT 'primary'
        CHECK (recipient_role IN ('primary', 'cc')),
      delivery_mode TEXT NOT NULL DEFAULT 'message'
        CHECK (delivery_mode IN ('message', 'message_pdf', 'pdf_only')),
  ...
  const recipientColumns = new Set(
    db.prepare("PRAGMA table_info(branch_order_recipients)").all()
      .map((column) => String(column.name)),
  );
  if (!recipientColumns.has("cc_email")) {
    db.exec("ALTER TABLE branch_order_recipients ADD COLUMN cc_email TEXT NOT NULL DEFAULT '' COLLATE NOCASE");
  }
  if (!recipientColumns.has("primary_delivery_mode")) {
    db.exec("ALTER TABLE branch_order_recipients ADD COLUMN primary_delivery_mode TEXT NOT NULL DEFAULT 'message' CHECK (primary_delivery_mode IN ('message', 'message_pdf', 'pdf_only'))");
  }
  if (!recipientColumns.has("cc_delivery_mode")) {
    db.exec("ALTER TABLE branch_order_recipients ADD COLUMN cc_delivery_mode TEXT NOT NULL DEFAULT 'message' CHECK (cc_delivery_mode IN ('message', 'message_pdf', 'pdf_only'))");
  }
  ...
  if (!deliveryColumns.has("recipient_role")) {
    db.exec("ALTER TABLE branch_order_deliveries ADD COLUMN recipient_role TEXT NOT NULL DEFAULT 'primary' CHECK (recipient_role IN ('primary', 'cc'))");
  }
  if (!deliveryColumns.has("delivery_mode")) {
    db.exec("ALTER TABLE branch_order_deliveries ADD COLUMN delivery_mode TEXT NOT NULL DEFAULT 'message' CHECK (delivery_mode IN ('message', 'message_pdf', 'pdf_only'))");
  }
  if (!deliveryColumns.has("attachment_filename")) {
    db.exec("ALTER TABLE branch_order_deliveries ADD COLUMN attachment_filename TEXT NOT NULL DEFAULT ''");
  }
  if (!deliveryColumns.has("attachment_sha256")) {
    db.exec("ALTER TABLE branch_order_deliveries ADD COLUMN attachment_sha256 TEXT NOT NULL DEFAULT ''");
  }
  ...
  if (!locationSettingsColumns.has("catalog_content_version")) {
    db.exec("ALTER TABLE branch_order_location_settings ADD COLUMN catalog_content_version INTEGER NOT NULL DEFAULT 0");
  }
}
...
function createSqliteBranchOrderOperations(database) {
  ...
  function ensureCurrentCatalogContent(locationId, actor = "system") {
      ...
      for (const title of ["Rolle", "Packung"]) {
        ...
        if (!unit) {
          const sortOrder = Number(db.prepare(`
            ...
            WHERE location_id = ?
          `).get(String(locationId))?.nextSort || 1);
          unit = { id: insertUnit(String(locationId), title, sortOrder, now, actor) };
          changed = true;
        }
        unitIds.set(title, unit.id);
      }
      const groupRecipient = db.prepare(`
        ...
        LIMIT 1
      `).get(plotterGroup.id) || db.prepare(`
        SELECT id
  ...
  function ensureLocationConfiguration(locationId, actor = "system") {
    ...
    if (hasLocationSettings(normalizedLocationId)) {
      ...
      ensureCurrentCatalogContent(normalizedLocationId, actor);
      return false;
    }
    return transaction(() => {
  ...
  function ensureConfiguredLocationCatalogContent(actor = "system:branch-order-catalog-startup") {
    ...
    for (const location of locations) {
      ...
      ensureCatalogConfiguration(locationId, actor);
      ensureCurrentCatalogContent(locationId, actor);
      const currentVersion = Number(db.prepare(`
        SELECT catalog_content_version AS version
        FROM branch_order_location_settings
        WHERE location_id = ?
  ...
  function normalizeConfiguration(locationId, input = {}) {
    ...
    const existingRecipientsById = new Map(db.prepare(`
  ...
  function replaceConfiguration(locationId, input, actor = "", options = {}) {
