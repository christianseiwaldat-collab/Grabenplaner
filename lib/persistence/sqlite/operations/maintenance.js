...
function managedImportRecoveryMetadataFromFile(filePath) {
  const database = openSqliteLegacyDatabase(filePath, { readOnly: true });
  try {
    const keys = tableExistsInDatabase(database, "data_import_runtime_keys")
      ? database.prepare("SELECT id, payload FROM data_import_runtime_keys LIMIT 2").all() : [];
    const hasManagedSources = (tableExistsInDatabase(database, "data_import_sources")
      && Boolean(database.prepare("SELECT 1 FROM data_import_sources LIMIT 1").get()))
      || (tableExistsInDatabase(database, "cash_snapshot_datasets")
      && Boolean(database.prepare("SELECT 1 FROM cash_snapshot_datasets LIMIT 1").get()));
    return { keys, hasManagedSources };
  } finally { database.close(); }
}

// A coherent read-only schema snapshot, without scanning business rows. The
// deployment policy uses this binding; it is not an integrity check.
function deploymentSchemaSnapshotFromFile(filePath) {
  const database = openSqliteLegacyDatabase(filePath, { readOnly: true });
  try {
    ...
    const migrations = database.prepare("SELECT id FROM schema_migrations ORDER BY id").all();
...
module.exports = {
  ...
  deploymentSchemaSnapshotFromFile,
  managedImportRecoveryMetadataFromFile,
