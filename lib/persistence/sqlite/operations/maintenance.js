...
function protectedSicknessAmuStorageSnapshotFromDatabase(database) {
  ...
  const candidateScopedRightsSchema =
    inspectSqlitePersonnelLifecycleScopedRightsSchema(connection);
  const candidateScopedRightsRows =
    inspectSqlitePersonnelLifecycleScopedRightsRows(connection);
  if ((!candidateScopedRightsSchema.absent && !candidateScopedRightsSchema.valid)
