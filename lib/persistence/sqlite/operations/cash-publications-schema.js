'use strict';
const { ensureSqliteCashBindingInventory } = require('./cash-inventory-schema');
...
`;
function ensureSqliteCashPublicationsSchema(database) {
  database.exec(CASH_PUBLICATIONS_SCHEMA_SQL);
  ensureSqliteCashBindingInventory(database);
}
