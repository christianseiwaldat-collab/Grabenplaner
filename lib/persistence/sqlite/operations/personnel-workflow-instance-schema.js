...
const PERSONNEL_WORKFLOW_INSTANCE_ASSIGNED_EMPLOYEE_MIGRATION_ID =
  "v0.90.6-personnel-workflow-assigned-employee-tasks";
const PERSONNEL_WORKFLOW_INSTANCE_ASSIGNED_EMPLOYEE_TRIGGER_NAME =
  "trg_custom_process_run_assignments_relation_insert";
...
const PERSONNEL_WORKFLOW_INSTANCE_LIFECYCLE_BASE_TRIGGER_DEFINITIONS = Object.freeze([
  ...
  definition("trg_custom_process_run_assignments_relation_insert", `
    ...
    WHEN NOT EXISTS (
      ...
      JOIN portal_user_roles portal_role
        ON portal_role.employee_number = portal_user.employee_number
      WHERE binding.run_id = NEW.run_id
        ...
        AND (
          /* lifecycle-assigned-employee:start */
          (
            portal_user.role = 'employee'
            AND NEW.responsibility_type = 'employee'
            AND NEW.employee_number = NEW.responsibility_reference
            AND EXISTS (
              SELECT 1
              FROM personnel_lifecycle_case_package_runs package_run
          ...
          /* lifecycle-assigned-employee:general-start */
          OR (
            NOT EXISTS (
              SELECT 1
          ...
          /* lifecycle-assigned-employee:general-end */
        )
    )
    BEGIN
...
function withoutLifecycleAssignedEmployeeException(item) {
  if (item.name !== PERSONNEL_WORKFLOW_INSTANCE_ASSIGNED_EMPLOYEE_TRIGGER_NAME) {
    return item;
  }
  const sql = item.sql.replace(
    /\/\* lifecycle-assigned-employee:start \*\/[\s\S]*?\/\* lifecycle-assigned-employee:general-start \*\/\s*OR\s*\(\s*([\s\S]*?)\s*\)\s*\/\* lifecycle-assigned-employee:general-end \*\//,
    "$1",
  );
  if (sql === item.sql) {
    throw new Error("Die strikte Workflow-Zuweisungsdefinition konnte nicht erzeugt werden.");
  }
  return definition(item.name, sql);
}

const PERSONNEL_WORKFLOW_INSTANCE_BASE_TRIGGER_DEFINITIONS = Object.freeze(
  PERSONNEL_WORKFLOW_INSTANCE_LIFECYCLE_BASE_TRIGGER_DEFINITIONS.map(
    withoutLifecycleAssignedEmployeeException,
  ),
);
...
const PERSONNEL_WORKFLOW_INSTANCE_ALL_TRIGGER_DEFINITIONS = Object.freeze([
  ...PERSONNEL_WORKFLOW_INSTANCE_LIFECYCLE_BASE_TRIGGER_DEFINITIONS,
...
function migrateSqlitePersonnelWorkflowInstanceAssignedEmployeeTrigger(database) {
  ...
  if (!sqliteObject(database, "table", "custom_process_run_step_assignments")
    || !lifecycleRelationTablesAvailable(database)) {
    return Object.freeze({ applied: false, present: false });
  }
  const expected = PERSONNEL_WORKFLOW_INSTANCE_LIFECYCLE_BASE_TRIGGER_DEFINITIONS.find(
    ({ name }) => name === PERSONNEL_WORKFLOW_INSTANCE_ASSIGNED_EMPLOYEE_TRIGGER_NAME,
  );
  if (!expected) {
    throw new Error("Der kanonische Workflow-Zuweisungstrigger fehlt.");
  }
  const current = sqliteObject(
    database,
    "trigger",
    PERSONNEL_WORKFLOW_INSTANCE_ASSIGNED_EMPLOYEE_TRIGGER_NAME,
  );
  if (current?.sql
    && normalizeDefinitionSql(current.sql, "trigger") === normalizeDefinitionSql(expected.sql, "trigger")) {
    return Object.freeze({ applied: false, present: true });
  }
  database.exec("SAVEPOINT personnel_workflow_assigned_employee_trigger");
  try {
    database.exec(
      `DROP TRIGGER IF EXISTS "${PERSONNEL_WORKFLOW_INSTANCE_ASSIGNED_EMPLOYEE_TRIGGER_NAME}"`,
    );
    database.exec(expected.sql);
    const repaired = sqliteObject(
      database,
      "trigger",
      PERSONNEL_WORKFLOW_INSTANCE_ASSIGNED_EMPLOYEE_TRIGGER_NAME,
    );
    if (!repaired?.sql
      || normalizeDefinitionSql(repaired.sql, "trigger") !== normalizeDefinitionSql(expected.sql, "trigger")) {
      throw new Error("Der Workflow-Zuweisungstrigger konnte nicht kanonisch aktualisiert werden.");
    }
    database.exec("RELEASE SAVEPOINT personnel_workflow_assigned_employee_trigger");
    return Object.freeze({ applied: true, present: true });
  } catch (error) {
    database.exec("ROLLBACK TO SAVEPOINT personnel_workflow_assigned_employee_trigger");
    database.exec("RELEASE SAVEPOINT personnel_workflow_assigned_employee_trigger");
    throw error;
  }
}
...
function ensureSqlitePersonnelWorkflowInstanceSchema(database) {
  ...
  `);
  const lifecycleTablesPresent = lifecycleRelationTablesAvailable(database);
  const expectedBaseTriggers = lifecycleTablesPresent
    ? PERSONNEL_WORKFLOW_INSTANCE_LIFECYCLE_BASE_TRIGGER_DEFINITIONS
    : PERSONNEL_WORKFLOW_INSTANCE_BASE_TRIGGER_DEFINITIONS;
  for (const item of expectedBaseTriggers) {
    const current = sqliteObject(database, "trigger", item.name);
    if (current?.sql
      && normalizeDefinitionSql(current.sql, "trigger") === normalizeDefinitionSql(item.sql, "trigger")) {
      continue;
    }
    database.exec(`DROP TRIGGER IF EXISTS "${item.name}"`);
    database.exec(item.sql);
  }
  if (lifecycleTablesPresent) {
