...
const PERSONNEL_WORKFLOW_INSTANCE_TRIGGER_DEFINITIONS = Object.freeze([
  definition("trg_custom_process_run_bindings_relation_insert", `
    ...
    WHEN NOT EXISTS (
      ...
      WHERE run.id = NEW.run_id
        ...
        AND publication.workflow_type NOT IN (
          'onboarding','offboarding','custom_personnel'
        )
        AND NOT EXISTS (
          SELECT 1
...
function inspectSqlitePersonnelWorkflowInstanceSchema(database) {
  ...
  return Object.freeze({
    ...
    absent: tables.missing.length === PERSONNEL_WORKFLOW_INSTANCE_TABLE_DEFINITIONS.length
      && triggers.missing.length === PERSONNEL_WORKFLOW_INSTANCE_TRIGGER_DEFINITIONS.length,
    issues,
...
function inspectSqlitePersonnelWorkflowInstanceRows(database) {
  ...
  const rows = database.prepare(`
    SELECT binding.*,
      ...
      publication.workflow_type, publication.data_classification,
  ...
  for (const row of rows) {
    ...
    const employeeValid = row.subject_type === "employee"
    ...
    const relationValid = snapshot
      ...
      && !["onboarding", "offboarding", "custom_personnel"].includes(row.workflow_type)
      && notificationChannelsValid
      && scopeValid
      && (candidateValid || employeeValid);
...
function ensureSqlitePersonnelWorkflowInstanceSchema(database) {
  ...
  `);
  for (const item of PERSONNEL_WORKFLOW_INSTANCE_TRIGGER_DEFINITIONS) {
    database.exec(item.sql);
  }
}
