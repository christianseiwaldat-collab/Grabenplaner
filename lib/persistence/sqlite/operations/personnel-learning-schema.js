...
function inspectSqlitePersonnelLearningRows(database) {
  ...
  for (const module of modules) {
    ...
    for (let index = 0; index < versions.length; index += 1) {
      ...
      if (sha256Text(version.content_json) !== version.content_sha256) {
        issues.push(`module-version-content-invalid:${module.id}:${version.version_number}`);
      }
      if (sha256Text(version.scope_snapshot_json) !== version.scope_snapshot_sha256) {
    ...
    for (let index = 0; index < events.length; index += 1) {
      ...
      if (sha256Text(event.event_payload_json) !== event.event_payload_sha256) {
