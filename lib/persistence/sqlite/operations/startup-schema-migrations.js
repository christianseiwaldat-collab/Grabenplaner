...
function legacyPersonnelLifecycleScopedRightsSchemaPresent() {
  ...
  `).get();
  return normalizedSqliteSchemaSql(row?.sql)
    === normalizedSqliteSchemaSql(
      PERSONNEL_LIFECYCLE_LEGACY_SCOPED_RIGHTS_TABLE_DEFINITION.sql,
    );
}
...
function migrateLegacyPersonnelLifecycleScopedRightsSchema() {
  ...
  if (!target || tableExists("portal_permission_scope_grants_r1_legacy")) {
  ...
  try {
    ...
    db.exec(`
      ALTER TABLE portal_permission_scope_grants
        RENAME TO portal_permission_scope_grants_r1_legacy;
      ${target.sql}
      ...
      SELECT
        employee_number, permission, location_id, department_id,
        approved_by, created_at, updated_at
      FROM portal_permission_scope_grants_r1_legacy;
      DROP TABLE portal_permission_scope_grants_r1_legacy;
    `);
...
function repairPersonnelLifecycleScopedRightsSchemaBeforeCreate(inspection, rowInspection) {
  ...
  if (inspection.invalidTables.includes("portal_permission_scope_grants")
