...
} = require("./central-article-loan-migration");
const {
  ensureSqliteSalesArticleCatalogSchema,
} = require("./sales-article-catalog-schema");
...
const personnelLifecycleProfilePhotoCategoryMigrationId =
  PERSONNEL_LIFECYCLE_PROFILE_PHOTO_CATEGORY_MIGRATION_ID;
...
const personnelLearningFoundationMigrationId = PERSONNEL_LEARNING_FOUNDATION_MIGRATION_ID;
const personnelLearningCompetencyMigrationId = PERSONNEL_LEARNING_COMPETENCY_MIGRATION_ID;
const personnelLearningAssignmentMigrationId = PERSONNEL_LEARNING_ASSIGNMENT_MIGRATION_ID;
const personnelLearningProgressMigrationId = PERSONNEL_LEARNING_PROGRESS_MIGRATION_ID;
const staffAssignmentRequestMigrationId = STAFF_ASSIGNMENT_REQUEST_MIGRATION_ID;
const portalBirthdayPresentationMigrationId = PORTAL_BIRTHDAY_PRESENTATION_MIGRATION_ID;
const portalBirthdayPresentationCatalogMigrationId =
  PORTAL_BIRTHDAY_PRESENTATION_CATALOG_MIGRATION_ID;
const portalBirthdayPresentationClaimMigrationId =
  PORTAL_BIRTHDAY_PRESENTATION_CLAIM_MIGRATION_ID;
...
const expectedBuiltinCandidateDocumentCategories = Object.freeze([
  ...
  Object.freeze(["profile-photo", "profile_photo", "Bewerberfoto", "scoped_leadership", "manual_review", null, 0, 1, 1, 70]),
]);
const legacyBuiltinCandidateDocumentCategories = Object.freeze(
  expectedBuiltinCandidateDocumentCategories.slice(0, -1),
);
const candidateDocumentCategoryCatalogColumns = Object.freeze([
  "id", "code", "label", "default_visibility", "retention_disposition",
  "default_retention_days", "transfer_eligible", "active", "builtin", "sort_order",
]);

function candidateDocumentCategorySignature(row) {
  return [
    row.id,
    row.code,
    row.label,
...
function inspectCandidateDocumentCategoryCatalog() {
  if (!tableExists("candidate_document_categories")) {
    return Object.freeze({ state: "absent", issues: Object.freeze(["table-missing"]) });
  }
  const actualColumns = new Set(db.prepare("PRAGMA table_info(candidate_document_categories)").all()
    .map(({ name }) => String(name)));
  const missingColumns = candidateDocumentCategoryCatalogColumns
    .filter((name) => !actualColumns.has(name));
  if (missingColumns.length) {
    return Object.freeze({
      state: "invalid",
      issues: Object.freeze(missingColumns.map((name) => `column-missing:${name}`)),
    });
  }
  const rows = db.prepare(`
    SELECT ${candidateDocumentCategoryCatalogColumns.join(", ")}
    FROM candidate_document_categories
    ORDER BY sort_order, id
  `).all();
  const builtinSignatures = rows
    .filter((row) => Number(row.builtin) === 1)
    .map(candidateDocumentCategorySignature);
  const reservedRows = rows.filter((row) => (
    row.id === "profile-photo" || row.code === "profile_photo"
  ));
  const serializedBuiltins = JSON.stringify(builtinSignatures);
  if (serializedBuiltins === JSON.stringify(expectedBuiltinCandidateDocumentCategories)
    && reservedRows.length === 1
    && JSON.stringify(candidateDocumentCategorySignature(reservedRows[0]))
      === JSON.stringify(expectedBuiltinCandidateDocumentCategories.at(-1))) {
    return Object.freeze({ state: "current", issues: Object.freeze([]) });
  }
  if (serializedBuiltins === JSON.stringify(legacyBuiltinCandidateDocumentCategories)
    && reservedRows.length === 0) {
    return Object.freeze({ state: "legacy", issues: Object.freeze([]) });
  }
  const issues = [];
  if (reservedRows.length) issues.push("profile-photo-reserved-row-conflict");
  if (serializedBuiltins !== JSON.stringify(expectedBuiltinCandidateDocumentCategories)
    && serializedBuiltins !== JSON.stringify(legacyBuiltinCandidateDocumentCategories)) {
    issues.push("builtin-catalog-unrecognized");
  }
  return Object.freeze({
    state: "invalid",
    issues: Object.freeze(issues.length ? issues : ["catalog-unrecognized"]),
  });
}

function personnelLifecycleProfilePhotoCategoryMigrationError(details = []) {
  const error = new Error(
    "Der bestehende Bewerberdokument-Katalog kann nicht eindeutig und sicher um die "
      + "Kategorie Bewerberfoto erweitert werden. Die Katalogmigration wurde ohne "
      + "Datenänderung beendet.",
  );
...
function migratePersonnelLifecycleProfilePhotoCategory() {
  const before = inspectCandidateDocumentCategoryCatalog();
  if (before.state === "current") {
    return Object.freeze({ migrated: false, deferred: false });
  }
  if (before.state === "absent") {
    return Object.freeze({ migrated: false, deferred: true });
  }
  if (before.state !== "legacy") {
    throw personnelLifecycleProfilePhotoCategoryMigrationError(before.issues);
  }
  const beforeCount = Number(db.prepare(`
    SELECT COUNT(*) AS count FROM candidate_document_categories
  `).get().count);
  db.exec("BEGIN IMMEDIATE");
  try {
    db.prepare(`
      INSERT INTO candidate_document_categories (
        ...
        'profile-photo', 'profile_photo', 'Bewerberfoto', 'scoped_leadership',
        'manual_review', NULL, 0, 1, 1, 70
      )
    `).run();
    const afterCount = Number(db.prepare(`
      SELECT COUNT(*) AS count FROM candidate_document_categories
    `).get().count);
    const after = inspectCandidateDocumentCategoryCatalog();
    if (afterCount !== beforeCount + 1 || after.state !== "current") {
      throw personnelLifecycleProfilePhotoCategoryMigrationError([
        ...(after.issues || []),
        ...(afterCount === beforeCount + 1 ? [] : ["category-count-mismatch"]),
      ]);
    }
    db.exec("COMMIT");
    return Object.freeze({ migrated: true, deferred: false });
  } catch (error) {
    try { db.exec("ROLLBACK"); } catch {}
    if (error?.code === "PERSONNEL_LIFECYCLE_PROFILE_PHOTO_CATEGORY_MIGRATION_UNSAFE") {
      throw error;
    }
    const migrationError = personnelLifecycleProfilePhotoCategoryMigrationError([
      "transaction-failed",
    ]);
...
function validatePersonnelLifecycleProfilePhotoCategoryCatalog() {
  const inspection = inspectCandidateDocumentCategoryCatalog();
  if (inspection.state !== "current") {
    throw personnelLifecycleProfilePhotoCategoryMigrationError(inspection.issues);
  }
}
...
function candidateDocumentCategoriesContainBusinessData() {
  ...
  return serialized !== JSON.stringify(expectedBuiltinCandidateDocumentCategories)
    && serialized !== JSON.stringify(legacyBuiltinCandidateDocumentCategories);
}
...
function personnelLearningFoundationSchemaDataError() {
  const error = new Error(
    "Das Datenfundament fuer Schulung und Wissen weicht vom erwarteten Stand ab "
      + "oder enthaelt bereits widerspruechliche Fachdaten. Die automatische Reparatur "
      + "wurde nach der Sicherheitssicherung ohne Datenverlust beendet.",
  );
  error.code = "PERSONNEL_LEARNING_FOUNDATION_SCHEMA_DATA_PRESENT";
  return error;
}

function removePersonnelLearningFoundationTriggers(triggerNames) {
  const allowed = new Set(
...
function removePersonnelLearningFoundationIndexes(indexNames) {
  const allowed = new Set(
    PERSONNEL_LEARNING_INDEX_DEFINITIONS.map(({ name }) => name),
  );
  for (const name of indexNames) {
    if (allowed.has(name)) db.exec(`DROP INDEX IF EXISTS "${name}"`);
  }
}

function personnelLearningFoundationDataExists() {
  try {
    if (PERSONNEL_LEARNING_TABLE_NAMES.some((name) => (
      tableExists(name) && tableContainsRows(name)
    ))) return true;
    return tableExists("portal_permission_denials") && Boolean(db.prepare(`
      SELECT 1
      FROM portal_permission_denials
      WHERE permission = ?
      LIMIT 1
    `).get(PERSONNEL_LEARNING_CROSS_LOCATION_PERMISSION));
  } catch {
    throw personnelLearningFoundationSchemaDataError();
  }
}

function rebuildEmptyPersonnelLearningFoundationSchema() {
  db.exec("BEGIN IMMEDIATE");
  try {
    removePersonnelLearningFoundationTriggers(
      PERSONNEL_LEARNING_TRIGGER_DEFINITIONS.map(({ name }) => name),
    );
    removePersonnelLearningFoundationIndexes(
      PERSONNEL_LEARNING_INDEX_DEFINITIONS.map(({ name }) => name),
    );
    for (const name of [...PERSONNEL_LEARNING_TABLE_NAMES].reverse()) {
      if (tableExists(name)) db.exec(`DROP TABLE "${name}"`);
    }
    db.exec("COMMIT");
  } catch (error) {
...
function repairPersonnelLearningFoundationSchemaBeforeCreate(inspection, rowInspection) {
  const tableSchemaDiverges = inspection.invalidTables.length > 0
    || (!inspection.absent && inspection.missingTables.length > 0);
  if ((tableSchemaDiverges || !rowInspection.valid)
    && personnelLearningFoundationDataExists()) {
    throw personnelLearningFoundationSchemaDataError();
  }
  if (tableSchemaDiverges) {
    rebuildEmptyPersonnelLearningFoundationSchema();
    return;
  }
  removePersonnelLearningFoundationTriggers(inspection.invalidTriggers);
  removePersonnelLearningFoundationIndexes(inspection.invalidIndexes);
}

function validatePersonnelLearningFoundationSchema() {
  const schema = inspectSqlitePersonnelLearningSchema(db);
  const rows = inspectSqlitePersonnelLearningRows(db);
  if (!schema.valid || !rows.valid) {
    const error = new Error(
      "Das Datenfundament fuer Schulung und Wissen konnte nicht sicher auf den "
        + "erwarteten Stand gebracht werden.",
    );
    error.code = "PERSONNEL_LEARNING_FOUNDATION_SCHEMA_INVALID";
    error.details = Object.freeze([...(schema.issues || []), ...(rows.issues || [])]);
    throw error;
  }
}

function personnelLearningCompetencySchemaDataError() {
  const error = new Error(
    "Das Datenfundament der Mitarbeiter-Kompetenzprofile weicht vom erwarteten Stand ab "
      + "oder enthält bereits widersprüchliche Fachdaten. Die automatische Reparatur "
      + "wurde nach der Sicherheitssicherung ohne Datenverlust beendet.",
  );
...
function removePersonnelLearningCompetencyDefinitions(type, definitions) {
  const keyword = type === "trigger" ? "TRIGGER" : "INDEX";
  for (const { name } of definitions) db.exec(`DROP ${keyword} IF EXISTS "${name}"`);
}

function personnelLearningCompetencyDataExists() {
  try {
    return PERSONNEL_LEARNING_COMPETENCY_TABLE_NAMES.some((name) => (
      tableExists(name) && tableContainsRows(name)
    ));
  } catch {
    throw personnelLearningCompetencySchemaDataError();
  }
}

function rebuildEmptyPersonnelLearningCompetencySchema() {
  db.exec("BEGIN IMMEDIATE");
  try {
    removePersonnelLearningCompetencyDefinitions(
      "trigger",
      PERSONNEL_LEARNING_COMPETENCY_TRIGGER_DEFINITIONS,
    );
    removePersonnelLearningCompetencyDefinitions(
    ...
    for (const name of [...PERSONNEL_LEARNING_COMPETENCY_TABLE_NAMES].reverse()) {
      if (tableExists(name)) db.exec(`DROP TABLE "${name}"`);
    }
    db.exec("COMMIT");
  } catch (error) {
...
function repairPersonnelLearningCompetencySchemaBeforeCreate(inspection, rowInspection) {
  const tableSchemaDiverges = inspection.invalidTables.length > 0
    || (!inspection.absent && inspection.missingTables.length > 0);
  if ((tableSchemaDiverges || !rowInspection.valid)
    && personnelLearningCompetencyDataExists()) {
    throw personnelLearningCompetencySchemaDataError();
  }
  if (tableSchemaDiverges) {
    rebuildEmptyPersonnelLearningCompetencySchema();
    return;
  }
  const invalidTriggers = new Set(inspection.invalidTriggers || []);
  const invalidIndexes = new Set(inspection.invalidIndexes || []);
  removePersonnelLearningCompetencyDefinitions(
    "trigger",
    PERSONNEL_LEARNING_COMPETENCY_TRIGGER_DEFINITIONS.filter(({ name }) => (
      invalidTriggers.has(name)
    )),
  );
  removePersonnelLearningCompetencyDefinitions(
    "index",
    PERSONNEL_LEARNING_COMPETENCY_INDEX_DEFINITIONS.filter(({ name }) => (
      invalidIndexes.has(name)
    )),
  );
}

function validatePersonnelLearningCompetencySchema() {
  const schema = inspectSqlitePersonnelLearningCompetencySchema(db);
  const rows = inspectSqlitePersonnelLearningCompetencyRows(db);
  if (!schema.valid || !rows.valid) {
    const error = new Error(
      "Das Datenfundament der Mitarbeiter-Kompetenzprofile konnte nicht sicher auf den "
        + "erwarteten Stand gebracht werden.",
    );
    error.code = "PERSONNEL_LEARNING_COMPETENCY_SCHEMA_INVALID";
    error.details = Object.freeze([...(schema.issues || []), ...(rows.issues || [])]);
    throw error;
  }
}

function personnelLearningAssignmentSchemaDataError() {
  const error = new Error(
    "Das Datenfundament der Schulungszuweisungen weicht vom erwarteten Stand ab "
      + "oder enthält bereits widersprüchliche Fachdaten. Die automatische Reparatur "
      + "wurde nach der Sicherheitssicherung ohne Datenverlust beendet.",
  );
...
function removePersonnelLearningAssignmentDefinitions(type, definitions) {
  const keyword = type === "trigger" ? "TRIGGER" : "INDEX";
  for (const { name } of definitions) db.exec(`DROP ${keyword} IF EXISTS "${name}"`);
}

function personnelLearningAssignmentDataExists() {
  try {
    return PERSONNEL_LEARNING_ASSIGNMENT_TABLE_NAMES.some((name) => (
      tableExists(name) && tableContainsRows(name)
    ));
  } catch {
    throw personnelLearningAssignmentSchemaDataError();
  }
}

function rebuildEmptyPersonnelLearningAssignmentSchema() {
  db.exec("BEGIN IMMEDIATE");
  try {
    removePersonnelLearningAssignmentDefinitions(
      "trigger",
      PERSONNEL_LEARNING_ASSIGNMENT_TRIGGER_DEFINITIONS,
    );
    removePersonnelLearningAssignmentDefinitions(
    ...
    for (const name of [...PERSONNEL_LEARNING_ASSIGNMENT_TABLE_NAMES].reverse()) {
      if (tableExists(name)) db.exec(`DROP TABLE "${name}"`);
    }
    db.exec("COMMIT");
  } catch (error) {
...
function repairPersonnelLearningAssignmentSchemaBeforeCreate(inspection, rowInspection) {
  const tableSchemaDiverges = inspection.invalidTables.length > 0
    || (!inspection.absent && inspection.missingTables.length > 0);
  if ((tableSchemaDiverges || !rowInspection.valid)
    && personnelLearningAssignmentDataExists()) {
    throw personnelLearningAssignmentSchemaDataError();
  }
  if (tableSchemaDiverges) {
    rebuildEmptyPersonnelLearningAssignmentSchema();
    return;
  }
  const invalidTriggers = new Set(inspection.invalidTriggers || []);
  const invalidIndexes = new Set(inspection.invalidIndexes || []);
  removePersonnelLearningAssignmentDefinitions(
    "trigger",
    PERSONNEL_LEARNING_ASSIGNMENT_TRIGGER_DEFINITIONS.filter(({ name }) => (
      invalidTriggers.has(name)
    )),
  );
  removePersonnelLearningAssignmentDefinitions(
    "index",
    PERSONNEL_LEARNING_ASSIGNMENT_INDEX_DEFINITIONS.filter(({ name }) => (
      invalidIndexes.has(name)
    )),
  );
}

function validatePersonnelLearningAssignmentSchema() {
  const schema = inspectSqlitePersonnelLearningAssignmentSchema(db);
  const rows = inspectSqlitePersonnelLearningAssignmentRows(db);
  if (!schema.valid || !rows.valid) {
    const error = new Error(
      "Das Datenfundament der Schulungszuweisungen konnte nicht sicher auf den "
        + "erwarteten Stand gebracht werden.",
    );
    error.code = "PERSONNEL_LEARNING_ASSIGNMENT_SCHEMA_INVALID";
    error.details = Object.freeze([...(schema.issues || []), ...(rows.issues || [])]);
    throw error;
  }
}

function personnelLearningProgressSchemaDataError() {
  const error = new Error(
    "Das Datenfundament der Schulungsfortschritte weicht vom erwarteten Stand ab "
      + "oder enthält bereits widersprüchliche Fachdaten. Die automatische Reparatur "
      + "wurde nach der Sicherheitssicherung ohne Datenverlust beendet.",
  );
...
function removePersonnelLearningProgressDefinitions(type, definitions) {
  const keyword = type === "trigger" ? "TRIGGER" : "INDEX";
  for (const { name } of definitions) db.exec(`DROP ${keyword} IF EXISTS "${name}"`);
}

function personnelLearningProgressDataExists() {
  try {
    return PERSONNEL_LEARNING_PROGRESS_TABLE_NAMES.some((name) => (
      tableExists(name) && tableContainsRows(name)
    ));
  } catch {
    throw personnelLearningProgressSchemaDataError();
  }
}

function rebuildEmptyPersonnelLearningProgressSchema() {
  db.exec("BEGIN IMMEDIATE");
  try {
    removePersonnelLearningProgressDefinitions(
      "trigger",
      PERSONNEL_LEARNING_PROGRESS_TRIGGER_DEFINITIONS,
    );
    removePersonnelLearningProgressDefinitions(
    ...
    for (const name of [...PERSONNEL_LEARNING_PROGRESS_TABLE_NAMES].reverse()) {
      if (tableExists(name)) db.exec(`DROP TABLE "${name}"`);
    }
    db.exec("COMMIT");
  } catch (error) {
...
function repairPersonnelLearningProgressSchemaBeforeCreate(inspection, rowInspection) {
  const tableSchemaDiverges = inspection.invalidTables.length > 0
    || (!inspection.absent && inspection.missingTables.length > 0);
  if ((tableSchemaDiverges || !rowInspection.valid)
    && personnelLearningProgressDataExists()) {
    throw personnelLearningProgressSchemaDataError();
  }
  if (tableSchemaDiverges) {
    rebuildEmptyPersonnelLearningProgressSchema();
    return;
  }
  const invalidTriggers = new Set(inspection.invalidTriggers || []);
  const invalidIndexes = new Set(inspection.invalidIndexes || []);
  removePersonnelLearningProgressDefinitions(
    "trigger",
    PERSONNEL_LEARNING_PROGRESS_TRIGGER_DEFINITIONS.filter(({ name }) => (
      invalidTriggers.has(name)
    )),
  );
  removePersonnelLearningProgressDefinitions(
    "index",
    PERSONNEL_LEARNING_PROGRESS_INDEX_DEFINITIONS.filter(({ name }) => (
      invalidIndexes.has(name)
    )),
  );
}

function validatePersonnelLearningProgressSchema() {
  const schema = inspectSqlitePersonnelLearningProgressSchema(db);
  const rows = inspectSqlitePersonnelLearningProgressRows(db);
  if (!schema.valid || !rows.valid) {
    const error = new Error(
      "Das Datenfundament der Schulungsfortschritte konnte nicht sicher auf den "
        + "erwarteten Stand gebracht werden.",
    );
    error.code = "PERSONNEL_LEARNING_PROGRESS_SCHEMA_INVALID";
    error.details = Object.freeze([...(schema.issues || []), ...(rows.issues || [])]);
    throw error;
  }
}

function staffAssignmentRequestSchemaDataError() {
  const error = new Error(
    "Das versionierte Datenfundament der standortübergreifenden Einsatzanfragen "
      + "weicht vom erwarteten Stand ab oder enthält widersprüchliche Fachdaten. "
      + "Die automatische Reparatur wurde nach der Sicherheitssicherung ohne "
      + "Datenverlust beendet.",
  );
  error.code = "STAFF_ASSIGNMENT_REQUEST_SCHEMA_DATA_PRESENT";
  return error;
}

function removeStaffAssignmentRequestDefinitions(type, definitions) {
  const keyword = type === "trigger" ? "TRIGGER" : "INDEX";
  for (const { name } of definitions) db.exec(`DROP ${keyword} IF EXISTS "${name}"`);
}

function staffAssignmentRequestDataExists() {
  try {
    return STAFF_ASSIGNMENT_REQUEST_TABLE_NAMES.some((name) => (
      tableExists(name) && tableContainsRows(name)
    ));
  } catch {
    throw staffAssignmentRequestSchemaDataError();
  }
}

function rebuildEmptyStaffAssignmentRequestSchema() {
  db.exec("BEGIN IMMEDIATE");
  try {
    removeStaffAssignmentRequestDefinitions(
      "trigger",
      STAFF_ASSIGNMENT_REQUEST_TRIGGER_DEFINITIONS,
    );
    removeStaffAssignmentRequestDefinitions(
    ...
    for (const name of [...STAFF_ASSIGNMENT_REQUEST_TABLE_NAMES].reverse()) {
      if (tableExists(name)) db.exec(`DROP TABLE "${name}"`);
    }
    db.exec("COMMIT");
  } catch (error) {
...
function repairStaffAssignmentRequestSchemaBeforeCreate(inspection, rowInspection) {
  const tableSchemaDiverges = inspection.invalidTables.length > 0
    || (!inspection.absent && inspection.missingTables.length > 0);
  const definitionDrift = tableSchemaDiverges
    || inspection.invalidTriggers.length > 0
    || inspection.invalidIndexes.length > 0
    || (!inspection.absent && (
      inspection.missingTriggers.length > 0 || inspection.missingIndexes.length > 0
    ));
  if ((definitionDrift || !rowInspection.valid)
    && staffAssignmentRequestDataExists()) {
    throw staffAssignmentRequestSchemaDataError();
  }
  if (tableSchemaDiverges) {
    rebuildEmptyStaffAssignmentRequestSchema();
    return;
  }
  const invalidTriggers = new Set(inspection.invalidTriggers || []);
  const invalidIndexes = new Set(inspection.invalidIndexes || []);
  removeStaffAssignmentRequestDefinitions(
    "trigger",
    STAFF_ASSIGNMENT_REQUEST_TRIGGER_DEFINITIONS.filter(({ name }) => (
      invalidTriggers.has(name)
    )),
  );
  removeStaffAssignmentRequestDefinitions(
    "index",
    STAFF_ASSIGNMENT_REQUEST_INDEX_DEFINITIONS.filter(({ name }) => (
      invalidIndexes.has(name)
    )),
  );
}

function validateStaffAssignmentRequestSchema() {
  const schema = inspectSqliteStaffAssignmentRequestSchema(db);
  const rows = inspectSqliteStaffAssignmentRequestRows(db);
  if (!schema.valid || !rows.valid) {
    const error = new Error(
      "Das versionierte Datenfundament der standortübergreifenden Einsatzanfragen "
        + "konnte nicht sicher auf den erwarteten Stand gebracht werden.",
    );
    error.code = "STAFF_ASSIGNMENT_REQUEST_SCHEMA_INVALID";
    error.details = Object.freeze([...(schema.issues || []), ...(rows.issues || [])]);
    throw error;
  }
}

function portalBirthdayPresentationSchemaDataError() {
  const error = new Error(
    "Das revisionsgeschützte Datenfundament der Portal-Geburtstagsdarstellungen "
      + "weicht vom erwarteten Stand ab oder enthält widersprüchliche Konfigurationen. "
      + "Die automatische Reparatur wurde nach der Sicherheitssicherung ohne "
      + "Datenverlust beendet.",
  );
  error.code = "PORTAL_BIRTHDAY_PRESENTATION_SCHEMA_DATA_PRESENT";
  return error;
}

function portalBirthdayPresentationDataExists() {
  try {
    return PORTAL_BIRTHDAY_PRESENTATION_TABLE_NAMES.some((name) => (
      tableExists(name) && tableContainsRows(name)
    ));
  } catch {
    throw portalBirthdayPresentationSchemaDataError();
  }
}

function rebuildEmptyPortalBirthdayPresentationSchema() {
  db.exec("BEGIN IMMEDIATE");
  try {
    for (const { name } of PORTAL_BIRTHDAY_PRESENTATION_TRIGGER_DEFINITIONS) {
      db.exec(`DROP TRIGGER IF EXISTS "${name}"`);
    }
    for (const name of [...PORTAL_BIRTHDAY_PRESENTATION_TABLE_NAMES].reverse()) {
      if (tableExists(name)) db.exec(`DROP TABLE "${name}"`);
    }
    db.exec("COMMIT");
  } catch (error) {
...
function repairPortalBirthdayPresentationSchemaBeforeCreate(inspection, rowInspection) {
  const tableSchemaDiverges = inspection.invalidTables.length > 0
    || (!inspection.absent && inspection.missingTables.length > 0);
  const definitionDrift = tableSchemaDiverges
    || inspection.invalidTriggers.length > 0
    || (!inspection.absent && inspection.missingTriggers.length > 0);
  if ((definitionDrift || !rowInspection.valid)
    && portalBirthdayPresentationDataExists()) {
    throw portalBirthdayPresentationSchemaDataError();
  }
  if (tableSchemaDiverges) {
    rebuildEmptyPortalBirthdayPresentationSchema();
    return;
  }
  const invalidTriggers = new Set(inspection.invalidTriggers || []);
  for (const { name } of PORTAL_BIRTHDAY_PRESENTATION_TRIGGER_DEFINITIONS) {
    if (invalidTriggers.has(name)) db.exec(`DROP TRIGGER IF EXISTS "${name}"`);
  }
}

function validatePortalBirthdayPresentationSchema() {
  const schema = inspectSqlitePortalBirthdayPresentationSchema(db);
  const rows = inspectSqlitePortalBirthdayPresentationRows(db);
  if (!schema.valid || !rows.valid) {
    const error = new Error(
      "Das Datenfundament der Portal-Geburtstagsdarstellungen konnte nicht sicher "
        + "auf den erwarteten Stand gebracht werden.",
    );
    error.code = "PORTAL_BIRTHDAY_PRESENTATION_SCHEMA_INVALID";
    error.details = Object.freeze([...(schema.issues || []), ...(rows.issues || [])]);
    throw error;
  }
}

function portalBirthdayPresentationClaimSchemaDataError() {
  const error = new Error(
    "Das Datenfundament der Geburtstagseinblendungs-Claims "
      + "weicht vom erwarteten Stand ab. Die automatische Reparatur wurde "
      + "ohne Datenänderung beendet.",
  );
  error.code = "PORTAL_BIRTHDAY_PRESENTATION_CLAIM_SCHEMA_DATA_PRESENT";
  return error;
}

function repairPortalBirthdayPresentationClaimSchemaBeforeCreate(inspection, rowInspection) {
  if (inspection.absent) return;
  const hasRows = tableExists(PORTAL_BIRTHDAY_PRESENTATION_CLAIM_TABLE_DEFINITION.name)
    && tableContainsRows(PORTAL_BIRTHDAY_PRESENTATION_CLAIM_TABLE_DEFINITION.name);
  if ((!inspection.valid || !rowInspection.valid) && hasRows) {
    throw portalBirthdayPresentationClaimSchemaDataError();
  }
  if (inspection.valid && rowInspection.valid) return;
  db.exec("BEGIN IMMEDIATE");
  try {
    for (const { name } of PORTAL_BIRTHDAY_PRESENTATION_CLAIM_TRIGGER_DEFINITIONS) {
      db.exec(`DROP TRIGGER IF EXISTS "${name}"`);
    }
    if (tableExists(PORTAL_BIRTHDAY_PRESENTATION_CLAIM_TABLE_DEFINITION.name)) {
      db.exec(`DROP TABLE "${PORTAL_BIRTHDAY_PRESENTATION_CLAIM_TABLE_DEFINITION.name}"`);
    }
    db.exec("COMMIT");
  } catch (error) {
    try { db.exec("ROLLBACK"); } catch {}
...
function validatePortalBirthdayPresentationClaimSchema() {
  const schema = inspectSqlitePortalBirthdayPresentationClaimSchema(db);
  const rows = inspectSqlitePortalBirthdayPresentationClaimRows(db);
  if (!schema.valid || !rows.valid) {
    const error = new Error(
      "Das Datenfundament der Geburtstagseinblendungs-Claims konnte nicht sicher "
        + "auf den erwarteten Stand gebracht werden.",
    );
    error.code = "PORTAL_BIRTHDAY_PRESENTATION_CLAIM_SCHEMA_INVALID";
    error.details = Object.freeze([...(schema.issues || []), ...(rows.issues || [])]);
    throw error;
  }
}
...
const centralArticleLoanSchemaBeforeMigration = inspectSqliteCentralArticleLoanMigration(db);
const centralArticleLoanMigrationRequired = !tableExists("schema_migrations")
  || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ? LIMIT 1")
    .get(CENTRAL_ARTICLE_LOAN_MIGRATION_ID)
  || centralArticleLoanSchemaBeforeMigration.required;
...
const personnelLifecycleProfilePhotoCategoryCatalogBeforeMigration =
  inspectCandidateDocumentCategoryCatalog();
const personnelLifecycleProfilePhotoCategoryMigrationRequired = !tableExists("schema_migrations")
  || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ? LIMIT 1")
    .get(personnelLifecycleProfilePhotoCategoryMigrationId)
  || personnelLifecycleProfilePhotoCategoryCatalogBeforeMigration.state !== "current";
...
const personnelLearningFoundationSchemaBeforeMigration =
  inspectSqlitePersonnelLearningSchema(db);
const personnelLearningFoundationRowsBeforeMigration =
  inspectSqlitePersonnelLearningRows(db);
const personnelLearningFoundationMigrationRequired = !tableExists("schema_migrations")
  || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ? LIMIT 1")
    .get(personnelLearningFoundationMigrationId)
  || !personnelLearningFoundationSchemaBeforeMigration.valid
  || !personnelLearningFoundationRowsBeforeMigration.valid;
const personnelLearningCompetencySchemaBeforeMigration =
  inspectSqlitePersonnelLearningCompetencySchema(db);
const personnelLearningCompetencyRowsBeforeMigration =
  inspectSqlitePersonnelLearningCompetencyRows(db);
const personnelLearningCompetencyMigrationRequired = !tableExists("schema_migrations")
  || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ? LIMIT 1")
    .get(personnelLearningCompetencyMigrationId)
  || !personnelLearningCompetencySchemaBeforeMigration.valid
  || !personnelLearningCompetencyRowsBeforeMigration.valid;
const personnelLearningAssignmentSchemaBeforeMigration =
  inspectSqlitePersonnelLearningAssignmentSchema(db);
const personnelLearningAssignmentRowsBeforeMigration =
  inspectSqlitePersonnelLearningAssignmentRows(db);
const personnelLearningAssignmentMigrationRequired = !tableExists("schema_migrations")
  || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ? LIMIT 1")
    .get(personnelLearningAssignmentMigrationId)
  || !personnelLearningAssignmentSchemaBeforeMigration.valid
  || !personnelLearningAssignmentRowsBeforeMigration.valid;
const personnelLearningProgressSchemaBeforeMigration =
  inspectSqlitePersonnelLearningProgressSchema(db);
const personnelLearningProgressRowsBeforeMigration =
  inspectSqlitePersonnelLearningProgressRows(db);
const personnelLearningProgressMigrationRequired = !tableExists("schema_migrations")
  || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ? LIMIT 1")
    .get(personnelLearningProgressMigrationId)
  || !personnelLearningProgressSchemaBeforeMigration.valid
  || !personnelLearningProgressRowsBeforeMigration.valid;
const staffAssignmentRequestSchemaBeforeMigration =
  inspectSqliteStaffAssignmentRequestSchema(db);
const staffAssignmentRequestRowsBeforeMigration =
  inspectSqliteStaffAssignmentRequestRows(db);
const staffAssignmentRequestMigrationRequired = !tableExists("schema_migrations")
  || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ? LIMIT 1")
    .get(staffAssignmentRequestMigrationId)
  || !staffAssignmentRequestSchemaBeforeMigration.valid
  || !staffAssignmentRequestRowsBeforeMigration.valid;
const portalBirthdayPresentationSchemaBeforeCatalogMigration =
  inspectSqlitePortalBirthdayPresentationSchema(db);
const portalBirthdayPresentationRowsBeforeCatalogMigration =
  inspectSqlitePortalBirthdayPresentationRows(db);
const portalBirthdayPresentationLegacyV1SchemaBeforeMigration =
  inspectSqlitePortalBirthdayPresentationLegacyV1Schema(db);
const portalBirthdayPresentationCatalogMigrationRequired = !tableExists("schema_migrations")
  || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ? LIMIT 1")
    .get(portalBirthdayPresentationCatalogMigrationId)
  || portalBirthdayPresentationLegacyV1SchemaBeforeMigration.valid;
const portalBirthdayPresentationMigrationRequiredBeforeCatalog =
  !tableExists("schema_migrations")
  || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ? LIMIT 1")
    .get(portalBirthdayPresentationMigrationId)
  || (!portalBirthdayPresentationLegacyV1SchemaBeforeMigration.valid
    && (!portalBirthdayPresentationSchemaBeforeCatalogMigration.valid
      || !portalBirthdayPresentationRowsBeforeCatalogMigration.valid));
const portalBirthdayPresentationClaimSchemaBeforeMigration =
  inspectSqlitePortalBirthdayPresentationClaimSchema(db);
const portalBirthdayPresentationClaimRowsBeforeMigration =
  inspectSqlitePortalBirthdayPresentationClaimRows(db);
const portalBirthdayPresentationClaimMigrationRequired = !tableExists("schema_migrations")
  || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ? LIMIT 1")
    .get(portalBirthdayPresentationClaimMigrationId)
  || !portalBirthdayPresentationClaimSchemaBeforeMigration.valid
  || !portalBirthdayPresentationClaimRowsBeforeMigration.valid;
...
const saturdayCreditMigrationId = 'saturday-credit-employee-cutover-v1';
const dailyBackupMigrationId = 'backup-daily-20-calendar-days-v1';
const missingMigration = id => !tableExists('schema_migrations') || !db.prepare('SELECT 1 FROM schema_migrations WHERE id=?').get(id);
const saturdayCreditMigrationRequired = missingMigration(saturdayCreditMigrationId)
  || !tableExists('saturday_credit_records') || !triggerExists('trg_saturday_credit_immutable_update') || !triggerExists('trg_saturday_credit_immutable_delete');
const dailyBackupMigrationRequired = missingMigration(dailyBackupMigrationId);
const importStateCountsMigrationRequired = tableExists('data_import_rows') && !tableExists('data_import_run_state_counts');
if (databaseExistedBeforeOpen && (saturdayCreditMigrationRequired || dailyBackupMigrationRequired || importStateCountsMigrationRequired || portalMobileBaselineMigrationRequired || protectedPersonnelMigrationRequired
  ...
  || vacationHistoryProtectionMigrationRequired || payrollHandoffMigrationRequired
   ...
   || centralArticleLoanMigrationRequired
   ...
   || personnelLifecycleProfilePhotoCategoryMigrationRequired
   ...
   || personnelLearningFoundationMigrationRequired
   || personnelLearningCompetencyMigrationRequired
   || personnelLearningAssignmentMigrationRequired
   || personnelLearningProgressMigrationRequired
   || staffAssignmentRequestMigrationRequired
   || portalBirthdayPresentationMigrationRequiredBeforeCatalog
   || portalBirthdayPresentationCatalogMigrationRequired
   || portalBirthdayPresentationClaimMigrationRequired
...
ensureSqliteSalesArticleCatalogSchema(db);

const centralArticleLoanMigrationResult = centralArticleLoanSchemaBeforeMigration.required
  ? migrateSqliteCentralArticleLoans(db)
  : Object.freeze({ migrated: false, articleCount: 0, identifierCount: 0, loanItemCount: 0 });

const portalBirthdayPresentationCatalogMigrationResult =
  portalBirthdayPresentationLegacyV1SchemaBeforeMigration.valid
    ? migrateSqlitePortalBirthdayPresentationCatalogV2(db)
    : Object.freeze({ migrated: false, rowsPreserved: 0 });
const portalBirthdayPresentationSchemaBeforeMigration =
  inspectSqlitePortalBirthdayPresentationSchema(db);
const portalBirthdayPresentationRowsBeforeMigration =
  inspectSqlitePortalBirthdayPresentationRows(db);
const portalBirthdayPresentationMigrationRequired = !tableExists("schema_migrations")
  || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ? LIMIT 1")
    .get(portalBirthdayPresentationMigrationId)
  || !portalBirthdayPresentationSchemaBeforeMigration.valid
  || !portalBirthdayPresentationRowsBeforeMigration.valid;

if (personnelLifecycleMigrationRequired) {
  repairPersonnelLifecycleSchemaBeforeCreate(personnelLifecycleSchemaBeforeMigration);
}
const personnelLifecycleProfilePhotoCategoryMigrationResult =
  personnelLifecycleProfilePhotoCategoryMigrationRequired
    ? migratePersonnelLifecycleProfilePhotoCategory()
    : Object.freeze({ migrated: false, deferred: false });
...
if (personnelLearningFoundationMigrationRequired) {
  repairPersonnelLearningFoundationSchemaBeforeCreate(
    personnelLearningFoundationSchemaBeforeMigration,
    personnelLearningFoundationRowsBeforeMigration,
  );
}
if (personnelLearningCompetencyMigrationRequired) {
  repairPersonnelLearningCompetencySchemaBeforeCreate(
    personnelLearningCompetencySchemaBeforeMigration,
    personnelLearningCompetencyRowsBeforeMigration,
  );
}
if (personnelLearningAssignmentMigrationRequired) {
  repairPersonnelLearningAssignmentSchemaBeforeCreate(
    personnelLearningAssignmentSchemaBeforeMigration,
    personnelLearningAssignmentRowsBeforeMigration,
  );
}
if (personnelLearningProgressMigrationRequired) {
  repairPersonnelLearningProgressSchemaBeforeCreate(
    personnelLearningProgressSchemaBeforeMigration,
    personnelLearningProgressRowsBeforeMigration,
  );
}
if (staffAssignmentRequestMigrationRequired) {
  repairStaffAssignmentRequestSchemaBeforeCreate(
    staffAssignmentRequestSchemaBeforeMigration,
    staffAssignmentRequestRowsBeforeMigration,
  );
}
if (portalBirthdayPresentationMigrationRequired) {
  repairPortalBirthdayPresentationSchemaBeforeCreate(
    portalBirthdayPresentationSchemaBeforeMigration,
    portalBirthdayPresentationRowsBeforeMigration,
  );
}
if (portalBirthdayPresentationClaimMigrationRequired) {
  repairPortalBirthdayPresentationClaimSchemaBeforeCreate(
    portalBirthdayPresentationClaimSchemaBeforeMigration,
    portalBirthdayPresentationClaimRowsBeforeMigration,
  );
}
...
createSchema();
if (dailyBackupMigrationRequired) {
  db.exec('BEGIN IMMEDIATE');
  try {
    db.prepare("INSERT INTO settings(key,value) VALUES('backup_interval_hours','24') ON CONFLICT(key) DO UPDATE SET value='24'").run();
    db.prepare('INSERT OR IGNORE INTO schema_migrations(id,app_version) VALUES(?,?)').run(dailyBackupMigrationId, packageMetadata.version);
    db.exec('COMMIT');
  } catch (error) { db.exec('ROLLBACK'); throw error; }
}
db.prepare('INSERT OR IGNORE INTO schema_migrations(id,app_version) VALUES(?,?)').run(saturdayCreditMigrationId, packageMetadata.version);
...
validatePersonnelLifecycleProfilePhotoCategoryCatalog();
...
validatePersonnelLearningFoundationSchema();
validatePersonnelLearningCompetencySchema();
validatePersonnelLearningAssignmentSchema();
validatePersonnelLearningProgressSchema();
validateStaffAssignmentRequestSchema();
validatePortalBirthdayPresentationSchema();
validatePortalBirthdayPresentationClaimSchema();
...
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
  .run(personnelLifecycleProfilePhotoCategoryMigrationId, packageMetadata.version);
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
...
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
  .run(personnelLearningFoundationMigrationId, packageMetadata.version);
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
  .run(personnelLearningCompetencyMigrationId, packageMetadata.version);
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
  .run(personnelLearningAssignmentMigrationId, packageMetadata.version);
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
  .run(personnelLearningProgressMigrationId, packageMetadata.version);
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
  .run(staffAssignmentRequestMigrationId, packageMetadata.version);
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
  .run(portalBirthdayPresentationMigrationId, packageMetadata.version);
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
  .run(portalBirthdayPresentationCatalogMigrationId, packageMetadata.version);
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
  .run(portalBirthdayPresentationClaimMigrationId, packageMetadata.version);
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
...
ensureColumn("positions", "active", "INTEGER NOT NULL DEFAULT 1");
ensureColumn("positions", "is_default", "INTEGER NOT NULL DEFAULT 0");
ensureColumn("positions", "employment_classification", "TEXT NOT NULL DEFAULT 'standard'");
ensureColumn("positions", "revision", "INTEGER NOT NULL DEFAULT 1");
ensureColumn("positions", "created_by", "TEXT NOT NULL DEFAULT ''");
ensureColumn("positions", "updated_by", "TEXT NOT NULL DEFAULT ''");
ensureColumn("positions", "updated_at", "TEXT NOT NULL DEFAULT ''");
ensureColumn("positions", "archived_by", "TEXT");
ensureColumn("positions", "archived_at", "TEXT");
...
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
  .run(CENTRAL_ARTICLE_LOAN_MIGRATION_ID, packageMetadata.version);
...
ensureColumn("shifts", "duty_code", "TEXT NOT NULL DEFAULT '' CHECK(duty_code IN ('','branch_supervision','department','general') AND (duty_code <> 'department' OR department_id IS NOT NULL) AND (duty_code NOT IN ('branch_supervision','general') OR department_id IS NULL))");
...
db.exec("CREATE INDEX IF NOT EXISTS idx_protected_case_events_entity ON protected_case_events(entity_kind, entity_id, created_at, id)");
  ...
  return Object.freeze({
    ...
    centralArticleLoanMigrationId: CENTRAL_ARTICLE_LOAN_MIGRATION_ID,
    centralArticleLoanMigrationRequired,
    centralArticleLoanMigrationResult,
    ...
    personnelLifecycleProfilePhotoCategoryCatalogBeforeMigration,
    personnelLifecycleProfilePhotoCategoryMigrationId,
    personnelLifecycleProfilePhotoCategoryMigrationRequired,
    personnelLifecycleProfilePhotoCategoryMigrationResult,
    ...
    personnelDocumentHistoryMigrationRequired,
    personnelLearningFoundationMigrationId,
    personnelLearningFoundationMigrationRequired,
    personnelLearningCompetencyMigrationId,
    personnelLearningCompetencyMigrationRequired,
    personnelLearningAssignmentMigrationId,
    personnelLearningAssignmentMigrationRequired,
    personnelLearningProgressMigrationId,
    personnelLearningProgressMigrationRequired,
    staffAssignmentRequestMigrationId,
    staffAssignmentRequestMigrationRequired,
    portalBirthdayPresentationCatalogMigrationId,
    portalBirthdayPresentationCatalogMigrationRequired,
    portalBirthdayPresentationCatalogMigrationResult,
    portalBirthdayPresentationClaimMigrationId,
    portalBirthdayPresentationClaimMigrationRequired,
    portalBirthdayPresentationMigrationId,
    portalBirthdayPresentationMigrationRequired,
