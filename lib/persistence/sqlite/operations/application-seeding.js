...
const upsertPortalRole = db.prepare(`
  ...
  ON CONFLICT(id) DO UPDATE SET
    ...
    builtin = 1,
    permissions = CASE WHEN portal_roles.id = 'developer' OR portal_roles.permissions_customized = 0
      THEN excluded.permissions ELSE portal_roles.permissions END,
    permissions_revision = portal_roles.permissions_revision + CASE
      WHEN (portal_roles.id = 'developer' OR portal_roles.permissions_customized = 0)
        AND portal_roles.permissions <> excluded.permissions THEN 1 ELSE 0 END,
...
// Legacy role restrictions are migrated once. Later Developer decisions survive restarts.
if (!db.prepare("SELECT 1 FROM schema_migrations WHERE id = 'developer-permission-defaults-v1'").get()) {
db.prepare(`
...
// Repair only the placeholder written by v0.92.32, without repeating the
// permission migration or changing its original application timestamp.
db.prepare(`
  INSERT INTO schema_migrations (id, app_version)
  VALUES ('developer-permission-defaults-v1', '0.92.32-beta')
  ON CONFLICT(id) DO UPDATE SET app_version = excluded.app_version
  WHERE schema_migrations.app_version = 'local'
`).run();
