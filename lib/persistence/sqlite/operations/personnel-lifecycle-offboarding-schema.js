...
const PERSONNEL_LIFECYCLE_OFFBOARDING_ASSIGNED_EMPLOYEE_MIGRATION_ID =
  "v0.90.6-personnel-lifecycle-assigned-employee-tasks";
const PERSONNEL_LIFECYCLE_OFFBOARDING_ASSIGNMENT_TRIGGER_NAME =
  "trg_personnel_lifecycle_case_assignments_o5_insert_guard";
...
const PERSONNEL_LIFECYCLE_OFFBOARDING_OWN_TRIGGER_DEFINITIONS = Object.freeze([
  ...
  definition("trg_personnel_lifecycle_case_assignments_o5_insert_guard", `
    ...
    WHEN NOT EXISTS (
      ...
      LEFT JOIN portal_user_roles portal_role ON portal_role.employee_number = portal_user.employee_number
      WHERE lifecycle_case.id = NEW.case_id
              ...
              (portal_user.role = 'employee'
                AND EXISTS (
                  ...
                  WHERE binding.case_id = NEW.case_id
                    AND runtime_step.recipient_class = 'asset_custodian'
                ))
              OR (
                NOT EXISTS (
              ...
              WHERE binding.case_id = NEW.case_id
                ...
                AND (
                  ...
                  OR (runtime_step.recipient_class = 'asset_custodian'
                    AND portal_user.role IN (
                      'employee','manager','department_manager','admin','it_admin','developer'
                    ))
...
function migrateSqlitePersonnelLifecycleOffboardingAssignedEmployeeTrigger(database) {
  ...
  if (!sqliteObject(database, "table", "personnel_lifecycle_offboarding_operations")) {
    return Object.freeze({ applied: false, present: false });
  }
  const expected = PERSONNEL_LIFECYCLE_OFFBOARDING_TRIGGER_DEFINITIONS.find(
    ({ name }) => name === PERSONNEL_LIFECYCLE_OFFBOARDING_ASSIGNMENT_TRIGGER_NAME,
  );
  if (!expected) {
    throw new Error("Der kanonische O5-Zuweisungstrigger fehlt.");
  }
  ...
  database.exec("SAVEPOINT personnel_lifecycle_o5_assigned_employee_trigger");
  try {
    database.exec(
      `DROP TRIGGER IF EXISTS "${PERSONNEL_LIFECYCLE_OFFBOARDING_ASSIGNMENT_TRIGGER_NAME}"`,
    );
    ...
    if (!repaired?.sql
      ...
      throw new Error("Der O5-Zuweisungstrigger konnte nicht kanonisch aktualisiert werden.");
    }
    database.exec("RELEASE SAVEPOINT personnel_lifecycle_o5_assigned_employee_trigger");
    return Object.freeze({ applied: true, present: true });
  } catch (error) {
    database.exec("ROLLBACK TO SAVEPOINT personnel_lifecycle_o5_assigned_employee_trigger");
    database.exec("RELEASE SAVEPOINT personnel_lifecycle_o5_assigned_employee_trigger");
    throw error;
  }
}
