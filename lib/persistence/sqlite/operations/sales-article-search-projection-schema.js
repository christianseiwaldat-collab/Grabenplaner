...
const ARTICLE_SEARCH_PROJECTION_SCHEMA_SQL = `
  CREATE TABLE IF NOT EXISTS sales_article_search_projection (
    ...
    ${PRICES.flatMap(p => [p.column + ' TEXT', p.column + '_sort TEXT']).join(',')}
  );
  ...
  CREATE TRIGGER IF NOT EXISTS trg_sales_article_search_identifier AFTER INSERT ON sales_article_identifiers
    ...
    BEGIN INSERT OR IGNORE INTO sales_article_search_dirty VALUES (NEW.product_id); END;
  CREATE TRIGGER IF NOT EXISTS trg_sales_article_search_price AFTER INSERT ON sales_article_price_snapshots
    WHEN EXISTS (SELECT 1 FROM sales_articles a JOIN sales_article_revisions r
      ON r.product_id=a.product_id AND r.revision=a.current_revision
      WHERE a.product_id=NEW.product_id AND r.source_snapshot_id=NEW.source_snapshot_id)
    BEGIN INSERT OR IGNORE INTO sales_article_search_dirty VALUES (NEW.product_id); END;
...
function ensureSqliteArticleSearchProjection(database) {
  ...
  try {
    ...
    const columns = new Set(database.prepare('PRAGMA table_info(sales_article_search_projection)').all().map(c => c.name));
    let pricesAdded = false;
    for (const column of PRICES.flatMap(p => [p.column, p.column + '_sort'])) if (!columns.has(column)) {
      database.exec(`ALTER TABLE sales_article_search_projection ADD COLUMN ${column} TEXT`); pricesAdded = true;
    }
    if (pricesAdded) database.exec('INSERT OR IGNORE INTO sales_article_search_dirty SELECT product_id FROM sales_articles');
