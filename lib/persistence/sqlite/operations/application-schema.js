"use strict";
const { ensureSqliteDataImportRuntimeSchema } = require("./data-import-runtime-schema");
const { ensureSqliteSaturdayCreditSchema } = require("./saturday-credit-schema");
...
} = require("./personal-action-log-schema");
...
function inspectSqlitePersonalNotificationContactRows(database) {
  ...
  `).all();
  return require("../../../personal-notification-contact-validation.js").validatePersonalNotificationContactRows(rows);
}
...
function ensureSqliteApplicationSchema(database) {
  ...
  ensureSqliteSalesArticleCatalogSchema(sqliteDatabase);
  sqliteDatabase.exec(`
  ...
  `);
  require('./portal-permission-defaults-schema').ensureSqlitePortalPermissionDefaultsSchema(sqliteDatabase);
  ...
  require("./personnel-learning-runs-schema").ensureLearningRunsSchema(sqliteDatabase);
  require("./xoffi-snapshots-schema").ensureSchema(sqliteDatabase);
  ...
  ensureSqliteCrmSchema(sqliteDatabase);
  ensureSqlitePersonalActionLogSchema(sqliteDatabase);
  // Empty protected storage only; no key provisioning or source import at startup.
  ensureSqliteDataImportRuntimeSchema(sqliteDatabase);
  ensureSqliteSaturdayCreditSchema(sqliteDatabase);
}
