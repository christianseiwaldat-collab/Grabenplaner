...
function inspectSqliteImportFile(importPath) {
  ...
  try {
    ...
    const candidateScopedRightsCompatibility =
      inspectSqlitePersonnelLifecycleScopedRightsCompatibility(imported);
    const candidateScopedRightsSchemaState = candidateScopedRightsCompatibility.state;
    ...
    const personnelWorkflowSchemaState = personnelWorkflowSchema.absent
      ...
      : personnelWorkflowSchema.valid && personnelWorkflowRows.valid
        ? "m4"
        : "invalid";
    const personnelWorkflowInstanceSchema =
      inspectSqlitePersonnelWorkflowInstanceSchema(imported);
    const personnelWorkflowInstanceRows =
      inspectSqlitePersonnelWorkflowInstanceRows(imported);
    const personnelWorkflowInstanceSchemaState = personnelWorkflowInstanceSchema.absent
      && personnelWorkflowInstanceRows.valid
      ? "pre-m5-compatible"
      : personnelWorkflowSchemaState === "m4"
        && personnelWorkflowInstanceSchema.valid
        && personnelWorkflowInstanceRows.valid
        ? "m5"
        : "invalid";
    const personnelLifecycleCaseSchema =
      inspectSqlitePersonnelLifecycleCaseSchema(imported);
    const personnelLifecycleCaseRows =
      inspectSqlitePersonnelLifecycleCaseRows(imported);
    const personnelLifecycleOnboardingSchema =
      inspectSqlitePersonnelLifecycleOnboardingSchema(imported);
    const personnelLifecycleOnboardingRows =
      inspectSqlitePersonnelLifecycleOnboardingRows(imported);
    const personnelLifecycleOffboardingSchema =
      inspectSqlitePersonnelLifecycleOffboardingSchema(imported);
    const personnelLifecycleOffboardingRows =
      inspectSqlitePersonnelLifecycleOffboardingRows(imported);
    const personnelLifecycleCaseSchemaState = personnelWorkflowInstanceSchemaState === "m5"
      && personnelLifecycleOffboardingSchema.valid
      && personnelLifecycleOffboardingRows.valid
      ? "o5-offboarding"
      : personnelLifecycleOffboardingSchema.absent
      && personnelWorkflowInstanceSchemaState === "m5"
      && personnelLifecycleOnboardingSchema.valid
      && personnelLifecycleOnboardingRows.valid
      ? "o4-onboarding"
      : personnelLifecycleOffboardingSchema.absent
        && personnelLifecycleOnboardingSchema.absent
        && personnelLifecycleCaseSchema.absent
        && personnelLifecycleCaseRows.valid
        ? "pre-o2-compatible"
        : personnelLifecycleOffboardingSchema.absent
          && personnelLifecycleOnboardingSchema.absent
          && personnelWorkflowInstanceSchemaState === "m5"
          && personnelLifecycleCaseSchema.valid
          && personnelLifecycleCaseRows.valid
          ? "o2-read-only"
          : "invalid";
    const personnelDocumentHistorySchema =
      inspectSqlitePersonnelDocumentHistorySchema(imported);
    const personnelDocumentHistoryRows =
      inspectSqlitePersonnelDocumentHistoryRows(imported);
    const personnelDocumentHistorySchemaState = personnelDocumentHistorySchema.absent
      && personnelDocumentHistoryRows.valid
      ? "pre-m6-compatible"
      : personnelDocumentHistorySchema.valid && personnelDocumentHistoryRows.valid
        ? "m6"
        : "invalid";
    ...
    if (personnelWorkflowInstanceSchemaState === "invalid") {
      protectedInspectionError = true;
    }
    if (personnelLifecycleCaseSchemaState === "invalid") {
      protectedInspectionError = true;
    }
    if (personnelDocumentHistorySchemaState === "invalid") {
      protectedInspectionError = true;
    }
    ...
    return Object.freeze({
      ...
      candidateScopedRightsPredecessorVersion:
        candidateScopedRightsCompatibility.predecessorVersion,
      personnelWorkflowSchemaState,
      personnelWorkflowInstanceSchemaState,
      personnelLifecycleCaseSchemaState,
      personnelDocumentHistorySchemaState,
      protected: Object.freeze({
        personnelEmploymentEpisodes: protectedRows(
          "personnel_employment_episodes",
          "protected_payload",
          `SELECT id, employee_number, protected_payload
           FROM personnel_employment_episodes
           ORDER BY employee_number, sequence_number, id`,
        ),
        personnelLifecycleCases: protectedRows(
          "personnel_lifecycle_cases",
          "protected_payload",
          `SELECT lifecycle_case.id, episode.employee_number,
                  lifecycle_case.protected_payload
           ...
           ORDER BY episode.employee_number, lifecycle_case.created_at, lifecycle_case.id`,
        ),
        personnelLifecycleReferenceDates: protectedRows(
          "personnel_lifecycle_case_reference_dates",
          "protected_payload",
          `SELECT reference.id, episode.employee_number, reference.protected_payload
           ...
           JOIN personnel_employment_episodes episode
             ON episode.id = lifecycle_case.employment_episode_id
           ORDER BY episode.employee_number, reference.changed_at, reference.id`,
        ),
        personnelLifecycleCaseEvents: protectedRows(
          "personnel_lifecycle_case_events",
          "protected_payload",
          `SELECT event.id, episode.employee_number, event.protected_payload
           ...
           JOIN personnel_employment_episodes episode
             ON episode.id = lifecycle_case.employment_episode_id
           ORDER BY episode.employee_number, event.sequence_number, event.id`,
        ),
        personnelLifecycleOffboardingPackageVersions: protectedRows(
          "personnel_lifecycle_offboarding_package_versions",
          "protected_snapshot",
          `SELECT id, series_id, family_code, protected_snapshot
           FROM personnel_lifecycle_offboarding_package_versions
           ORDER BY family_code, series_id, version_number, id`,
        ),
        personnelLifecycleOffboardingPackageVersionArchives: protectedRows(
          "personnel_lifecycle_offboarding_package_version_archives",
          "protected_payload",
          `SELECT package_version_id, protected_payload
           FROM personnel_lifecycle_offboarding_package_version_archives
           ORDER BY archived_at, package_version_id`,
        ),
        personnelLifecycleOffboardingOperations: protectedRows(
          "personnel_lifecycle_offboarding_operations",
          "protected_result_payload",
          `SELECT operation.operation_id, operation.case_id,
                  episode.employee_number, operation.protected_result_payload
           FROM personnel_lifecycle_offboarding_operations operation
           JOIN personnel_lifecycle_cases lifecycle_case
             ON lifecycle_case.id = operation.case_id
           JOIN personnel_employment_episodes episode
             ON episode.id = lifecycle_case.employment_episode_id
           ORDER BY operation.occurred_at, operation.operation_id`,
        ),
        personnelLifecycleOffboardingRuntimeSteps: protectedRows(
          "personnel_lifecycle_offboarding_runtime_steps",
          "protected_payload",
          `SELECT runtime_step.run_id, runtime_step.step_reference,
                  episode.employee_number, runtime_step.protected_payload
           ...
           JOIN personnel_employment_episodes episode
             ON episode.id = lifecycle_case.employment_episode_id
           ORDER BY runtime_step.run_id, runtime_step.sort_order`,
        ),
        personnelLifecycleOffboardingRunTerminations: protectedRows(
          "personnel_lifecycle_offboarding_run_terminations",
          "protected_payload",
          `SELECT termination.run_id, termination.case_id,
                  episode.employee_number, termination.protected_payload
           FROM personnel_lifecycle_offboarding_run_terminations termination
           JOIN personnel_lifecycle_cases lifecycle_case
             ON lifecycle_case.id = termination.case_id
           JOIN personnel_employment_episodes episode
             ON episode.id = lifecycle_case.employment_episode_id
           ORDER BY termination.terminated_at, termination.run_id`,
        ),
        ...
        personnelDocumentVersions: personnelDocumentHistorySchema.valid
          ? protectedRows(
            "personnel_record_document_versions",
            "protected_payload",
            `SELECT version.document_id, version.version_number,
                    document.employee_number, version.protected_payload
