...
// Global scans belong to an actual migration. The domain checks below still
// run on every start, including when the large imported cash tables are present.
const costCenterValidationRequired = !costCenterTypeMigrationApplied
  || costCenterTypeTableMissingBeforeSchema || costCenterTypePositionTableMissingBeforeSchema
  || employeeCostCenterAssignmentMigrationRequired
  || !costCenterMigrationId || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ?").get(costCenterMigrationId);
const integrityValidationPrefix = "organization-integrity-verified-v1:";
if (costCenterValidationRequired) {
  db.prepare("DELETE FROM schema_migrations WHERE id LIKE ?").run(`${integrityValidationPrefix}%`);
}
migrateCostCenterTypes();
const positionGovernanceMigration = migrateSqlitePositionCatalogGovernance(db, { appVersion: packageMetadata.version });
const positionAssignmentMigration = migrateSqlitePositionCatalogAssignments(db, { appVersion: packageMetadata.version });
migrateCostCenters();
...
// Persist success only after the scans finish. A failed or interrupted first
// validation must run again even if the preceding migrations already committed.
const integrityMigrationIds = db.prepare("SELECT id FROM schema_migrations WHERE id NOT LIKE ? ORDER BY id")
  .all(`${integrityValidationPrefix}%`).map(row => row.id);
const integrityValidationId = integrityValidationPrefix + createHash("sha256").update(JSON.stringify(integrityMigrationIds)).digest("hex");
if (costCenterValidationRequired || positionGovernanceMigration.applied || positionAssignmentMigration.applied
  || !db.prepare("SELECT 1 FROM schema_migrations WHERE id = ?").get(integrityValidationId)) {
  ...
  db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)").run(integrityValidationId, packageMetadata.version);
}
    ...
    try {
      db.exec(`
        CREATE TABLE shifts_v071 (
          ...
          duty_code TEXT NOT NULL DEFAULT '' CHECK(duty_code IN ('','branch_supervision','department','general')
            AND (duty_code <> 'department' OR department_id IS NOT NULL)
            AND (duty_code NOT IN ('branch_supervision','general') OR department_id IS NULL)),
        ...
        INSERT INTO shifts_v071
          (id, employee_number, location_id, department_id, duty_code, shift_date, start_time, end_time, area, note, created_at)
        SELECT id, employee_number, location_id, department_id, duty_code, shift_date, start_time, end_time, area, note, created_at
        FROM shifts;
        DROP TABLE shifts;
...
db.prepare("INSERT OR IGNORE INTO schema_migrations (id, app_version) VALUES (?, ?)")
  .run("schedule-duty-v1", packageMetadata.version);
