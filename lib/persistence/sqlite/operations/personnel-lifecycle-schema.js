...
function inspectSqlitePersonnelLifecycleScopedRightsRows(database) {
  ...
  if (!schema.absent && schema.invalidTables.length === 0
    && tableExists(database, "portal_permission_scope_grants")) {
    const dependenciesPresent = tableHasColumns(database, "portal_permission_grants", [
