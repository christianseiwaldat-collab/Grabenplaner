...
const { DATA_IMPORT_COLUMNS } = require('../statements/data-import');
...
const SQLITE_IMPORT_MASTER_CATALOG = Object.freeze([
  entry(S.reportDictionaryRuns, `SELECT ${select(DATA_IMPORT_COLUMNS.RUN)} FROM data_import_runs
    WHERE scope_id=$scopeId AND profile_hash=$profileHash
    AND status IN ('ready','applied') ORDER BY created_at DESC,id DESC LIMIT $limit`),
  ...
  entry(S.crmFind, `SELECT ${select(C.CRM)} FROM crm_customers WHERE lower(account_number)=lower($number) OR (account_number IS NULL AND lower(customer_number)=lower($number))`),
