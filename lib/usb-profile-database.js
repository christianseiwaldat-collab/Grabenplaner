...
const BUILTIN_COST_CENTER_TYPE_IDS = Object.freeze([
  "branch",
  "administration",
  "production",
  "other",
...
function resetTargetDatabase(database) {
  ...
  try {
    ...
    for (const trigger of triggerDefinitions) database.exec(trigger.sql);
...
function populateUsbProfileDatabase({
  ...
  try {
    ...
    const departmentLocations = new Map(departmentRows.map((row) => [Number(row.id), String(row.location_id || "")]));
    ...
    const referencedCostCenterTypeIds = [...new Set(costCenterRows
      .map((costCenter) => String(costCenter.cost_center_type_id || costCenter.type || "other"))
      .filter(Boolean))];
    const exportedCostCenterTypeIds = tableExists(sourceDatabase, "cost_center_types")
      ? [...new Set([...BUILTIN_COST_CENTER_TYPE_IDS, ...referencedCostCenterTypeIds])]
      : referencedCostCenterTypeIds;
    const costCenterTypeRows = exportedCostCenterTypeIds.length && tableExists(sourceDatabase, "cost_center_types")
      ? rows(
        sourceDatabase,
        "cost_center_types",
        `WHERE id IN (${exportedCostCenterTypeIds.map(() => "?").join(",")})`,
        exportedCostCenterTypeIds,
      )
      : [];
    if (tableExists(sourceDatabase, "cost_center_types")) {
      const foundCostCenterTypeIds = new Set(costCenterTypeRows.map((type) => String(type.id || "")));
      if (referencedCostCenterTypeIds.some((id) => !foundCostCenterTypeIds.has(id))) {
        throw new Error("Mindestens ein referenzierter Kostenstellentyp wurde nicht gefunden.");
      }
      if (BUILTIN_COST_CENTER_TYPE_IDS.some((id) => !foundCostCenterTypeIds.has(id))) {
        throw new Error("Mindestens ein eingebauter Kostenstellentyp wurde nicht gefunden.");
      }
    }
    const costCenterTypePositionRows = exportedCostCenterTypeIds.length
      && tableExists(sourceDatabase, "cost_center_type_positions")
      ? rows(
        sourceDatabase,
        "cost_center_type_positions",
        `WHERE cost_center_type_id IN (${exportedCostCenterTypeIds.map(() => "?").join(",")})`,
        exportedCostCenterTypeIds,
      )
      : [];
    ...
    try {
      ...
      insertRows(target, "cost_center_types", costCenterTypeRows);
      insertRows(target, "cost_center_type_positions", costCenterTypePositionRows);
      ...
      for (const employee of uniqueEmployees) {
        ...
        if (!isCreator) {
          ...
          const denials = [...new Set(input.deniedPermissions || [])].filter((permission) => knownPermissions.has(permission));
          insertRows(target, "portal_permission_grants", grants.map((permission) => ({
            employee_number: employee.personnel_number,
            permission,
            granted_by: creatorNumber,
          })));
          insertRows(target, "portal_permission_denials", denials.map((permission) => ({
            employee_number: employee.personnel_number,
            permission,
            denied_by: creatorNumber,
          })));
          let scopes = Array.isArray(input.scopes) ? input.scopes : [];
          if (!scopes.length && ["manager", "department_manager"].includes(role)) {
            scopes = [{
              locationId: employee.home_location_id,
              departmentId: role === "department_manager" ? Number(employee.preferred_department_id) : null,
            }];
          }
          if (!scopes.length && grants.some((permission) => permission === "schedule:read" || permission === "schedule:write")
            && employee.preferred_department_id) {
            scopes = [{ locationId: employee.home_location_id, departmentId: Number(employee.preferred_department_id) }];
          }
          const normalizedScopes = scopes.map((scope) => ({
            locationId: String(scope?.locationId || "").trim(),
            departmentId: Number(scope?.departmentId || 0),
          }));
          for (const scope of normalizedScopes) {
            if (!scope.locationId || scope.locationId !== String(employee.home_location_id || "")
              || !locations.includes(scope.locationId)) {
              throw new Error(`Die Bereichsrechte von ${employee.personnel_number} müssen zur Stammfiliale gehören.`);
            }
            if (scope.departmentId && departmentLocations.get(scope.departmentId) !== scope.locationId) {
              throw new Error(`Die Abteilung in den Bereichsrechten von ${employee.personnel_number} gehört nicht zur Filiale.`);
            }
          }
          if (!normalizedScopes.length
            && grants.some((permission) => permission === "schedule:read" || permission === "schedule:write")) {
            throw new Error(`Für die Dienstplanrechte von ${employee.personnel_number} fehlt ein gültiger Bereich.`);
          }
          const uniqueScopes = new Map(normalizedScopes.map((scope) => [
            `${scope.locationId}:${scope.departmentId}`,
            scope,
          ]));
          insertRows(target, "portal_access_scopes", [...uniqueScopes.values()].map((scope) => ({
            employee_number: employee.personnel_number,
