...
function createSalesReportBatchWorker({ databasePath, keyConfiguration, scopeId = 'grabenplaner-main', today,
  workerFile = path.join(__dirname, 'sales-report-worker.js'), timeoutMs = 120000 }) {
  ...
  function getWorker() {
    ...
    const next = new Worker(workerFile, { workerData: { databasePath, keyConfiguration, scopeId, today },
    ...
    const fail = () => {
      ...
      worker = null; finish(failure('IMPORT_REPORT_WORKER_FAILED')); void next.terminate();
    };
    next.on('error', fail); next.on('exit', fail);
    next.on('message', message => {
      if (worker !== next || !pending || message.id !== pending.id) return;
      const code = /^IMPORT_[A-Z_]+$/.test(message.error || '') ? message.error : 'IMPORT_REPORT_FAILED';
      finish(message.error ? failure(code) : null, message.result);
    });
    ...
    run(input) {
      ...
      return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
          ...
          finish(failure('IMPORT_REPORT_WORKER_TIMEOUT')); void active.terminate();
        }, timeoutMs);
