...
const isPdfReportQuery = query => [2, 3].includes(query?.reportVersion);
...
function normalizeReportQuery(input = {}, { today, projection, locations }) {
  C.exact(input, ['reportVersion', 'sourceId', 'kind', 'dateFrom', 'dateTo', 'comparisonFrom', 'comparisonTo', 'locationId', 'locationIds',
    'manufacturerIds', 'productGroupIds', 'sellerIds', 'groupBy', 'metrics', 'changes', 'chartMetric', 'orientation', 'chartType']);
  ...
  if (!['portrait', 'landscape'].includes(orientation) || !['auto', 'bars', 'shares', 'none'].includes(chartType)) C.fail('IMPORT_REPORT_SELECTION');
...
function add(bucket, line) {
  ...
  const quantity = line.metric.status === 'adjustment' ? 0n : scaled(line.quantity, 6);
...
module.exports = { METRICS, DIMENSIONS, reportAuthority, previousYear, normalizeReportQuery, isPdfReportQuery, manufacturerKey, scaled, decimal, divide, comparison, positionMargin, accumulator, accumulate, finishReport };
