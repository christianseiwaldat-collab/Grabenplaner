...
const DIMENSIONS = Object.freeze([
  { id: 'productGroup', label: 'WGR / Sortiment' }, { id: 'manufacturer', label: 'Hersteller / Marke' },
...
function normalizeReportQuery(input = {}, { today, projection, locations }) {
  C.exact(input, ['reportVersion', 'sourceId', 'kind', 'dateFrom', 'dateTo', 'comparisonFrom', 'comparisonTo', 'locationId', 'locationIds',
    'manufacturerIds', 'productGroupIds', 'sellerIds', 'groupBy', 'metrics', 'changes', 'chartMetric']);
  if (input.sourceId !== 'compact-cash' || input.kind && input.kind !== 'sales' || input.reportVersion && input.reportVersion !== 2) C.fail('IMPORT_HISTORY_KIND');
  ...
  if (!groupBy.length || groupBy.some(id => !DIMENSIONS.some(d => d.id === id))) C.fail('IMPORT_REPORT_SELECTION');
  ...
  return { reportVersion: 2, sourceId: 'compact-cash', dateFrom, dateTo, comparisonFrom, comparisonTo, locationIds,
    productGroupIds: selection(input.productGroupIds, 300), manufacturerIds: selection(input.manufacturerIds, 100, manufacturerKey),
    sellerIds, groupBy, metrics, changes, chartMetric };
}
...
function bucket() { return { records: 0, checked: 0, review: 0, excluded: 0, marginMissing: 0, quantityMissing: 0,
...
function add(bucket, line) {
  bucket.records++;
  if (!line.metric) { bucket.review++; return; }
  if (line.metric.status === 'excluded') { bucket.excluded++; return; }
  bucket.checked++;
  ...
  const margin = line.margin === null ? null : scaled(line.margin);
...
function accumulate(state, phase, query, line) {
  ...
  if (!Object.hasOwn(period.groups, key)) {
    if (Object.keys(period.groups).length >= 5000) C.fail('IMPORT_REPORT_GROUP_LIMIT', 413);
...
function finishReport(state, query) {
  const item = (dimensions, current, previous) => {
    ...
    return { dimensions, metrics: Object.fromEntries(query.metrics.map(id => [id, { current: a[id], previous: b[id],
      ...
      quality: { current: { records: current.records, review: current.review, marginMissing: current.marginMissing }, previous: { records: previous.records, review: previous.review, marginMissing: previous.marginMissing } } };
  };
  ...
  return { rows, total: item([], state.current.total, state.comparison.total), processed: state.processed,
    coverage: Object.fromEntries(['current', 'comparison'].map(phase => [phase, { sourceRecords: state[phase].sourceRecords,
      selectedRecords: state[phase].total.records, review: state[phase].total.review, excluded: state[phase].total.excluded,
      anonymousReceipts: Object.keys(state[phase].total.anonymousReceipts).length, marginMissing: state[phase].total.marginMissing }])) };
}
module.exports = { METRICS, DIMENSIONS, reportAuthority, previousYear, normalizeReportQuery, manufacturerKey, scaled, decimal, divide, comparison, positionMargin, accumulator, accumulate, finishReport };
