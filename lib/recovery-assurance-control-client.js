...
async function recoveryAssuranceControlStatus(options = {}) {
  ...
  try {
    if (process.platform !== "linux" && options.allowNonLinux !== true) {
      return { available: false, busy: false, reason: "ASSURANCE_CONTROL_UNAVAILABLE" };
    }
    const socketPath = assertSafeSocket(options.socketPath || DEFAULT_SOCKET_PATH, options);
    ...
    if (result.code === "ASSURANCE_CONTROL_READY") return { available: true, busy: false, reason: null };
    if (result.code === "ASSURANCE_REQUEST_BUSY") return { available: true, busy: true, reason: "ASSURANCE_REQUEST_BUSY" };
    return { available: false, busy: false, reason: result.code };
  } catch (error) {
    return { available: false, busy: false, reason: String(error?.code || "ASSURANCE_CONTROL_UNAVAILABLE") };
  }
}
