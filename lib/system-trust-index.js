...
function monitorEvidence(diagnostics) {
  ...
  const ageKnown = hasFiniteNumber(monitor.ageHours);
  const maximumAgeHours = hasFiniteNumber(monitor.maximumAgeHours) ? Number(monitor.maximumAgeHours) : 30;
  const current = configured && available && ageKnown && Number(monitor.ageHours) >= 0
    && Number(monitor.ageHours) <= maximumAgeHours && monitor.stale !== true
    && monitor.lastErrorCode !== "MONITOR_STATUS_TIMESTAMP_FUTURE" && monitor.complete === true;
  let state = CHECK_STATES.UNKNOWN;
  if (configured && available && monitor.state === "error") state = CHECK_STATES.FAIL;
  else if (current && monitor.state === "ok") state = CHECK_STATES.PASS;
  return {
    monitor,
    current,
    state,
...
function monitoredCheck(monitorEvidenceValue, id) {
  ...
  if (!monitorEvidenceValue.current) {
...
function buildDatabaseChecks(input, context) {
  ...
  return [
    ...
    check({ id: "database_monitor_integrity", label: database.providerId === "postgresql" ? "Aktuelle PostgreSQL-Prüfung" : "Aktuelle SQLite-Prüfung", points: 7, state: monitoredState, critical: true, reasonCode: reasonFor(monitoredState, "DATABASE_MONITOR_INTEGRITY"), observedAt: monitor.observedAt }),
...
function buildRecoveryChecks(input, context) {
  ...
  // A later verified full recovery proves current recoverability. Keep the
  // failed automation visible and deduct its points, but do not cap the entire
  // current index for that historical run. Disabled/overdue timers stay critical.
  const laterRecoveryProof = automation.reasonCode === "AUTOMATION_RUN_FAILED"
    && chainState === CHECK_STATES.PASS && runState === CHECK_STATES.PASS
    && restoreState === CHECK_STATES.PASS && appSmoke.state === CHECK_STATES.PASS
    && freshnessState(automation.latestFullProofAt, now, 36) === CHECK_STATES.PASS
    && Date.parse(automation.latestFullProofAt) > Date.parse(automation.lastRunAt);
  return [
    ...
    check({ id: "recovery_nightly_automation", label: "Nächtliche Recovery-Automatik", points: 3, state: automationState, critical: automationState === CHECK_STATES.FAIL && !laterRecoveryProof, reasonCode: laterRecoveryProof ? "RECOVERY_AUTOMATION_FAILED_LATER_PROOF" : automation.reasonCode || reasonFor(automationState, "RECOVERY_AUTOMATION"), observedAt: automation.lastRunAt || assurance.generatedAt }),
...
function buildUpdateChecks(input, context) {
  ...
  return [
    ...
    check({
      id: "updates_restart_clear",
      label: restartState === CHECK_STATES.FAIL
        ? "Ubuntu-VPS-Sicherheitsneustart offen"
        : restartState === CHECK_STATES.UNKNOWN
          ? "Status des Ubuntu-VPS-Sicherheitsneustarts"
          : "Kein offener Sicherheitsneustart",
      points: 1,
      state: restartState,
      reasonCode: reasonFor(restartState, "UPDATES_RESTART_CLEAR"),
      observedAt: hostSecurity.checkedAt,
    }),
  ];
}
