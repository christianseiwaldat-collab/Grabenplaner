...
function buildBackupChecks(input, context) {
  ...
  const externalState = serverMode
    ? stateFromBoolean(productionCheck(diagnostics, "backup"))
  ...
  return [
    ...
    check({ id: "backup_external", label: "Getrennte lokale Sicherung", points: 5, state: externalState, critical: serverMode, reasonCode: reasonFor(externalState, "BACKUP_EXTERNAL"), observedAt: backups.latestExternal?.modifiedAt }),
...
function buildRecoveryChecks(input, context) {
  ...
  return [
    ...
    check({ id: "recovery_full_run", label: "Vollständiger Assurance-Lauf", points: 5, state: runState, critical: runState === CHECK_STATES.FAIL, reasonCode: reasonFor(runState, "RECOVERY_FULL_RUN"), observedAt: assurance.generatedAt }),
...
function buildStorageChecks(input, context) {
  ...
  const externalApplicable = diagnostics.backups?.externalEnabled === true;
