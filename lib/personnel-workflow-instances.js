...
function ensureWorkflowMatrix(subjectType, workflowType) {
...
function verifyInstanceRelation(row, publication) {
  ...
  ensureWorkflowMatrix(row.subject_type, publication.row.workflow_type);
...
function createPersonnelWorkflowInstanceService(repositoryValue, {
  ...
  async function authorizeStart(currentRepository, input, publication, access) {
    const subjectValue = await loadSubject(currentRepository, input.subject, publication);
    ...
    ensureWorkflowMatrix(input.subject.type, publication.row.workflow_type);
  ...
  async function replay(currentRepository, existing, input, hash, access, actor) {
    verifyBindingReceipt(existing);
    const publicationRow = await currentRepository.workflowPublicationById({
    ...
    const steps = normalizedSnapshotSteps(publication);
    verifyInstanceRelation(existing, publication);
    ...
    const assignmentRows = await currentRepository.listRunStepAssignments({
      runId: existing.id,
      stepId: null,
    });
    verifyStoredAssignments(assignmentRows, steps, existing, input.assignments);
    const active = await currentRepository.activeRunStep({ runId: existing.id });
    return deepFreeze({
  ...
  async function start(inputValue, { access, actorId } = {}) {
    ...
    try {
      return await repository.transaction(async (currentRepository) => {
        ...
        if (existing) return replay(currentRepository, existing, input, hash, access, actor);
        ...
        const publication = verifiedPublication(publicationRow);
        const authorized = await authorizeStart(
          currentRepository,
          input,
          publication,
          access,
        ...
        await currentRepository.insertRun({
          id: runId,
        ...
        await currentRepository.insertRunBinding({
          runId,
        ...
        for (const step of steps) {
          await currentRepository.insertRunStep({
            runId,
            stepId: step.id,
        ...
        for (const assignment of authorized.assignments) {
          ...
          await currentRepository.insertRunStepAssignment({
            runId,
        ...
        const active = await advanceNewRun(currentRepository, runId, steps);
        assertWriteResult(
          await currentRepository.insertAudit({
            actor,
        ...
        const row = await currentRepository.personnelWorkflowInstanceById({ id: runId });
  ...
  async function completeTask(
    ...
    try {
      return await repository.transaction(async (currentRepository) => {
        ...
        const state = await currentRepository.runStepById({ runId, stepId });
        const assignmentRows = await currentRepository.listRunStepAssignments({
          runId,
          stepId: null,
        });
        ...
        if (["completed", "skipped"].includes(state.status)) {
          ...
          return taskCompletionProjection(
            row,
            state,
            await taskProgress(currentRepository, runId, steps),
            true,
          );
        }
        ...
        const updated = await currentRepository.completeRunStep({
        ...
        const completedStep = await currentRepository.runStepById({ runId, stepId });
        if (!completedStep
        ...
        await advanceNewRun(currentRepository, runId, steps);
        const currentRow = await currentRepository.personnelWorkflowInstanceById({ id: runId });
        if (!currentRow) throw integrityError();
        verifyInstanceRelation(currentRow, publication);
        const progress = await taskProgress(currentRepository, runId, steps);
        assertWriteResult(
          await currentRepository.insertAudit({
            actor: actorId,
