...
- Am 13.09.2026 installiert und auf PostgreSQL umgestellt: 248 Tabellen mit 2.887.718 Zeilen vollständig verglichen, geschützte Dateien und Schlüssel geprüft, erste gemeinsame Sicherung sowie Betriebs-/Offsite-Selbsttests bestanden. Der [Produktivnachweis](docs/postgresql-migration/BLOCK-12-PRODUCTION.md) unterscheidet die erfolgreiche Aktivierung von der abschließenden Wiederherstellungs- und Bedienprüfung.
...
- Im Serverbetrieb ersetzt ein passwortgeschützter Download einer konsistenten und integritätsgeprüften SQLite-Kopie die frühere Konfiguration eines zweiten lokalen Sicherungspfads. Fotos, PDF-Anhänge, verschlüsselte Dokumentdateien und Wiederherstellungsschlüssel bleiben bewusst außerhalb dieses Datenbankdownloads und weiterhin Bestandteil des gekoppelten Sicherungs- und Recovery-Konzepts.
- Datenbankdownload sowie mutierende Offsite-Ordneraktionen verlangen neben der laufenden Sitzung das aktuelle Passwort, eine der Rollen Admin, IT-Admin oder Developer, das jeweils wirksame Einzelrecht sowie CSRF- und Same-Origin-Nachweis. Passwörter, Drive-IDs, Remotes und interne Pfade werden weder in Antworten noch im Auditprotokoll offengelegt.
- Verwaltete Google-Drive-Ordner können ausschließlich unterhalb des festen Grabenplaner-Bereichs aufgelistet, neu angelegt und nach einer gesonderten Warnbestätigung als künftiges Sicherungsziel ausgewählt werden. Freie Pfadeingaben, Umbenennen, Löschen oder Verschieben vorhandener Sicherungen sind nicht möglich.
- Der Zielwechsel übernimmt das bestehende Restic-Repository idempotent, prüft die vollständige und unveränderte Übernahme aller aktuellen Quellobjekte sowie die gepinnte Repository-ID, erzeugt vor der atomaren Umschaltung einen doppelt verifizierten root-only Recovery-Zwischenstand und rollt bei einem Fehler auf die bisherige Bindung zurück. Das alte Drive-Repository bleibt unverändert als Rückfallpunkt bestehen; der neue Recovery-Satz ist anschließend noch getrennt zu übertragen und zu prüfen. Bis dieser Nachweis abgeschlossen und der Server-Zwischenstand entfernt ist, bleibt jeder weitere Zielwechsel gesperrt.
- Der eigenständige Offsite-Modulvertrag steigt dafür auf Version 6. Ein enger root-seitiger Unix-Socket-Broker übernimmt nur den exakt freigegebenen Zielordnervertrag, verwirft freie Befehle und gibt rclone-Aufrufe an das unprivilegierte Offsite-Dienstkonto ab. Bestehende verifizierte Module der Versionen 1 bis 5 werden ausschließlich über den kontrollierten Migrationsweg übernommen.
- Die frühere lokale Zweitsicherung ist im Servermodus nicht mehr als wirksamer Nachweis anwendbar. Ein fehlender Offsite-Nachweis bleibt im System-Center sichtbar, setzt den laufenden Grabenplaner-Dienst jedoch nicht selbst auf „nicht bereit“.
