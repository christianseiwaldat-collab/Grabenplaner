...
v0.86 bündelt die Planarbeitszeit-Regeln in einem eigenen Dashboard. Personalverantwortliche erhalten dort einen gemeinsamen Überblick über gesetzliche Basisprofile, Jugendprofile, Kollektivverträge, betriebliche Regeln, Quellen, Geltungsbereiche und Simulationen. Filial- und Abteilungsleitungen sehen eine datensparsame Lesesicht für die ihnen zugewiesenen Bereiche; globale oder fremde Personalzuordnungen und fachliche Verwaltungsfunktionen bleiben ausgeblendet.

v0.86.3 ergänzt dafür die eng begrenzte Rolle **Planungsverantwortung**. Sie kann die Dienstplanung ausschließlich im ausdrücklich zugewiesenen Standort bearbeiten und dort bereits betrieblich genehmigte Urlaube sowie vereinbarte Zeitausgleiche direkt eintragen. Urlaubs-/ZA-Anträge, Selbstanträge und Freigaben gehören ausdrücklich nicht zu dieser Rolle. Öffnungszeiten, Mindestbesetzung, Teamfarben, Dienstplan-Spitznamen und Telefonnummern bleiben im zugewiesenen Bereich pflegbar. Konkrete Warnmeldungen der Arbeitszeit-Regelprüfung sind beim Planen sichtbar; das Regelwerk-Dashboard, System-Center, Rechteverwaltung, Exporte, Krankenstand/AUM, Leihverwaltung und globale Einstellungen bleiben gesperrt.

Ein bestätigtes Geburtsdatum steuert die Altersanwendbarkeit automatisch. Für Handelslehrlinge unter 18 bleibt das Jugendprofil zusätzlich zu gesetzlichen, kollektivvertraglichen und betrieblichen Regeln wirksam. Fehlende Profildaten werden nicht als bestanden behandelt. Hinweise auf eine noch ungeklärte Anwendbarkeit lassen sich vorübergehend zurückstellen, während konkrete Planverstöße weiterhin direkt bei der Dienstplanung erscheinen.

Das Kollektivvertragsregister trennt Vertrag, Fassung, Quelle, Betriebsteil und zeitliche Zuordnung. Eigene Regeln entstehen als versionierte Entwürfe, werden vor einer Einreichung gegen definierte Testfälle simuliert und können erst nach einem persönlichen Vier-Augen-Verfahren veröffentlicht und gesondert einem Geltungsbereich zugeordnet werden. Inhalt, Quellen, Konfliktstand, Entscheidungen und Wirksamkeit werden nachvollziehbar belegt; historische Fassungen werden nicht überschrieben.

Das Personal-Regelwerk ist eine technische Governance- und Planungshilfe. Es ersetzt weder die fachliche Pflege aktueller Quellen und Kollektivverträge noch die rechtliche Prüfung des konkreten Arbeitsverhältnisses oder einer betrieblichen Ausnahme. Der Abschluss der sieben Entwicklungsblöcke ist unter [Personal-Regelwerk – Abschluss und Produktgrenzen](docs/PERSONAL-REGELWERK-ABSCHLUSS-v0.1.md) dokumentiert.

## Leihmodul in v0.85 Beta

v0.85 integriert die Leihe in das Mitarbeiterportal. Bis zu fünf Artikel können in einem Vorgang ausgegeben, als offene Leihe überblickt und gemeinsam zurückgenommen werden. Neue Leihen werden ausschließlich für den aktuell angemeldeten Mitarbeiter erfasst; auch eine Leitungsrolle kann dabei keine andere Person auswählen. Artikel werden zentral mit exakt sechsstelliger numerischer Artikelnummer geführt; zusätzlich werden gültige EAN/GTIN erkannt und nach einem bestätigten Abgleich lokal wiederverwendet. Standorte können die Leihe und eine optionale Shopware-Artikelsuche getrennt aktivieren; Funktionsprofile und Rollenrechte begrenzen den Zugriff.
...
Ausgabe- und Rücknahmefotos werden auf dem Server neu kodiert, verkleinert, von Metadaten bereinigt und verschlüsselt gespeichert. Beteiligte und berechtigte Leitungen können sie ausschließlich über geschützte, nicht zwischengespeicherte Routen öffnen. Eine neue Leihverwaltung zeigt offene, fällige und abgeschlossene Vorgänge standortbezogen an. Filialleitungen können Vorgänge revisionsgesichert bearbeiten, ohne Gegenbestätigung und ohne neuen Rücknahmebeleg schließen sowie später wieder öffnen. Bestehende Belege und Verlaufseinträge bleiben dabei unverändert erhalten. PDF-Belege werden intern zugestellt; optional kann je Standort ein SMTP-Empfänger für den PDF-Anhang konfiguriert und ein fehlgeschlagener Versand nachvollziehbar wiederholt werden.

Die einmalige Übernahme der bestehenden F18-Lagerware-Daten wurde abgeschlossen. Der frühere Importassistent und seine API-Endpunkte sind deshalb nicht mehr Bestandteil der laufenden Anwendung. Die technische Cutover-Dokumentation bleibt als historischer Nachweis unter [docs/F18-ABLOESUNG.md](docs/F18-ABLOESUNG.md) erhalten.
...
Im HTTPS-Serverbetrieb können Admin, IT-Admin und Developer nach erneuter Prüfung ihres aktuellen Passworts eine konsistente SQLite-Kopie herunterladen und ausschließlich app-eigene Google-Drive-Ordner verwalten. Der Datenbankdownload enthält keine externen Dokumentdateien oder Wiederherstellungsschlüssel. Ein ausdrücklich bestätigter Zielwechsel übernimmt das bestehende Restic-Repository vollständig und geprüft in den ausgewählten Ordner, erzeugt vor der atomaren Umschaltung einen verifizierten root-only Recovery-Zwischenstand und lässt das bisherige Repository als Rückfallpunkt unverändert. Der neue Recovery-Satz muss anschließend noch verschlüsselt auf ein getrenntes Administrationsgerät übertragen und dort geprüft werden; bis zum bestätigten Abschluss blockiert das System jeden weiteren Zielwechsel.
