...
Ab Offsite-Modul 11 können Developer, IT-Administration und Administration mit `backup:write` die Zeitgeber unter **Einstellungen → System & Backup → Wartungs- & Backupzeiten** direkt in einer Matrix bearbeiten. Verfügbar sind Systemprüfung, vollständige Sicherung, Repository-Prüfung, Wiederherstellungstest und – sofern installiert – Sicherheitsprüfung. Die Matrix liest die tatsächlich wirksamen, von systemd zusammengeführten Zeitpläne einschließlich vorhandener Anpassungen; sie zeigt Aktivzustand, Wochentage, Rhythmus, Uhrzeit und den nächsten Lauf. Eine Wochenplanung ohne Wochentag wird nicht gespeichert.

Uhrzeiten gelten in der Serverzeitzone. Zeitpläne mit zusätzlichen Auslösern, etwa nach einem Serverstart, einem abweichenden Zieldienst oder einem in der Matrix nicht vollständig darstellbaren Kalender werden ausdrücklich als nicht bearbeitbar erklärt und unverändert beibehalten. Nicht eingerichtete Vorgänge können hier nicht aktiviert werden.

Beim Speichern werden ausschließlich geänderte Zeilen angewendet. Die wirksamen Werte für zufällige Verzögerung (`RandomizedDelaySec` und `FixedRandomDelay`), Zeitgenauigkeit (`AccuracySec`) und Persistenz (`Persistent`) bleiben erhalten. Für einen bewusst geänderten, anschließend aktivierten Timer wird vor seinem Start der persistente Zeitstempel bereinigt, damit allein das Speichern keine sofortige Nachholung verpasster Termine auslöst. Die Persistenzregel für spätere reguläre Neustarts bleibt bestehen; unveränderte Timer werden weder umgeschrieben noch neu gestartet.

Laufende Arbeiten werden nicht beendet; während eines aktiven Wartungsdienstes ist eine Änderung gesperrt. Ein root-isolierter Broker verwendet die gemeinsame Wartungssperre und prüft die beim Laden erhaltene Revision vor jeder Änderung. Ein inzwischen abweichender Serverstand erfordert erneutes Laden, damit keine fremde Änderung überschrieben wird.

Der gemeinsame Nachtablauf gilt, sobald Anwendungspaket und installiertes Offsite-Modul mindestens Vertragsversion 10 besitzen. Modul 11 ergänzt die geschützte Zeitplansteuerung. Der Installer erhält bei Modulupdates die zuvor erfassten Aktivierungs- und Laufzustände. Vorhandene, gültig gespeicherte Matrixtimer werden beim Appupdate nicht erneut aktiviert; eine bewusst deaktivierte Zeile bleibt deaktiviert. Die Betriebsprüfung unterscheidet diesen gewollten Zustand von einem aktivierten, aber unerwartet stillstehenden Timer. Eine belegte Deploypause wird berücksichtigt. Ohne gespeicherte Matrix stellt der bisherige Übergang den gemeinsamen Nachtablauf her; neue Installationen erhalten dessen Standardzeiten. Upload und Vorbereitung bleiben interne Schritte des gemeinsamen Laufs. Die zwei lokalen Sicherungspaare und die Offsite-Aufbewahrung bleiben erhalten.

Das Speichern mehrerer Matrixzeilen führt nur eine gemeinsame Neuladung der systemd-Konfiguration aus. Die Steuerung berücksichtigt längere Neuladezeiten unter Last und hält ein getrenntes Zeitbudget für ein notwendiges Zurücksetzen bereit. Sichere Phasenmeldungen erscheinen im Journal. Auch der isolierte Wiederherstellungstest veröffentlicht Fortschrittsmeldungen; sein abschließender CLI-Fehler enthält ausschließlich einen freigegebenen Fehlercode, keine ungefilterte Ausnahme oder Zugangsdaten.
