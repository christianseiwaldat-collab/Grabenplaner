# Personalmodul – Fach- und Organisationskonzept Onboarding/Offboarding v1.0

Stand: 2. August 2026

Status: fachlich-organisatorisch freigegeben am 2. August 2026; keine technische Umsetzung, keine Produktivfreigabe und keine Freigabe für Automatisierungen

Freigabeumfang: Die gemeinsame Freigabe umfasst die fachlichen und organisatorischen Regeln, die ausdrücklich beschlossenen Entscheidungen sowie die Reihenfolge O1 bis O8. Sie ermächtigt nicht dazu, O1 oder einen späteren technischen Block ohne gesonderten Startauftrag zu beginnen.

Vorgänger: Diese freigegebene Fassung führt den Konzeptentwurf `PERSONALMODUL-ONBOARDING-OFFBOARDING-FACHKONZEPT-v0.1.md` fort. Der Entwurf bleibt unverändert als historischer Abstimmungsstand erhalten.
...
Für alle in diesem Konzept einer `PL`, einer `zentralen PL` oder einer `fallverantwortlichen PL` zugeordneten zentralen Fachaktionen darf deshalb auch ein Admin-, IT-Admin- oder Developer-Konto als verantwortliche Person ausgewählt werden, wenn das jeweils erforderliche Fachrecht ausdrücklich wirksam ist. Die technische Basisrolle allein begründet weder Fallverantwortung noch vertraulichen Zugriff oder Freigabebefugnis.
...
| Arbeitsplatz, Systeme und Arbeitsmittel | zentrale PL mit ausführender Fachstelle | ausdrücklich konfigurierbares Unternehmenspaket plus lokale Ergänzung | Unternehmen oder Standort |
...
Mitarbeiteridentität, persönlicher Portalzugang und externe Systemzugänge werden getrennt behandelt. Die historische Mitarbeiteridentität darf nicht vor Abschluss der erforderlichen Offboarding-Nachweise entfernt werden. Da der heutige M5-Start einen aktiven Mitarbeiterbezug verlangt, muss der technische Folgeblock den eigenen geschützten Zustand `Austritt läuft` einführen. Damit bleiben historische Identität und notwendige PL-Restbearbeitung bis zum kontrollierten Abschluss erhalten, während der operative Portalzugang und spätere externe Zugänge zum jeweils freigegebenen Sperrzeitpunkt enden können. Eine vorzeitige vollständige Deaktivierung darf keinen unauflösbaren Restprozess erzeugen.
...
Die Bezeichnung `fallverantwortliche PL` benennt in dieser RACI die fachliche Funktion und nicht zwingend die technische Basisrolle des Kontos. Nach Abschnitt 2.4 kann diese Funktion auch eine ausdrücklich fachberechtigte Person mit Admin-, IT-Admin- oder Developer-Konto wahrnehmen.
...
Bei streng vertraulichen Offboarding-Fällen werden auch erfolgreiche Detailansichten, Suchzugriffe mit Treffern, Exporte und Dokumentabrufe in einer geschützten Zugriffsspur protokolliert. Abgewiesene Zugriffe bleiben wertneutral und dürfen die Existenz des Falls nicht offenlegen. Die Zugriffsspur darf nur über ein eigenes, ausdrücklich zugewiesenes Fachrecht gelesen werden; dieses Recht kann einer zentral fachberechtigten PL+-Person und damit auch einem Admin-, IT-Admin- oder Developer-Konto ausdrücklich zugewiesen werden. Es entsteht niemals aus der technischen Basisrolle. Jeder Lesezugriff auf die geschützte Zugriffsspur wird selbst auditiert. Die Aufbewahrungsdauer wird kundenspezifisch mit Datenschutz und Security festgelegt; ohne diese Festlegung bleibt die produktive Aktivierung gesperrt. Vertrauliche Nutzdaten werden nicht in das allgemeine Audit kopiert.
...
## 15. Abgrenzung von O1 und den späteren technischen Ausbaustufen

Die Konzeptfreigabe startet keinen technischen Ausbau. Nach einem gesonderten ausdrücklichen Startauftrag beginnt die technische Umsetzung ausschließlich mit O1. O1 bleibt ein Rechte-, Daten-, Fall- und Testvertrag und umfasst:

- eigene aktionsbezogene Rechte und Datenklassifikationen,
- die Zustandsautomaten und die gemeinsame Fall- und Schutzgrenze für Onboarding und Offboarding,
- positive Projektions- und Auditgrenzen,
- die technische Zielrepräsentation von Beschäftigungsepisode, Fall, Paketbindung und Einzelzuweisung,
- den Schutzvertrag für die vertrauliche Zugriffsspur sowie
- Integritäts-, Vertrags- und Negativtests.

O1 erzeugt keine Onboarding- oder Offboarding-Fälle, Instanzen, Aufgaben oder Profilprojektionen. O1 führt keine produktive Migration, Aktivierung, Automatisierung oder Außenwirkung aus. O2 und O3 bleiben danach jeweils eigene Blöcke: O2 schafft das additive Fall- und Instanzfundament bei weiterhin gesperrten Starts; O3 ergänzt ausschließlich die serverseitig geschützte read-only Onboarding-Vorschau im Mitarbeiterprofil.

Für O1 und die nachfolgenden technischen Blöcke gelten zusätzlich folgende Grenzen:
...
## 16. Freigegebene Entscheidungen und verbleibende Aktivierungsvorbehalte

Die folgenden fachlichen Ausgangspunkte wurden am 2. August 2026 gemeinsam freigegeben. Sie sind verbindliche Konzeptgrenzen. Kundenspezifische Fristwerte, Aufbewahrungsdauern, Rechtsgrundlagen und Integrations-Positivlisten bleiben gesonderte Aktivierungsvoraussetzungen und erhalten ohne ausdrückliche Festlegung keinen produktiven Standardwert.

| Entscheidung | Freigegebener Beschluss |
|---|---|
| Wer ist standardmäßig Fallverantwortlicher? | eine ausdrücklich ausgewählte fachberechtigte PL+-Person; PL, Admin, IT-Admin oder Developer können die Funktion mit dem erforderlichen Fachrecht wahrnehmen; keine Ableitung allein aus Rolle oder Organisation |
| Welche Onboarding-Pakete sind unternehmensweit verpflichtend? | Personal/Administration, Basissicherheit und Datenschutz; konkrete Inhalte werden kundenspezifisch freigegeben; lokale Pakete dürfen nur ergänzen |
| Welche Fristen gelten relativ zu welchem Referenztermin? | je veröffentlichter Prozessversion ausdrücklich festlegen; keine impliziten Standardfristen; offene relative Fristen nur nach bestätigter Neuberechnung ändern |
| Was darf der Mitarbeiter nach dokumentierter persönlicher Information selbst sehen und bestätigen? | erst ab Zustand `Mitarbeiter informiert` nur eigene ausdrücklich freigegebene Aufgaben und Unterlagen; keine PL-internen Gründe, Bewertungen oder Sperrplanung |
| Wer darf Pflichtschritte als nicht anwendbar bestätigen? | eine zentral fachberechtigte PL+-Person mit dem erforderlichen Aktionsrecht; lokale Leitung nur für ausdrücklich lokale optionale Schritte |
| Wie werden Mehrfachzuordnungen und Wechsel während eines laufenden Falls behandelt? | durch ausdrückliche Fallentscheidung und versionierten Scope-Snapshot, nicht automatisch; ohne eindeutigen wirksamen Bereich kein Start |
| Welche Identität gilt bei einer Wiedereinstellung? | immer neue Beschäftigungsepisode und neues Onboarding; Personalnummer und Mitarbeiteridentität bleiben kundenabhängige Stamm-/Lohnverrechnungsentscheidungen; keine automatische Reaktivierung von Portalprofil, Sitzungen, Rollen, Fachscopes oder externen Zugängen; Anerkennung alter Nachweise nur je Kategorie nach ausdrücklicher Prüfung |
| Wann werden Mitarbeiteridentität, Portalzugang und externe Zugänge deaktiviert? | eigener geschützter Zustand `Austritt läuft`; operativer Zugang kann zum freigegebenen Sperrzeitpunkt enden, historische Identität und notwendige PL-Restbearbeitung bleiben bis zum kontrollierten Abschluss erhalten |
| Wie werden Abbruch, Neustart und Korrektur abgebildet? | terminale Fälle nie wieder öffnen; neuer Fall, Wiederherstellungsaufgabe oder Folgeprozess nur mit Vorgängerbezug, Grund, Idempotenz und eigener Rechteprüfung |
| Welche Dokumentkategorien, Aufbewahrungsfristen und Rechtsgrundlagen gelten? | vor Upload-, Bereinigungs- oder Löschautomatik je Kategorie kundenspezifisch dokumentieren und rechtlich prüfen; bis dahin keine produktive Automatik |
| Wo endet Schulungsnachweis und wo beginnt ein externes Lernsystem? | Grabenplaner hält Zuweisung, Status und Nachweis; Lerninhalt nur nach ausdrücklicher Integrationsentscheidung |
| Welche Systeme dürfen später Zugangs- oder Arbeitsmittelaktionen erhalten? | ausschließlich eine freigegebene Positivliste je Integration mit minimaler Projektion, Freigabe, Idempotenz und kontrollierter Rückmeldung |
| Welche Bedeutung haben digitale Kenntnisnahmen? | ausschließlich interner Nachweis ohne Signatur- oder Rechtswirkungsbehauptung |
| Wer darf die geschützte Zugriffsspur vertraulicher Fälle lesen und wie lange bleibt sie erhalten? | eigenes ausdrücklich zugewiesenes Audit-Fachrecht, auch für eine entsprechend berechtigte PL+-Person mit Admin-, IT-Admin- oder Developer-Konto; jeder Zugriff wird selbst auditiert; Dauer kundenspezifisch mit Datenschutz und Security festlegen; ohne Festlegung keine produktive Aktivierung |

Wo eine kundenspezifische Aktivierungsvoraussetzung noch fehlt, bleibt die betreffende Mutation, Datenfreigabe, Bereinigung oder Außenwirkung gesperrt.

## 17. Freigegebene Reihenfolge späterer technischer Issues und PRs

Die Reihenfolge O1 bis O8 ist fachlich freigegeben. Der Beginn jedes einzelnen Blocks benötigt dennoch einen gesonderten ausdrücklichen Startauftrag; die Konzeptfreigabe ist keine Umsetzungsfreigabe.
...
Mit der gemeinsamen Freigabe vom 2. August 2026 sind folgende Abnahmekriterien bestätigt:
...
Die fachliche Freigabe dieses Dokuments ist keine Produktiv-, Release-, Automatisierungs- oder Integrationsfreigabe und startet insbesondere O1 nicht. Das Installationsmerkmal `personnelLifecycle` bleibt deaktiviert, bis die jeweils erforderlichen technischen Verträge umgesetzt, geprüft und ausdrücklich freigegeben wurden.
