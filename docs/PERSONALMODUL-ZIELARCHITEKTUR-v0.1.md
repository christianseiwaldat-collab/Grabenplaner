...
- ein Mitarbeiterprofil mit sieben Tabs, dessen Übersicht, Stammdaten/Organisation und Dokumentliste getrennte positive Read-only-Projektionen verwenden und dessen vier Prozess-/Historientabs fail-closed bleiben,
...
M7 führt eigene Leserechte für Profilübersicht, Stammdaten und Dokumentliste sowie ein getrenntes PL+-Delegationsrecht ein. Schulungs-, Onboarding-, Offboarding-, Historien- und spätere Mutationsrechte erhalten in eigenen Issues weiterhin eigene Aktions-, Sichtbarkeits- und Schutzverträge. Technische Rechte wie Rollenverwaltung, Systemdiagnose, Schlüsselmaterial oder Sicherheitskonfiguration bleiben außerhalb der fachlichen PL+-Datensicht. Das Installationsmerkmal bleibt standardmäßig deaktiviert.
...
M7 erweitert ausschließlich denselben lesenden Profilendpunkt um `tab=master_org` und `tab=documents`. Jeder Abruf liefert nur die positive Projektion des ausdrücklich angeforderten Registers; unbekannte oder gesperrte Register bleiben geschlossen. `/api/employees` ergänzt pro bereits serverseitig gefilterter Person ausschließlich `personnel_profile_access: { available }` als objektspezifischen UI-Einstieg. Unbekannte und außerhalb des wirksamen Fachbereichs liegende Personalnummern erhalten dieselbe `404`-Grenze.
...
- Das Mitarbeiterprofil verwendet pro Tab eine positive Serverprojektion. M7 gibt ausschließlich Übersicht, Stammdaten/Organisation und die sichere M6-Dokumentliste read-only frei; Onboarding, Schulungen, Offboarding und Historie besitzen weder Datenabrufe noch Mutationen. Zusätzliche Registerdaten werden erst beim tatsächlichen Tabwechsel geladen und bei Wechsel, Schließen, Fehler oder jeder Reduktion der wirksamen Profil-, Dokument- und Feldrechte samt laufender Antworten wieder aus DOM und UI-Zustand entfernt.
...
- M7-Profiltests für Feature-Grenze, vier getrennte Rechte, PL-/PL+-Trennung, FL-/AL-Scope-Schnittmenge, identische Objekt-`404`, positive Tabprojektionen, Lazy Loading, Zustandsbereinigung, geschlossene Prozess-/Historientabs und mobile Überläufe,
