...
const root=path.resolve(__dirname,'../../..'),directory=fs.mkdtempSync(path.join(root,'tmp','trade-final-preview-'));
...
async function main(){
 ...
 await source.ingest('ARTIKEL_STAMMGelöscht',[
  {EAN:'OLD-42',Artikelbezeichnung:'Demo Archivobjektiv',Anlagedatum:'2010-01-01T00:00:00.000',Löschdatum:'2026-09-12T00:00:00.000'},
 ...
 await source.ingest('ARTIKEL_STAMM',[{EAN:'000042',Artikelbezeichnung:'Demo Systemkamera',Anlagedatum:'2020-01-01T00:00:00.000'}],{master:true});
 ...
 await require('../../../test-support/trade-suggestions-fixture').seedSuggestions({...source,app:store,protection,vault,state:{session:principal},run:(kind,input)=>runtime.run(getPrincipal,kind,input)},{branches:['93','94'],snapshotAt:'2026-09-25T09:00:00.000Z'});
 const output=path.resolve(root,'../output/Trade-Block6-2026-09-25');fs.mkdirSync(output,{recursive:true});
 const job=await jobs.create(principal,{kind:'stocktakes',query:{},title:'Inventuren - Synthetische Abnahme'});await jobs.tick();
 const saved=await jobs.get(principal,job.id),head=saved.result.rows.find(r=>r.sourceLocation==='93');
 const detailJob=await jobs.create(principal,{kind:'stocktakes',query:{stocktakeId:head.id,stocktakeVersion:String(head.revision),stocktakeSource:head.sourceHash},title:'Inventur 101 - Filiale 93 - Synthetische Abnahme'});await jobs.tick();
 const detail=await jobs.get(principal,detailJob.id);
 fs.writeFileSync(path.join(output,'Inventuren-Abnahme.pdf'),await require('../../../lib/trade-insight-pdf').createTradeInsightPdf(saved,{sort:'date',direction:'desc'}));
 fs.writeFileSync(path.join(output,'Inventur-Differenzen.pdf'),await require('../../../lib/trade-insight-pdf').createTradeInsightPdf(detail,{sort:'difference',direction:'asc'}));
 fs.writeFileSync(path.join(output,'Pruefergebnis.json'),JSON.stringify({synthetic:true,overview:job.id,details:detailJob.id,rows:saved.result.rows.length,positions:detail.result.rows.length},null,2));
 const suggestionJob=await jobs.create(principal,{kind:'suggestions',query:{},title:'Handlungshinweise - Synthetische Abnahme'});for(let i=0;i<5;i++)await jobs.tick();const suggestions=await jobs.get(principal,suggestionJob.id);fs.writeFileSync(path.join(output,'Handlungshinweise-Abnahme.pdf'),await require('../../../lib/trade-insight-pdf').createTradeInsightPdf(suggestions,{sort:'action'}));
 protection.destroy();jobs.start();
 const express=require('express'),preview=express();
 preview.get('/start',(req,res)=>{res.set('Cache-Control','no-store');res.cookie('grabenplaner_session',token,{httpOnly:true,sameSite:'strict'});res.cookie('grabenplaner_csrf',csrf,{sameSite:'strict'});res.redirect('/?view=tradeInsights&section=suggestions');});
 preview.use(app);
 const server=preview.listen(Number(process.env.GP_TRADE_FINAL_PREVIEW_PORT||0),'127.0.0.1',()=>console.log('TRADE_FINAL_PREVIEW=http://127.0.0.1:'+server.address().port+'/start'));
}
