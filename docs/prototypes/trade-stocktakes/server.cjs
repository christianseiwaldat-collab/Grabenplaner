...
const root=path.resolve(__dirname,'../../..'),directory=fs.mkdtempSync(path.join(root,'tmp','stocktakes-preview-'));
...
async function main(){
 ...
 const output=path.resolve(root,'../output/Trade-Block4-2026-09-25');fs.mkdirSync(output,{recursive:true});
 ...
 preview.get('/start',(req,res)=>{res.set('Cache-Control','no-store');res.cookie('grabenplaner_session',token,{httpOnly:true,sameSite:'strict'});res.cookie('grabenplaner_csrf',csrf,{sameSite:'strict'});res.redirect('/?view=tradeInsights&section=stocktakes');});
 preview.use(app);
 const server=preview.listen(Number(process.env.GP_STOCKTAKE_PREVIEW_PORT||0),'127.0.0.1',()=>console.log('STOCKTAKE_PREVIEW=http://127.0.0.1:'+server.address().port+'/start'));
}
