...
function command(runner, executable, args, accepted = new Set([0]), options = {}) {
  const result = runner(executable, args, {
    encoding: "utf8", timeout: options.timeout || 5000, maxBuffer: options.maxBuffer || 4096,
...
function applySchedules(input, options = {}) {
  ...
  try {
    ...
    for (const item of affected) {
      ...
      systemctl([schedule.enabled ? "enable" : "disable", item.task.timer], options);
    ...
    if (changed || stopped.length) {
      try {
        for (const item of stopped) systemctl(["stop", item.task.timer], options);
        for (const item of affected) {
          if (!changed) continue;
          if (item.body === null) fs.rmSync(item.file, { force: true }); else atomicWrite(item.file, item.body, 0o644);
        }
        if (changed) systemctl(["daemon-reload"], options);
        for (const item of stopped) {
          systemctl(item.state.enabled ? ["enable", item.task.timer] : ["disable", item.task.timer], options);
          if (item.state.active) startTimerWithoutCatchUp(item.task, item.effective.properties, options);
        }
