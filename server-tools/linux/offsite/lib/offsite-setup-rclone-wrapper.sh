...
for target in "$setup_root/rclone" "$setup_root/rclone.conf" "$setup_root/read-secret" "$credentials/rclone-config-password"; do
  [[ -f "$target" && ! -L "$target" ]] || exit 1
done
[[ "$(stat --format='%u:%g:%a:%h' -- "$setup_root/rclone")" == "0:0:755:1" ]] || exit 1
[[ "$(stat --format='%U:%G:%a:%h' -- "$setup_root/rclone.conf")" == "grabenplaner-offsite:grabenplaner-offsite:600:1" ]] || exit 1
