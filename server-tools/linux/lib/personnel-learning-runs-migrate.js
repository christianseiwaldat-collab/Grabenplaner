...
async function main(args=process.argv.slice(2)){
 ...
 if(process.platform!=='linux'||process.getuid()!==0||args.length!==2||! /^[a-f0-9]{64}$/.test(expectedManifest||'')
   ||lease!=='--maintenance-lock-held'||fs.realpathSync(path.resolve(__dirname,'../../..'))!==app)throw new Error('PG_LEARNING_MIGRATION_INSTALLED_SCOPE');
 if(fs.realpathSync('/proc/self/fd/9')!=='/run/grabenplaner/maintenance.lock'
   ||require('node:child_process').spawnSync('/usr/bin/flock',['--nonblock','9'],{stdio:Array.from({length:10},(_,i)=>i===9?9:'ignore')}).status!==0)throw new Error('PG_LEARNING_MIGRATION_LEASE');
 ...
 if(hash(manifestBytes)!==expectedManifest)throw new Error('PG_LEARNING_MIGRATION_MANIFEST');
 const manifest=JSON.parse(manifestBytes);
 require('./deploy-policy').applicationContract(app);
 for(const relative of ['server-tools/linux/lib/personnel-learning-runs-migrate.js','lib/persistence/postgresql/core/personnel-learning-runs.js']){
  const entry=manifest.files.find(f=>f.path===relative);
  if(!entry||hash(fs.readFileSync(app+'/'+relative))!==entry.sha256)throw new Error('PG_LEARNING_MIGRATION_SOURCE');
 }
 ...
 try{await client.connect();await client.query("SET lock_timeout='5s';SET statement_timeout='30s'");
  result=await require('../../../lib/persistence/postgresql/core/personnel-learning-runs').migrate(client,{binding:config.binding});
 }finally{await client.end();}
 const after=await R.inspectPair(config);
 if(before.sales.schemaSha256!==after.sales.schemaSha256||before.core.database!==after.core.database)throw new Error('PG_LEARNING_MIGRATION_DOMAIN_DRIFT');
...
if(require.main===module)main().then(result=>process.stdout.write(JSON.stringify(result)+'\n')).catch(e=>{process.stderr.write(JSON.stringify({failed:true,code:/^PG_/.test(e.message)?e.message:e.code||'PG_LEARNING_MIGRATION_FAILED'})+'\n');process.exitCode=1;});
module.exports={main};
