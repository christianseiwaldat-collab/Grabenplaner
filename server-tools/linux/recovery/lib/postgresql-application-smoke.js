...
const {smokeDiagnostics}=require('./postgresql-smoke-diagnostics');
async function qualifyHttp({root,config,rehearsal='application-11',connectionPort=55487,scenario='full',extraScenario,managed=false}){
 ...
 const diagnostic=smokeDiagnostics(root),step=value=>diagnostic.step(value);
 progress('preparing');
 ...
 try{
  ...
  for(const route of ['/api/health/live','/api/health/ready']){
   step(route.endsWith('/live')?'health-live':'health-ready');
   progress(route);
   ...
   diagnostic.response(response.status,200);assert.equal(response.status,200,route);assert.equal(body.ok??body.ready??body.status==='ok',true,route);
  ...
  async function request(route,{method='GET',body,expected=200,anonymous=false,contentType='application/json',stream=false}={}){
   ...
   diagnostic.response(response.status,expected);assert.equal(response.status,expected,route);endpoints.push({route:route.split('?')[0],status:response.status,milliseconds:Math.round(performance.now()-at)});return value;
  }
  try{
   step('login');
   await request('/api/portal/v1/auth/login',{method:'POST',body:{employeeNumber:identity,password},anonymous:true});
   if(extraScenario){step('extra-scenario');return await extraScenario({request,subject,endpoints,identity,password});}
   if(scenario!=='full'){
    step('resume-report');
   ...
   step('concurrent-reads');await request('/api/locations');
   await request('/api/system-info');
   step('historical-articles');
   ...
   step('schedule');await request('/api/schedule?week=2026-09-07&locationId='+encodeURIComponent(location.id));
   step('schedule-pdf');
   ...
   step('receipt-context');const receipts=await request('/api/receipt-search/context');assert.equal(receipts.available,true);
   step('report-context');const context=await request('/api/sales-report-jobs/context');assert.ok(context);
   ...
   step('concurrent-reads');await Promise.all(Array.from({length:8},()=>request('/api/locations')));
   step('import-catalog');const importCatalog=await request('/api/sales/articles/import/catalog');
   const profile=require('../../../../lib/tradefoto-article-source-profile');
   step('import-fixture');const articleNumber=await require('./postgresql-smoke-fixture').unusedArticleNumber(request);
   ...
   step('import-preview');const preview=await request('/api/sales/articles/import/preview',{method:'POST',body:exchange,contentType:importCatalog.mimeType,expected:201});
   assert.equal(preview.summary.create,1,'Synthetic import must contain one valid new article');
   step('import-conflict');await request('/api/sales/articles/import/apply',{method:'POST',body:{previewId:preview.previewId,confirmationFingerprint:'0'.repeat(64)},expected:409});
   step('import-apply');const imported=await request('/api/sales/articles/import/apply',{method:'POST',body:{previewId:preview.previewId,confirmationFingerprint:preview.confirmationFingerprint},expected:201});
   assert.ok(imported.personalActionId,'Import has a durable Core action receipt');
   step('import-search');const importSearch=await request('/api/sales/articles?query='+articleNumber);assert.equal(importSearch.items.length,1);
   step('import-undo');await request('/api/portal/v1/me/actions/'+encodeURIComponent(imported.personalActionId)+'/undo',{method:'POST',body:{}});
   step('import-undo-replay');const replayUndo=await request('/api/portal/v1/me/actions/'+encodeURIComponent(imported.personalActionId)+'/undo',{method:'POST',body:{}});assert.equal(replayUndo.alreadyUndone,true);
   step('work-rule-integrity');const workRuleReceipts=await subject.ensureWorkRuleEvaluationReceiptIntegrity({deep:true});assert.ok(Number.isSafeInteger(workRuleReceipts));
   step('receipt-search');const receiptResult=await request('/api/receipt-search/search',{method:'POST',body:{sourceId:'compact-cash',kind:'receipts',dateFrom:'2026-01-23',dateTo:'2026-01-23',locationId:'18',limit:20,query:'Sony'}});assert.ok(receiptResult);
   step('report-create');const job=await request('/api/sales-report-jobs',{method:'POST',body:{title:'PostgreSQL Generalprobe Januar',query:{sourceId:'compact-cash',reportVersion:3,kind:'sales',dateFrom:'2026-01-01',dateTo:'2026-01-31',locationIds:['18'],groupBy:['manufacturer'],metrics:['netRevenue','grossMargin','quantity','receiptCount'],orientation:'landscape',chartType:'bars'}}});
   step('report-run');
   let report;
   ...
   step('report-pdf');const salesPdf=await request('/api/sales-report-jobs/'+job.id+'/download');assert.ok(Buffer.isBuffer(salesPdf)&&salesPdf.subarray(0,5).toString()==='%PDF-');
   step('report-delete');await request('/api/sales-report-jobs/'+job.id,{method:'DELETE'});
 ...
 }catch(error){diagnostic.failed(error);throw error;}finally{
