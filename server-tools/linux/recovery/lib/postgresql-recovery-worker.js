...
async function main(){
 ...
 const result=await restorePair({bundle,commitMarker,workRoot:root+'/work',privateApplicationNetwork:true,async onRestored({config,connect,workRoot}){
  ...
  return {...smoke,compatibility:true};
 }});
 fs.writeFileSync(root+'/result.json',JSON.stringify(result)+'\n',{flag:'wx',mode:0o600});
}
main().catch(error=>{process.stderr.write(JSON.stringify({failed:true,code:error.code||null,error:/^PG_/.test(error.message)?error.message:'PostgreSQL recovery failed'})+'\n');process.exitCode=1;});
