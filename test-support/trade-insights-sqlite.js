...
async function fixture(t,{databasePath=':memory:',vaultKey=Buffer.alloc(32,42),resume=false}={}){
 const app=require('../lib/persistence/sqlite/provider').openSqliteApplicationPersistence({databasePath,catalog:require('../lib/persistence/sqlite/application-catalog').SQLITE_APPLICATION_CATALOG});
 const db=app.database;if(!resume){db.exec(require('../lib/persistence/sqlite/trade-annotations-catalog').TRADE_ANNOTATIONS_SCHEMA);db.exec(`CREATE TABLE audit_log(id INTEGER PRIMARY KEY,actor TEXT,action TEXT,entity_type TEXT,entity_id TEXT,detail TEXT,created_at TEXT);
 ...
 const vault=require('../lib/integration-secret-vault').createIntegrationSecretVault({activeKeyId:'synthetic',keys:{synthetic:vaultKey}});
 ...
 const source=await insightFixture({access:app.provider,protection,seedBase:!resume});
